...
simulated event SpawnChunkDestroyEffect(ParticleSystem Effect, box ChunkBox, vector ChunkDir, float Scale)
{
	...
	//`log("Spawning check effects at: "$ChunkMiddle);
	EffectComp = WorldInfo.MyEmitterPool.SpawnEmitter(Effect, ChunkMiddle, rotator(ChunkDir));
	EffectComp.SetScale(Scale);
}

// Firaxis BEGIN
// RAM - supporting effects spawned from quantizing the total area
simulated event SpawnChunkDestroyEffectFromPt(ParticleSystem Effect, vector EffectPos, vector ChunkDir, float Scale)
{
	local ParticleSystemComponent EffectComp;
	EffectComp = WorldInfo.MyEmitterPool.SpawnEmitter(Effect, EffectPos, rotator(ChunkDir));
	EffectComp.SetScale(Scale);
}
// Firaxis END
