...
class TextureRenderTarget2D extends TextureRenderTarget
	...
	//hidecategories(Texture);  // FIRAXIS change
...
var() const EPixelFormat Format;   // FIRAXIS change  - exposing  texture format to the editor
...
var() const bool bForceLinearGamma; // FIRAXIS change
/** Firaxis Addition to address gamma correction issues with GFX */
var() bool bGFXRenderTarget;

// Callback for when a capture is requested
delegate OnScriptConstructTextureFinished(Texture2D FinishedTexture);
...
cpptext
{
	 ...
	 */
	UTexture2D* ConstructTexture2D(UObject* Outer, const FString& NewTexName, EObjectFlags ObjectFlags, DWORD Flags=CTF_Default, TArray<BYTE>* AlphaOverride=NULL, UBOOL bSaveSourceArt=FALSE, UBOOL bForceAlphaOne=FALSE);
...
static native noexport final function TextureRenderTarget2D Create(int InSizeX, int InSizeY, optional EPixelFormat InFormat = PF_A8R8G8B8, optional LinearColor InClearColor, optional bool bOnlyRenderOnce, optional bool bInForceLinearGamma, optional bool bInNeedsTwoCopies, optional Object InOuter );
static native final function Resize(TextureRenderTarget2D RenderTarget, int InSizeX, int InSizeY);
//FIRAXIS END

//If a delegate is specified, the construction will happen on a worker thread ( avoid stalls )
native final function Texture2D ConstructTexture2DScript(Object OuterObject, String NewTexName, BOOL bCompress, BOOL bSRGB, BOOL bGenerateMips, delegate<OnScriptConstructTextureFinished> TextureConstructedCallback = none);
