...
// FIRAXIS begin
struct native CanvasVertex
{
	var Vector2D Position;
	var Vector2D TexCoord;
};
// FIRAXIS end
...
var const int SizeX, SizeY;  // Zero-based actual dimensions.
	...
	//Firaxis Change - JW to blend mode
var int BlendMode;	 // Blend operation for drawing tiles
...
cpptext
{
	...
	FVector ProjectNative(FVector Location);
}
...
// FIRAXIS begin
//helper function
function CanvasVertex MakeCanvasVertex(float X, float Y, float U, float V)
{
	local CanvasVertex NewCanvasVertex;
	NewCanvasVertex.Position = vect2d(X, Y);
	NewCanvasVertex.TexCoord = vect2d(U, V);
	return NewCanvasVertex;
}

// Draws the static mesh without any transformations.  To be used with materials that use unwrapped mode.
//native final function DrawStaticMesh( StaticMeshComponent StaticMesh, MaterialInterface Mat ); // FIRAXIS addition
// FIRAXIS end
...
event Reset(optional bool bKeepOrigin)
{
	...
	BlendMode   = BLEND_Translucent; //Firaxis change
	if ( !bKeepOrigin )
	{
		OrgX        = Default.OrgX;
		OrgY        = Default.OrgY;
...
//FIRAXIS Addition -sboeckmann -Add support for reserving space in the line array through the canvas
native final function BatchedLinesReserve(int Num);
...
// FIRAXIS
native final function DrawTriangle(Texture Tex, CanvasVertex V1, CanvasVertex V2, CanvasVertex V3);
...
defaultproperties
{
	...
	ColorModulate=(X=1,Y=1,Z=1,W=1)
     BlendMode=BLEND_Translucent	// FIRAXIS
