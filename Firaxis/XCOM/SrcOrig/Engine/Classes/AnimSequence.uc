 ...
 *   . . .
 */
//var	native	Array_Mirror	CompressedTrackOffsets {TArray<INT, TPooledHeapAllocator<ePool_Anim> >};
 ...
 */
var	native	Array_Mirror	CompressedByteStream {TArray<BYTE, TPooledHeapAllocator<ePool_Anim> >};
//var native array<byte> CompressedByteStream;

// if not -1, this refers to a stream index whose CompressedByteStream exactly matched the current AnimSequence data
var int CompressedStreamRefIndex;
var private transient native pointer CompressedData;
...
cpptext
{
	...
	typedef TArray<BYTE, TPooledHeapAllocator<ePool_Anim> > CompressedByteStreamArray;

	// set by postload to either the current CompressedByteStream Data, or the referred to one if the byteStream is a duplicate
	CompressedByteStreamArray& GetCompressedData() const
	{
		return GIsEditor ? (CompressedByteStreamArray&)CompressedByteStream : (*( (CompressedByteStreamArray *) CompressedData));
	}

	void SetCompressedStreamData(CompressedByteStreamArray *pData)
	{
		CompressedData = pData;
	}
}
