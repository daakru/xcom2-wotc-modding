...
struct MaterialViewRelevance
{
	...
	var bool bUsesOcclusionPercentage;
	var bool bForceOcclusion;
	var bool bDisableDepthTest; //Firaxis Change - JW
	// FIRAXIS End
...
var const native transient int CheckTag[8];
...
// FIRAXIS Begin - mgiordano
/** The primitive's secondary scene info. This allows rendering the primitive again in a frame using different rendering settings*/
var private native transient const pointer SecondarySceneInfo{FPrimitiveSceneInfo};
// FIRAXIS End
...
// FIRAXIS BEGIN
// Storing these variables on the component and accessing/writing them from the render thread
//  is much more efficient than storing them in a hash because pretty much every primitive needs to be able to do this. 
//  Obviously there are hazards with this but it shouldnt actually ever matter

// Cutdown/cutout heights are set immediately by SetPrimitiveCutdownFlag and SetPrimitiveCutoutFlag
var native transient float fCutoutMaskHeight;
var native transient float fCutdownMaskHeight;

// The opacity mask height is either the cutout or cutdown height, depending on what flags are set. The floor component
//     will set these at the beginning of a transition in its ::Tick()
var native transient float fOpacityMaskHeight;
var native transient float fPreviousOpacityMaskHeight;

// The current visibility transition fade amount and the target it is being lerped towards
var native transient float fCutoutFade; 
var native transient float fTargetCutoutFade; 
// FIRAXIS END
...
/** Shift the local to world transform along the camera vector to tweak sorting. */
var(Rendering) float fCameraOffset <DisplayName=CameraOffset>; // FIRAXIS ADDITION
...
/** Increases the hide height of a mesh for building vis. Used to allow certain pieces to render above typical floor level. */
var(Rendering) float CutDownOffset;
...
var(Rendering) const bool   HideDuringCinematicView; // if true, this component will not render while cinematic mode is on

// FIRAXIS Begin - mgiordano
/** If this is True, then this primitive will use an alternative depth rendering shader to cutout surfaces when appropriate. **/
var (Rendering) bool bCutoutMask; // EPACE
/** If this is True, then the portion of the mesh cutout due to height ("cutdown") will appear scanlined */
var (Rendering) bool bCutdownScanup; // JMS

/** If this is True,  then this primitive is currently applying cutout **/
var transient bool bShouldCutout; // JMS
/** If this is True, then this primitive is currently applying cutdown **/
var transient bool bShouldCutdown; // JMS

var transient bool bPerformingVisFade; // JMS

/* If TRUE, this primitive will render with the dither effect (similar to our cutout dithering) */
var transient bool bShouldProximityDither;

/** If this is True, this primitive will need to create a secondary proxy and scene info. */
var bool bNeedsSecondaryProxy;
/** If this is True, then the primitive will not draw using the primary proxy (the secondary may still be used) */
var bool bNeedsPrimaryProxy; // EPACE

/** If this is True, track the visibility of this primitive on the gamethread */
var bool bNeedsGameThreadVisibility; // JMS

/** True when the object is hidden because of building visibility. */
var bool bHiddenByBuildingVisibility; // SAB

/** If this is True, then decals consider this primitive component as a receiver even if it (this primitive component) is hidden */
var bool bReceiverOfDecalsEvenIfHidden; // EPACE
// FIRAXIS End
...
/** If this object is Translucent, it will never be FOW'd out */
var(Rendering) bool bTranslucentIgnoreFOW; //FIRAXIS ADDITION

/** If true, the object will always bilinear filter when applying the LPV */
var(Rendering) bool bForceLPVBilinearFiltering;

/** Percentage of height of bounding box to use when calculating height fog for translucent objects */
var(Rendering) float HeightPercentageForHeightFog; // FIRAXIS ADDITION

/** Percentage of height of bounding box to use when calculating fog of war for translucent objects */
var(Rendering) float HeightPercentageForFogOfWar; // FIRAXIS ADDITION

enum ETranslucentDOFOption
{
	eTO_NormalDOF,
	eTO_NeverDOF,
	eTO_AlwaysDOF
...
/** Used to indicate how Translucent objects should interact with DOF 
 *  eTO_NormalDOF - Object will move between pre and post DOF passes, depending if the object is calculated to need DOF
 *  eTO_NeverDOF - Object will always render after DOF pass, effectively never being blurred
 *  eTO_AlwaysDOF - Object will always render before the DOF pass, effectively taking the DOF of the objects behind it
 *  */
var(Rendering) ETranslucentDOFOption TranslucentDOFOptions; // FIRAXIS ADDITION
...
/** If TRUE will block line checks for the movement grid, FALSE will skip right through it. */
var(Collision)  const bool  bBlockMovementGridChecks;
...
/** When this is set, any lightmap data that is present is ignored */
var bool bIgnoreLightMapData; // FIRAXIS ADDITION
...
//FIRAXIS BEGIN
struct RenderChannelContainer
{
	var()	bool	MainScene;
	var()	bool	UnitVisibility;
	var()   bool    RainCollisionStatic;
	var()   bool    RainCollisionDynamic;
	var()   bool    Occluded;
	var()   bool    Occluded2;
	var()	bool	SecondaryScene; // Check for SecondaryScene is currently used (as a hack) to avoid improper occlusion on soldiers in Photobooth. This prevents the Klingon bug in Photobooth.
	var()	bool	XenoformGrass;
};

var(Rendering) const RenderChannelContainer RenderChannels;
/** If this mesh is rendered scanline, which outline group should it end up in? */
var (Rendering) EVisibilityGroup VisOutlineGroup; 
//FIRAXIS END
 ...
 * @param WakeTime The minimum amount of time this body will remain awake (defaults to NX_SLEEP_INTERVAL)
 */
native final function WakeRigidBody(optional name BoneName, optional float WakeTime);
 ...
 */
final native function SetRBCollidesWithChannel(ERBCollisionChannel Channel, bool bNewCollides, optional bool bManualUpdate = false);
 ...
 */
final native function SetRBChannel(ERBCollisionChannel Channel, optional bool bManualUpdate = false);
...
//FIRAXIS BEGIN
native final function SetRenderChannels(RenderChannelContainer NewRenderChannels);
//FIRAXIS END

native final function SetTranslucencySortPriority(int Priority); //FIRAXIS ADDITION
...
// Checks bNeedsReattach and reattaches if true.
native function ReattachIfNeeded();
...
defaultproperties
{
	...
	bBlockMovementGridChecks=TRUE
	bAllowAmbientOcclusion=TRUE
	bTranslucentIgnoreFOW=FALSE //FIRAXIS ADDITION
	bForceLPVBilinearFiltering=false

	// FIRAXIS BEGIN
	CanBlockCamera=false
	bNeedsPrimaryProxy=true
	bNeedsGameThreadVisibility=false
	bHiddenByBuildingVisibility=false
	bReceiverOfDecalsEvenIfHidden=FALSE
	VisibilityId=-1
	bNeedsSecondaryProxy=false
	bIgnoreLightMapData=false 
	bCutoutMask=false 
	bShouldCutout=false
	bShouldCutdown=false
	VisOutlineGroup=VisGroup_Outline1
	bShouldProximityDither=false
	
	// Primitives that aren't explicitly cached and subsequently updated by building vis.
	//  floor changes need default vis height values that basically cut out the whole thing
	fCutoutMaskHeight=-999999.0
	fCutdownMaskHeight=999999.0


	fCutoutFade=1.0; 
	fTargetCutoutFade=1.0; 
	fCameraOffset=0.0
	HeightPercentageForHeightFog=0.5
	HeightPercentageForFogOfWar=0.5
	TranslucentDOFOptions=eTO_NormalDOF
