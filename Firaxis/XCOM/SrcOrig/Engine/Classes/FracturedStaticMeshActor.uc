...
// Firaxis BEGIN
// RAM - This is reset and filled out inside of BreakOffPartsInRadius. This information is used later to efficiently rebuild the nav mesh.
var transient init array<INT> BreakOffPartsResults;
// Firaxis END
...
// FIRAXIS change - remove 'final' keyword
simulated native function FracturedStaticMeshPart SpawnPartMulti(array<int> ChunkIndices, vector InitialVel, vector InitialAngVel, float RelativeScale, bool bExplosion);

native final function array<int> GetBreakOffParts() const; // FIRAXIS ADDITION
native final function ClearBreakOffParts();
...
simulated event PostBeginPlay()
{
	...
	// FIRAXIS REMOVAL
	// The FSM components now reset their own visibility.. this causes it to happen twice
	//ResetVisibility();
}

/** Used to init/reset health array. */
simulated native function ResetHealth(); // FIRAXIS jboswell: removed final
 ...
 */
native simulated event BreakOffIsolatedIslands(out array<int> FragmentVis, array<int> IgnoreFrags, vector ChunkDir, array<FracturedStaticMeshPart> DisableCollWithPart, bool bWantPhysChunks);  // Firaxis change
...
{
	local array<int> FragmentVis;  // Firaxis change
