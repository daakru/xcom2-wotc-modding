...
// FIRAXIS Change -sboeckmann
// The enum EParticleSysParamType and ParticleSysParam was moved to ParticleSystem.uc
/**
 ...
 */
var()	editinline array<ParticleSystem.ParticleSysParam>		InstanceParameters;
...
/** Used to set the minimum Tick value for the particle */
var()   float                                   fMinimumTickRate;
...
var(Rendering)     float       DownsampleThresholdDistance; //FIRAXIS Addition -sboeckmann
...
enum EParticleEventType
{
	...
	EPET_Kismet,
	/** DebrisSpawn - an event generated when debris is spawned by the particle system */
	EPET_Debris
};
...
/**
 *  Debris particle event data.
 */
struct native ParticleEventDebrisData extends ParticleEventdata
{
};
...
/** The Debris event that occured for this PSysComp. */
var transient array<ParticleEventDebrisData>    DebrisEvents;
...
var transient int GameThreadDynamicDataIndex;	

struct native DynamicParticleStruct
{
	var native pointer DynamicData{FParticleDynamicData};
	var native ParticleSystem Template;
	var native int LOD;
	var native bool bForceReset;
...
var native transient DynamicParticleStruct aParticleDynamicData[2];

struct native DeferredDynamicDeletionData
{
	var native pointer DynamicData{FParticleDynamicData};
	var native pointer pRenderFence{FRenderCommandFence};
};

var native transient array<pointer> aDeferredDynamicDataDeletion{FDeferredDynamicDeletionData};

var native transient pointer pRenderFence{FRenderCommandFence};
...
native final function DeactivateSystem(bool bForceRemoveOnDeactivate = false);
native final function KillParticlesForced();

native final function SetDefaultValuesFromTemplate();
...
/**
 * FIRAXIS CHANGE: Has the particle system completed?
 * Originally this was native-only in cpptext, Firaxis change: moved it as a native function.
 **/
native function bool HasCompleted();
...
// FIRAXIS Addition -raasland
native function JumpForward( float DeltaTime );
...
protected:
	...
	/** Orients the X axis towards the camera, Z axis becomes screenspace up.*/
	void OrientXAxisFullTowardCamera();

public:
#if PARTICLE_DOUBLE_BUFFER
	void HandleDeferredDynamicDeletion(FParticleDynamicData* pData);

	FParticleDynamicData* GetParticleDynamicData();
#endif
	...
	// FIRAXIS BEGIN
	//    JMS - Adding this functionality to make PSCs act more like other PCs (needed for matinee UInterpTrackFloatParticleParam etc. )
	//			Hopefully no existing engine code relies on this function not being implemented :\
	//          SetElementMaterial is currently not implemented as materials in the PSC is not as simple as other PCs and
	//          Epic will most likely fix this in the future.
	// Actually returns the number of materials
	virtual INT GetNumElements() const;

	virtual UMaterialInterface* GetElementMaterial(INT ElementIndex) const;

	// Returns a specific material used by the PSC
	...
	// FIRAXIS Begin
	// Are there any mesh particles in the system that are casting shadows?
	UBOOL ShouldCastShadow();
	// FIRAXIS End

//	void InitializeSystem(); -FIRAXIS CHANGE, made accessible in script;
	...
	*/
	...
	/**
	 * Record a Debris Spawn event.
	 */
	void UParticleSystemComponent::ReportEventDebris(FName& InEventName, FLOAT InEmitterTime, FVector& InLocation, 
												 FVector& InDirection, FVector& InVelocity);
...
native final function SetMeshParameter(name ParameterName, StaticMesh Param);
native final function SetSkeletalMeshParameter(name ParameterName, SkeletalMeshComponent Param); // FIRAXIS CHANGE -raasland
native final function SetEmitterEnabledParameter(name ParameteRName, bool Param );
...
/**
 *	Retrieve the Mesh parameter value for the given name.
 *
 *	@param	InName		Name of the parameter
 *	@param	OutMesh  	The value of the parameter found
 *
 *	@return	TRUE		Parameter was found - OutMesh is valid
 *			FALSE		Parameter was not found - OutMesh is invalid
 */
native function bool GetMeshParameter(const name InName, out StaticMesh OutMesh);
native function bool GetSkeletalMeshParameter(const name InName, out SkeletalMeshComponent OutMesh); // FIRAXIS CHANGE -raasland
...
defaultproperties
{
	...
	bAllowApproximateOcclusion=true; // FIRAXIS ADDITION - particle systems usually don't need this 

	DownsampleThresholdDistance=0.0f //FIRAXIS Addition

	fMinimumTickRate=0.0f
	bRemoveWithDestruction=false;
}
