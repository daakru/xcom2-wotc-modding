...
// FIRAXIS change: removed const-ness as our games can have different settings based on member variable values and such. -tsmith
...
/** If true, destroy and recreate online sessions at mapchange (Steam used to require this, but no longer) */
...
{
	T = T * GameSpeedMultiplier;
	GameSpeed = FMax(T, 0.00001);
	WorldInfo.TimeDilation = GameSpeed;
	SetTimer(WorldInfo.TimeDilation, true);
}

function SetGameSpeedMultiplier(FLOAT T)
{
	local int BaseGameSpeed;
	BaseGameSpeed = GameSpeed / GameSpeedMultiplier;
	GameSpeedMultiplier = FMax(T, 0.00001);
	...
	*/
	...
	if (WorldInfo.NetMode == NM_ListenServer || WorldInfo.NetMode == NM_DedicatedServer )
	{
		...
		if (WorldInfo.IsInSeamlessTravel())
		{
			...
			{
				if (CurAC.bPendingPostSeamlessInit)
				{
					CurAC.bPendingPostSeamlessInit = False;
					AccessControl = CurAC;
	...
	if ( ParseOption( Options, "AutoTests" ) ~= "1" || class'Engine'.static.GetEngine().bAutoTesting) //bAutoTesting indicates the app was launched in autotest mode
	{
		if (class'Engine'.static.GetEngine().AutoTestClassName != "")
		{
			AutoTestManagerClass = class<AutoTestManager>( class'Engine'.static.FindClassType( class'Engine'.static.GetEngine().AutoTestClassName ) );
		}
	...
	if (OnlineSub != None)
	{
		...
		// FIRAXIS: dont add the code block about Steamworks and creating a new online session back. the 3rd party Steamworks code removed that. -tsmith 
	}
...
function ProcessServerTravel(string URL, optional bool bAbsolute)
{
	...
	if (AccessControl != none)
	{
		AccessControl.NotifyTravel(bSeamless);
	}
...
event PlayerController Login(string Portal, string Options, const UniqueNetID UniqueID, out string ErrorMessage)
{
	...
	NewPlayer = SpawnPlayerController(StartLocation, SpawnRotation);
...
function StartOnlineGame()
{
	...
	if (GameInterface != None)
	{
		...
		{
			// FIRAXIS begin: dont make client calls on PlayerControllers that are about to be destroyed. This will cause the wrong controllers and bad logic on clients. -tsmith
			if(PC.bDeleteMe || PC.PendingSwapConnection != none)
			{
				`log(self $ "::GameInfo::" $ GetFuncName() @ "Skipping StartOnlineGame for" @ `ShowVar(PC) 
					@ `ShowVar(PC.bDeleteMe) 
					@ `ShowVar(PC.PendingSwapConnection) 
					@ `ShowVar(PC.IsLocalPlayerController()), true, 'XCom_Online');
				continue;
			}
			// FIRAXIS end: dont make client calls on PlayerControllers that are about to be destroyed. This will cause the wrong controllers and bad logic on clients. -tsmith

			// Skip notifying local PCs as they are handled automatically
			if (!PC.IsLocalPlayerController())
			{

				PC.ClientStartOnlineGame(PlayerReplicationInfoClass.default.SessionName);
			}
		}
...
function Pawn SpawnDefaultPawnFor(Controller NewPlayer, NavigationPoint StartSpot)
{
	...
	ResultPawn = Spawn(DefaultPlayerClass,,,StartLocation,StartRotation,,true); // FIRAXIS - No failing on collisions
	if ( ResultPawn == None )
	{
...
function Logout( Controller Exiting )
{
	...
	if ( PC != None )
	{
		...
		{
			// TODO: notify the online systems that a player has dropped during arbitration. also need a way for clients to determine if host dropped. -tsmith 
...
function EndGame( PlayerReplicationInfo Winner, string Reason )
{
	...
	// FIRAXIS change: bumped the timer to allow a little more time for replication. -tsmith 
	SetTimer( 3.0,false,nameof(PerformEndGameHandling) );
	...
	function StartArbitrationRegistration()
	{
		...
		if (!bHasArbitratedHandshakeBegun)
		{
			...
			foreach WorldInfo.AllControllers(class'PlayerController', PC)
			{
				...
				if(PC.bDeleteMe || PC.PendingSwapConnection != none)
				{
					`log(self $ "::GameInfo::" $ GetFuncName() @ "Skipping arbitration for" @ `ShowVar(PC) 
						@ `ShowVar(PC.bDeleteMe) 
						@ `ShowVar(PC.PendingSwapConnection) 
						@ `ShowVar(PC.IsLocalPlayerController()), true, 'DevOnline');
					continue;
				}
				// FIRAXIS end: dont make client calls on PlayerControllers that are about to be destroyed. This will cause the wrong controllers and bad logic on clients. -tsmith
