...
// FIRAXIS BEGIN
/**
 * Be careful when using this. Force the LE to not reset it's self when something is attached to it
 */
...
// FIRAXIS BEGIN
/** If TRUE, then the constructed shadow frustum uses the subject matrix with a far distance biased by fBiasedSubjectFarDistance additional units (for modulated shadows).
 *  Additionally it forces the modulated shadows to always render, even if shadowed by a dominant light. */
var bool bUseBiasedSubjectMatrix;
/** Additional distance padded onto the far distance of the subject matrix if using bUseBiasedSubjectMatrix (for modulated shadows). */
...
cpptext
{
	...
	/** Reset the light environment */
	virtual void ResetEnvironment() {} // FIRAXIS ADDITION
...
defaultproperties
{
	...
	bDoNotResetOnAttachingTo=False
}
