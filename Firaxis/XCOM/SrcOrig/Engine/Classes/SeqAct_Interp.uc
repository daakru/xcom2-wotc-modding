...
/// FIRAXIS_BEGIN
enum MatineeSelectionPriority
{
	MatineeSelectionPriority_High,
	MatineeSelectionPriority_Med,
	MatineeSelectionPriority_Low,
	MatineeSelectionPriority_VeryLow,
};
/// FIRAXIS_END
...
{
	var vector	Location;
	var vector  	Direction; // FIRAXIS Add -sboeckmann
	var float	Timestamp;
};
...
//FIRAXIS BEGIN
/** For multimatinee selection algorithms, what is the priority of this matinee? */
var(XCom) MatineeSelectionPriority SelectionPriority;

/** For mod layering (playing two matinees at the same time so we can add tracks for new character types), 
 *  what matinee is this layering on top of? */
var(XCom) string BaseMatineeComment;

var(XCom) bool EnableDitherEffectInMatinee;
//FIRAXIS END
...
/** Native script wrapper so these functions can be slightly modified and used for fully script-based (i.e. non-kismet) matinee sequences */
native function NativeUpdateConnectorsFromData();
native function NativeInitInterp();
native function NativeActivated(int InputIdx);
native function NativeUpdateOp(FLOAT deltaTime);
native function NativeDeActivated();

static native function bool NativeIsAnyMatineePlaying();
//FIRAXIS END
...
///FIRAXIS_BEGIN
	SelectionPriority=MatineeSelectionPriority_Med
	bRewindOnPlay = true
	EnableDitherEffectInMatinee = false
///FIRAXIS_BEGIN
}
