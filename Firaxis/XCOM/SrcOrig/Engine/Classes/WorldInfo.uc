...
// FIRAXIS addition begin
struct native PIEInfo  
{
	/** Whether to create soldier spawn points or not */
	var() bool m_bSpawnSoldiers;
	/** Whether to create alien spawn points or not */
...
//You must call UpdateGlobalXComParameters() to apply any changes you make to the values.
struct native XComMaterialParametersInfo
{
	/** Normalized vector pointing towards the sun */
	var() LinearColor EnvironmentDirection;
	/** Dominant Direction Light Color and Intensity in linear Space */
	var() LinearColor EnvironmentColor;
	/** Controls Weather Material Settings (R,G,B,A) => (Specular Intensity,Empty,Empty,Empty) */
	var() LinearColor WeatherParameters;
	/** Controls building visibility parameters (cursor position, angle axes) **/
	var() LinearColor BuildingVisParameters;
	var() LinearColor BuildingVisParameters2;
	/** RGB is environment color used in Light Mass. A is emissive Fade  */
	var() LinearColor EnvironmentColorAndEmissiveFade;
	/** Test Vector can be used for testing. Don't use this for anything permanent. **/
	...
	structdefaultproperties
	{
		EnvironmentDirection=(R=0,G=0,B=1,A=1)
		EnvironmentColor=(R=0.5,G=0.5,B=0.5,A=1)
		WeatherParameters=(R=1,G=1,B=1,A=1)
		BuildingVisParameters=(R=99999999999,G=99999999999,B=99999999999,A=0)
		bRaining=false
	}
};
var(XCom) XComMaterialParametersInfo XComMaterialParameters;

struct native XComDefaultWindInfo
{
...
var(XCom) XComDefaultWindInfo XComDefaultWind;

//Representation of a tile location in script code
struct native TTile
{
	var int X;
	var int Y;
	...
	{
		FTTile() : X(-1), Y(-1), Z(-1) {}
		FTTile(INT SetX, INT SetY, INT SetZ) : X(SetX), Y(SetY), Z(SetZ) {}
		FTTile(EEventParm) : X(-1), Y(-1), Z(-1) {}

		FORCEINLINE UBOOL operator==(const FTTile &Other) const
		{
			return X == Other.X && 
				Y == Other.Y &&
				Z == Other.Z;		
		...
		FORCEINLINE FTTile operator+(const FTTile& Other) const
		{
			return FTTile(X + Other.X, Y + Other.Y, Z + Other.Z);
		}
		...
		void FromFVector(const FVector& Other)
		{
			X = appRound(Other.X);
			Y = appRound(Other.Y);
			Z = appRound(Other.Z);
		...
		/** Serializer. */
		friend FArchive& operator<<(FArchive& Ar, FTTile& Object)
		{
			Ar << Object.X;
			Ar << Object.Y;
			Ar << Object.Z;
		...
		FORCEINLINE FLOAT SizeSq() const
		{
			return X*X + Y*Y + Z*Z;
		}
		FORCEINLINE FLOAT SizeSq2D() const 
		{
			return X*X + Y*Y;
		}
	}
...
var private transient LinearColor     CutoutParameter1;
var private transient Vector         XComCursorPosition;
enum ETOD
{
	eTOD_None<DisplayName=NONE>,
    eTOD_Dawn<DisplayName=Dawn>,
	eTOD_Sunrise<DisplayName=Sunrise>,
    eTOD_Noon<DisplayName=Noon>,
    eTOD_Afternoon<DisplayName=Afternoon>,
    eTOD_Sunset<DisplayName=Sunset>,
	eTOD_Dusk<DisplayName=Dusk>,
	eTOD_Twilight<DisplayName=Twilight>,
	eTOD_Night<DisplayName=Night>
};

var(XCom) ETOD                        m_eTimeOfDay<DisplayName=Time Of Day>;
var(XCom) DominantDirectionalLight  m_kDominantDirectionalLight<DisplayName=DominantDirectionalLight>;

var(XCom) bool bHasNoPathing;
var(XCom) bool bShowLevelBorder;
/** An offset applied to newly created XComCameras, allowing map specific 'default' orientation of the camera */
var(XCom) float CameraYawOffset;

//var(XCom)
// FIRAXIS addition end

// Firaxis BEGIN
// RAM - adding various managers for our stuff
...
var array<Actor>        RemoteEventListeners; //A list of actors that want to find out when remote events are fired
// Firaxis END

/** PostProcessObject used to grab settings from                                                    */
var(Rendering)                  PostProcessSettingsObject   DefaultPostProcessObject<editcondition = bUseWorldAmbientVars>;

/** Default post process settings used by post processing volumes.									*/
var transient      				PostProcessSettings         DefaultPostProcessSettings;

/** Specific Fog of War color for this particular map -intended use is for environment lighting maps **/
var(Rendering) bool bOverrideXComFOWColor;
var(Rendering) Color XComFogOfWarColor;

var(Rendering) bool bOverrideXComHaveSeenTint;
var(Rendering) Color HaveSeenTintColor;

/** Boost to Translucency lighting */
var(Rendering) float TranslucencyLightingBoost<editcondition = bUseWorldAmbientVars>;
...
var(WorldInfo) bool bRecacheBuildingActorsAtRunTime; // FIRAXIS
...
var						bool					bUseWorldAmbientVars;
var()                   TextureCube             WorldAmbientCubemap<editcondition=bUseWorldAmbientVars>;
var()                   float                   WorldAmbientIntensity<editcondition=bUseWorldAmbientVars>;
...
var const globalconfig  float                   FOWUpdateTimeLimit;     // default time that FOW will take to update - set in defaultgame.ini
...
/** Particle Point Light Component pool for lights spawned from particles */
var globalconfig string EmitterLightPoolClassPath;
var transient EmitterLightPool MyEmitterLightPool;
...
/** Debris Manager */
var globalconfig string DebrisManagerClassPath;
var transient DebrisManager MyDebrisManager;

/** Manager used to handle clipping light to tile boundaries (for interior/exterior wall boundaries). */
var transient XComLightClippingManager MyLightClippingManager;

/** Manager used to handle Local Environment Ambient maps and their generation. */
var transient X2LocalEnvMapManager MyLocalEnvMapManager;

/** Manager used to handle reflections for translucent objects. */
var transient X2TranslucentReflectionManager MyTranslucentReflectionManager;

var globalconfig string TileDestructionClassPath;
...
/** Should we fade out lights at a distance? Just used in Avenger currently.*/
var(Rendering) globalconfig bool		bUseLightingLOD;
...
/** Larger values increase the contrast of the indirect lighting calculated by Lightmass that is used by light environments. */
var (Rendering)                 float   LightEnvironmentIndirectScaleFactor; // FIRAXIS ADDITION

/** Larger values increase the contrast of the indirect lighting calculated by Lightmass that is used by light environments. */
var (Rendering)                 float   LightEnvironmentAmbientScaleFactor; // FIRAXIS ADDITION

/** Z level to create the cascades from */
var(Rendering)				float CascadeShadowZLevel;
var transient float WorldCascadeZ;
...
// FIRAXIS BEGIN
var(Lightmass) bool bHasNoPrecomputedLighting;
// FIRAXIS END
...
struct native LightmassWorldInfoSettings
{
	...
	/** A scale for dynamic object based level scale. Should normally be used to counteract the 
	 *	StaticLightingLevelScale;
	 */
    var(General)    float       StaticLightingDynamicObjectScale; // FIRAXIS
	...
	structdefaultproperties
	{
        ...
        StaticLightingDynamicObjectScale=1 // FIRAXIS
...
var transient init array<PathBlockingVolume> BlockingVolumeCache;
// FIRAXIS END

var transient bool bContinueToSeamlessTravelDestination;

var transient int DecalSpawnAccumulator; // Used to determine when to spawn a decal or not based off of the number of decals spawned from Impact Actors
...
cpptext
{
	...
	/** Checks if we should fade out lights based on distance. */
	UBOOL GetUseLightingLOD() const;

	FLinearColor GetEnvironmentColor() const;

	float GetIndirectLightingScaleFactor(); // FIRAXIS ADDITION
	float GetAmbientScaleFactor();          // FIRAXIS ADDITION 
	float GetCascadeShadowZLevel();			// FIRAXIS ADDITION
...
// FIRAXIS BEGIN jboswell
native final function CacheBlockingVolumes();
// FIRAXIS END
 ...
 * NOTE: this only works 100% on PC. XBL/GFWL uses secure sockets. PS3 will only work if nat traversal is disabled.
 * 
 * @param bAppendPort - whether or not to append the port # to the IP address
 * 
 * @return the ip address of the the machine
 */
native simulated function string GetLocalNetworkAddress(optional bool bAppendPort);
// FIRAXIS end -tsmith
...
simulated function PreBeginPlay()
{
	...
	local class<EmitterLightPool> EmitterLightPoolClass;
	local class<DebrisManager> DebrisManagerClass;
	local DominantDirectionalLight kDominantDirectionalLight; // FIRAXIS ADDITION
	...
	if (WorldInfo.NetMode != NM_DedicatedServer && IsInPersistentLevel())
	{
		...
		if (EmitterLightPoolClassPath != "")
		{
			EmitterLightPoolClass = class<EmitterLightPool>(DynamicLoadObject(EmitterLightPoolClassPath, class'Class'));
			if( EmitterLightPoolClass != none )
			{
				MyEmitterLightPool = Spawn(EmitterLightPoolClass, self,, vect(0,0,0), rot(0,0,0));
			}
		}
		...
		if (DebrisManagerClassPath != "")
		{
			DebrisManagerClass = class<DebrisManager>(DynamicLoadObject(DebrisManagerClassPath, class'Class'));
			if( DebrisManagerClass != None )
			{
				MyDebrisManager = Spawn(DebrisManagerClass, self,, vect(0,0,0), rot(0,0,0));
			}
		}

		// Firaxis BEGIN 
		// RAM - various mgr support
		MyWatchVariableMgr = Spawn( class'WatchVariableMgr', self );	
		MyKismetVariableMgr = Spawn( class'KismetVariableMgr', self );
		MyDestructibleDebrisMgr = Spawn( class'XComDestructibleDebrisManager', self );
		MyLocalEnvMapManager = Spawn( class'X2LocalEnvMapManager', self );
		MyTranslucentReflectionManager = Spawn( class 'X2TranslucentReflectionManager', self );
		SpawnLightClippingManager();
		// Firaxis END
	}

	// FIRAXIS BEGIN
	// JMS - TimeOfDay replacement : Find Dominant Directional Light and set it
	foreach AllActors( class 'DominantDirectionalLight', kDominantDirectionalLight )
	{
		WorldInfo.m_kDominantDirectionalLight = kDominantDirectionalLight;
		break;
	}

	// jboswell: save a list of blocking volumes for the camera code
	CacheBlockingVolumes();
	// FIRAXIS END
}

// FIRAXIS BEGIN
native function SetCutoutParameter1 (const out LinearColor cParam);
native function GetCutoutParameter1 (out LinearColor cParam);
native function SetXComCursorPosition( const out Vector kCursorPos );
native function GetXComCursorPosition(out Vector kCursorPos );
native function SetXComUnitPosition(const out Vector kUnitPos);
// FIRAXIS END
...
// FIRAXIS Begin MarcG
native function UpdateGlobalXComParameters();
// FIRAXIS End
/**
...
native function ClearAmbientCubemap();

simulated event SpawnLightClippingManager()
{
	MyLightClippingManager = Spawn(class'XComLightClippingManager',self);
}

native function SetupNonTacticalWorldVolumeInfo(Volume LevelVolume);
...
defaultproperties
{
	...
	CharacterLitIndirectBrightness=5 // FIRAXIS CHANGE
	CharacterLitIndirectContrastFactor=1 // FIRAXIS CHANGE
	CharacterShadowedIndirectBrightness=5
	CharacterShadowedIndirectContrastFactor=1
	CharacterLightingContrastFactor=1.5
	LightEnvironmentIndirectScaleFactor=0.2 // FIRAXIS ADDITION
	LightEnvironmentAmbientScaleFactor=1.8 // FIRAXIS ADDITION
	CascadeShadowZLevel=0.0
	WorldCascadeZ=-1.0
	...
	// FIRAXIS BEGIN
	CutoutParameter1=(R=0,G=0,B=0,A=0)

	UseDownsampledOutlineDepth=false;
	// FIRAXIS END
	...
	bContinueToSeamlessTravelDestination = true; // FIRAXIS jboswell

	bShowLevelBorder=true; // FIRAXIS - Scott Boeckmann
	bForceNoPrecomputedLighting=true;

	bUseWorldAmbientVars=false
	WorldAmbientIntensity=1.0

	XComDefaultWind=(Direction=(X=.688,y=.688,z=.229), Speed=0, Strength=0)
