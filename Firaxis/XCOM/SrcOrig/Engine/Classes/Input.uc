...
struct native KeyBind
{
	...
	var config init string	Command;
	...
	/** FXS: Config bools used in UIKeybindingsPCScreen */ 
	var config bool     bPrimaryBinding, bSecondaryBinding;
	...
	structcpptext
	{
		...
		, bPrimaryBinding(FALSE), bSecondaryBinding(FALSE)
		{}
	}
};
 ...
 * FXS BEGIN (sbatista)
 * 
 *  The following functions exist to simplify keyboard bindings.
 */
...
function RemoveBind(const name BindName, const string BindCommand, const bool bSecondaryBind)
{
	...
	for(BindIndex = Bindings.Length - 1; BindIndex >= 0; BindIndex--)
	{
		if(Bindings[BindIndex].Command == BindCommand &&
		   Bindings[BindIndex].bSecondaryBinding == bSecondaryBind &&
		   Bindings[BindIndex].bPrimaryBinding == !bSecondaryBind)
		{
