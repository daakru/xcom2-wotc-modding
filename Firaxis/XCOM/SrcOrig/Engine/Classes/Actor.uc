...
// FIRAXIS CHANGE HERE
var(Movement) const databinding vector			Location;

/** The actor's rotation; use SetRotation to change. */
// FIRAXIS CHANGE HERE
var(Movement) const databinding rotator			Rotation;
...
// Firaxis BEGIN
// RAM - Adding to facilitate per-actor tick stats in the stats viewer
var private transient int TickStatsIndex;

/** A translation offset applied to the actor when SetLocation is used. */
var const Vector WorldSpaceOffset;
// Firaxis END
...
var(Movement) const enum EPhysics
{
	...
	// FIRAXIS addition: physics mode for our RMA animation interactions -tsmith
	PHYS_RMAInteract,
} Physics;
...
// FIRAXIS CHANGE HERE
/** Team this actor is on */ 
var const repnotify ETeam m_eTeam;

/** which team this actor is visible to */
var const repnotify ETeam m_eTeamVisibilityFlags;

/** whether or not this actor is visible with respect to the local player */
var protected bool m_bVisible;  

/** whether or not this actor is in FOW with respect to the local player */
var bool m_bInFOW;

/** whether or not to replicate the bHidden variable */
var bool m_bReplicateHidden;

/** forces replication of the m_eTeam variable. necessary because some Actor's m_eTeam variable changes after initial replication. */
var const bool          m_bReplicateDirtyTeamType;
var privatewrite bool   m_bReplicatingDirtyTeamType;

/** == true, perform physics for actor's with Role == ROLE_AutonomousProxy. this is to allow our Cursor Pawn to be authoritative over its movement on the client. */
var bool m_bPerformPhysicsForRoleAutonomous;

/** == true, call EndState event in GWorld::DestroyActor. */
var bool m_bDoEndStateOnDestroyActor;
// FIRAXIS end -tsmith
...
var const public{private} bool bStaticCollision;
// FIRAXIS END
...
var                 bool    bForceNoTick; // FIRAXIS add - easier than setting the correct flags

// FIRAXIS CHANGE - exposed this so we can disable and set XComLevelActor collision to COLLIDE_TOUCHALL.
...
var			  const	bool	bOnlyRelevantToFriendlies;  // this actor is only relevant to players where (PlayerController.m_eTeam & self.m_eTeam) > 0 -tsmith 
...
// FIRAXIS begin -tsmith
/**
 * If true this actor will not get deleted when the level initializes its actors.
 * Normally actors that are deemed uninteresting to client are deleted.
 * This is governed in stock UE3 by the bScriptInitialized, bNoDelete, and bStatic flags.
 * However, we need finer grained control in certain situations (dynamic level loading 
 * with static mesh actors, clientside Kynapse, etc.) and reliance on the stock flags is insufficient.
 * 
 * NOTE: trying to spawn an actor that dynamically replicates ((bNoDelete || bStatic == TRUE) && (RemoteRole == ROLE_AutonomousProxy || ROLE_SimulatedProxy
 * will fail as this would cause duplicates on the client if the actor had also been placed in the level.
 * 
 * @see ULevel::InitializeActors
 */
var           const bool    m_bNoDeleteOnClientInitializeActors;  
// FIRAXIS end -tsmith
...
// Firaxis begin
//  --Ned  - changed to avoid millions of temp allocations
	var	native			Array_Mirror	ChannelWeights{TPreallocatedArray< FLOAT, 4>};
//Firaxis end
};
...
// This damn thing is used everywhere, there was no good place to put it, as it's used in both UI and AI
struct native TRect
{
	var float fLeft;
	var float fTop;
...
struct native ActorIdentifier
{
	...
	structcpptext
	{
	FActorIdentifier() : ActorName(NAME_None), OuterName(NAME_None), Location(FVector(0,0,0)) {}
	FActorIdentifier(EEventParm) : ActorName(NAME_None), OuterName(NAME_None), Location(FVector(0,0,0)) {}

	UBOOL IsValid() const { return ActorName != NAME_None; }

	UBOOL operator==(const FActorIdentifier &rhs) const
	{
		return (ActorName == rhs.ActorName) &&
			(OuterName.GetIndex() == rhs.OuterName.GetIndex()) &&
			(Location == rhs.Location);
	}
	}
};

// Make sure this doesn't exceed a byte
// Also, if you add more, make sure you use only powers of 2, it's used as a bitfield elsewhere
enum EEnvLightClassification
{
	eEnvLight_None,
	eEnvLight_Interior,
	eEnvLight_Exterior
...
enum EEnvClassification
{
	eEnvClass_DayClear,
	eEnvClass_DayRain,
	eEnvClass_DaySnow,
	eEnvClass_NightClear,
	eEnvClass_NightRain,
	eEnvClass_NightSnow
};

struct native EnvironmentToggleInfo
{
	var() array<EEnvClassification> EnabledEnvironmentConditions<Tooltip = Array of environment conditions in which this Actor should be enabled.An empty array indicates the actor should always be enabled.>;

	structcpptext
	{
		UBOOL Matches(EEnvClassification EnvClass)
		{
			if( EnabledEnvironmentConditions.Num() == 0 )
				return TRUE;

			for( INT i = 0; i < EnabledEnvironmentConditions.Num(); i++ )
			{
				if( EnabledEnvironmentConditions(i) == EnvClass )
					return TRUE;
			}
...
var transient bool bUsePersistentSoundAkObject; //Firaxis - designates this object to use the PERSISTENT_SOUNDTRACK_GAMEOBJ when posting Wise events
...
var const float				LatentCrossFadeTime; // Jwats: FinishAnim will end this amount of time before the end of the animation
...
// Tag array for world hash checks. See FOctreeContext for more info.
var	native transient int CheckTag[8];

// this should never be used, it exists only to provide compatibility with RTM cooked data
...
// Collision flags.
var				bool				bDoNotTriggerSeqEventTouch;
...
var bool bDecalCollisionOverride;
...
// FIRAXIS begin: variables used only for debugging -tsmith
/** used to set a conditional breakpoint or excute conditional code if true */
var bool bDebugUseToSetBreakpoint;
// FIRAXIS end -tsmith

// FIRAXIS begin
var() bool bActorDisabled; // A way for us to set whether an actor should be treated as enabled or disable for any reason (Path generation and chaos layers)

// Runtime flag to cache whether or not this actor lives in the "cinematic layer", which 
// contains actors that should only appear in cinematic cameras. Used to prevent having to do
// an expensive layer check when entering and leaving cinematic mode
var transient bool bHideInNonCinematicViews; 
var transient bool bCinematicLevelActor; //Set TRUE by XCOM level loading logic for actors contained within a cinematic map. Used by Matinee to turn actor ticking on / off based on whether or not
										 //the actor is part of a running matinée sequence. This is necessary because cinematic maps are loaded and ready to use all the time rather than loaded and 
										 //run all the time.
var transient float LOD_TickRate; //Allow the game to control the tick rate of distant / non-essential actors
// FIRAXIS end
//-----------------------------------------------------------------------------
// cpptext.
...
cpptext
{
	...
	// Utility functions for Octree searches. Allows us to only check any given actor 
	// once
	UBOOL AlreadySeen(struct FOctreeContext* Context);
	void MarkSeen(struct FOctreeContext* Context);
	...
	virtual void PreEditMove(); // FIRAXIS jboswell: called before a move, to notify an object that it is about to move
	...
	void SetCollisionFromCollisionType(UBOOL bReattachComponents=TRUE);
...
// FIRAXIS BEGIN jboswell
	// Whether this object is static for collision purposes
	FORCEINLINE UBOOL IsStaticForCollision() { return bStaticCollision; }
// FIRAXIS END
...
// WWISEMODIF_START
	UINT PlayAkSound(const FString& AkEventName, UBOOL bNotReplicated = FALSE, UBOOL bNoRepToOwner = FALSE, UBOOL bStopWhenOwnerDestroyed = FALSE, FVector* SoundLocation = NULL, UBOOL bNoRepToRelevant = FALSE);
	void StopAkSound(UINT AkSoundID);
...
public:
	...
	virtual UBOOL ActorPointCheck(FCheckResult& Result,const FVector& Location,const FVector& Extent,DWORD TraceFlags);

	// Natives.
	DECLARE_FUNCTION(execPollSleep);
	DECLARE_FUNCTION(execPollFinishAnim);
	DECLARE_FUNCTION(execPollWaitForAnimToBeRelevant);
	DECLARE_FUNCTION(execPollWaitForAnimToNotBeRelevant);
	...
	virtual void GetAllPrimitiveComponents(TArray<UPrimitiveComponent*>& OutComponents);
}
...
replication
{
	...
	// FIRAXIS addition: check m_bReplicateHidden -tsmith
	if ( m_bReplicateHidden && (!bSkipActorPropertyReplication || bNetInitial) && (Role==ROLE_Authority) && bNetDirty )
		bHidden;
	...
	// FIRAXIS addition -tsmith
	if( (!bSkipActorPropertyReplication || bNetInitial) && bNetDirty && (Role==ROLE_Authority) )
		m_eTeamVisibilityFlags;

	if( (!bSkipActorPropertyReplication || bNetInitial || m_bReplicateDirtyTeamType) && bNetDirty && (Role==ROLE_Authority) )
		m_eTeam;
	// FIRAXIS end -tsmith
}
...
native(261) final latent function FinishAnim( AnimNodeSequence SeqNode, optional bool bReplayAnim=false, optional float CrossFadeTime = 0.0f );
native final latent function WaitForAnimToBeRelevant( AnimNodeSequence SeqNode );
native final latent function WaitForAnimToNotBeRelevant( AnimNodeSequence SeqNode );
 ...
 * Only has an effect when called on bStatic or bNoDelete or m_bNoDeleteOnClientInitializeActors Actors
 ...
 */
simulated native function SetHidden(bool bNewHidden);// FIRAXIS addition: change to simulated  -tsmith
...
simulated native function bool IsVisibleToTeam(ETeam eCheckTeam);

simulated native function bool IsVisible();

native function SetTeamType(ETeam eNewTeam);
simulated native function bool IsFriendly(Actor kCheckActor);
simulated native function bool IsTeamFriendly(ETeam eCheckTeam);
// FIRAXIS end -tsmith
...
(
	...
	optional ETeam    SpawnTeam                 // FIRAXIS addition -tsmith
);
...
// FIRAXIS begin -tsmith
/**
 * Tells the server to delete the actor.
 * This will close network connection and free up the actor on both server and client.
 * 
 * NOTE: this should reference count how many clients are still using this actor before destroying but since our game is 1v1
 * the notification from the 1 client controller is sufficient.
 */ 
simulated function ServerDestroy()
{
	GetALocalPlayerController().ServerDestroyActor(self);
}
...
/**
 * Adds the actor to the world's list of actors that need to have special cleanup code called when the world is being cleaned up
 */
...
/**
 * Removes the actor from the world's list of actors that need to have special cleanup code called when the world is being cleaned up
 */
 ...
 * @param bNoRepToOwner (opt) - sound is not replicated to the Owner of this Actor (typically for Inventory sounds)
 * @param bStopWhenOwnerDestroyed (opt) - whether the sound should cut out early if the playing Actor is destroyed
 * @param SoundLocation (opt) - alternate location to play the sound instead of this Actor's Location ( NOT SUPPORTED! )
 * @param bNoRepToRelevant (opt) - sound is not replicated to clients for which this Actor is relevant (for important sounds that are locally simulated when possible)
 * @return - AkSoundID ID of the sound being played
 */

native noexport final function int PlayAkSound(string AkEventName, optional bool bNotReplicated, optional bool bNoRepToOwner, optional bool bStopWhenOwnerDestroyed, optional vector SoundLocation, optional bool bNoRepToRelevant);

native noexport final function StopAkSound(int AkSoundID);
...
//Stops all sounds for this game object
native final function StopSounds();
...
// FIRAXIS begin -tsmith
/** get the PlayerController (if any) that owns this Actor. OK to return none */
 ...
 * FIRAXIS ADDITION
 * EP: added 7/18/08
 * Searches for the first actor the specified name.
 */
...
//Firaxis BEGIN
//rmcfall - added these methods to allow references to actors to be stored by name ( ie. effectively a weak reference ). This is 100% safe to do 
//          for static ( editor placed ) actors. Use caution if using this with dynamic (spawned ) actors.

/// <summary>
/// Given an actor identifier struct ( name + outer ), this method will return TRUE if an actor matching that
/// description is found and out_Actor will contain a reference to the matching actor.
///
/// ActorIdentifier ActorId would be const out ( reference ) except that unreal script does not allow dynamic array elements
/// to be passed in as const out.
/// </summary>
final static native function bool FindActorByIdentifier(ActorIdentifier ActorId, out Actor out_Actor);

/// <summary>
/// Returns an ActorIdentifier structure to be used by 'FindActorByIdentifier'
/// </summary>
final native function ActorIdentifier GetActorId();

/// <summary>
/// Returns the outermost object - should usually be a map or package
/// </summary>
final native function Object GetActorOutermost(); 
//Firaxis END
...
// Called after a game has finished loading. Note the following event order:
//   1) LoadMap
//   2) PostBeginPlay() on each actor
//   3) Load Save Game Data
//   4) PostLoadGame() on each actor
...
simulated function DisplayDebug(HUD HUD, out float out_YL, out float out_YPos)
{
	...
	Canvas.DrawText("Game Slomo:"@WorldInfo.Game.GameSpeed);
	out_YPos += out_YL;
	Canvas.SetPos(4, out_YPos);
...
/**
 * HUD will call PostRenderFor() on this actor every frame.
 */
// FIRAXIS change - made simulated, don't know why it wasn't that would disable replicated actors for being PostRenderFor on clients. -tsmith 
simulated function AddHUDOverlayActor()
{
	local PlayerController PC;
...
simulated function RemoveHUDOverlayActor()
{
	local PlayerController PC;
	...
	ForEach LocalPlayerControllers(class'PlayerController', PC)
	{
		if ( PC.myHUD != None )
		{
			...
			if( PC.myHUD.PostRenderedActors.Length == 0 )
			{
				PC.myHUD.bShowOverlays = false;
			}
		}
	}
...
// FIRAXIS BEGIN
// JMS - XCOM visibility-related functions
...
/** Grabs the Cutdown Offset set on the mesh. We should move this to the actor level. */
native simulated function float GetCutdownOffset();

/* Checks bShouldCutout settings of all primitive components */
native simulated function bool CanUseCutout();

/* Hide this actor instead of cutting out. */
...
native simulated function SetPeripheryHideable(bool bShouldHide);

/* Used to indicated the mesh is hiding from an external mesh group and needs to not be cutout by CutoutBoxes and VisbilityTraces */
...
/* Used to indicate this actor should render with the dither hole effect */
...
// cjcone@psyonix: helpful for print debug info for currently spawned actors.
// Invoked by an exec function in the xcom player controll, PDI()
...
simulated function bool SnapToGround( optional float Distance = 1024.0f )
{
	...
	HitActor = Trace( HitLocation, HitNormal, Location + vect(0,0,-1) * Distance, Location, false );

	if (HitActor != none )
	{
		if ( CylinderComponent(CollisionComponent) != none )
		{
			CollisionHeight = CylinderComponent(CollisionComponent).CollisionHeight;
		}
		else
		{
			GetBoundingCylinder( CollisionRadius, CollisionHeight );
			CollisionHeight *= 0.50f;
		}
...
event StopParticleEffect( const AnimNotify_StopParticleEffect AnimNotifyData );		// FIRAXIS addition
/**
...
// JMS - generic function for determining if a position is visible or not due to building visibility, etc.
// Position is the position to test
// bCutout is true if the position is invisible due to cutout
// HitInfo is the TraceHitInfo return from a trace on this actor
...
/**
 * Used for soldier / weapon customization - sbatista
 */ 
simulated function RotateInPlace(int Dir)
{
	local rotator WeaponRot;
	WeaponRot = Rotation;
	WeaponRot.Yaw += 45.0f * class'Object'.const.DegToUnrRot * 0.33f * Dir;
	SetRotation(WeaponRot);
}
...
native function ScriptAddRBGravAndDamping();

native function SetWorldSpaceOffset(const out Vector NewWorldSpaceOffset);
// Multi-Tile units are handles in SetLocation by applying the WorldSpaceoffset.  
// Actor.SetLocation(Actor.Location) is not safe for Multi-Tile units (changes their location).
// Actor.SetLocationNoOffset(Actor.Location) is safe to do and will not move the unit.
native function bool SetLocationNoOffset(vector NewLocation);
//  FIRAXIS END
...
defaultproperties
{
	...
	SupportedEvents(1) = class'SeqEvent_HitWall'
	SupportedEvents(2) = class'SeqEvent_AnimNotify'
	...
	// FIRAXIS begin -tsmith
	m_eTeam = eTeam_None
	m_bReplicateDirtyTeamType = FALSE
	m_bNoDeleteOnClientInitializeActors = FALSE
	m_bReplicateHidden = TRUE
	m_bVisible = TRUE
	m_bPerformPhysicsForRoleAutonomous = FALSE
	m_bDoEndStateOnDestroyActor = TRUE
	// FIRAXIS end -tsmith
	...
	// Firaxis Addition
	bDirtyComponents = true

	bHideInNonCinematicViews = false
	bDoNotTriggerSeqEventTouch = false
	bDecalCollisionOverride = false
