...
{
	if (self.IsA(classname))
		return self;

	if (self.IsA('AnimNodeBlendList'))
		return none;

	if (Children[0].Anim != none)
	{
		if (Children[0].Anim.IsA('AnimNodeBlendList'))
		{
			return AnimNodeBlendList(Children[0].Anim).GetFirstActiveNode(classname);
		}
		if (Children[0].Anim.IsA('AnimNodeBlendBase'))
		{
			return AnimNodeBlendBase(Children[0].Anim).GetFirstActiveNode(classname);
		}
...
cpptext
{
	 ...
	 */
	virtual INT BlendCurveWeights(const FArrayCurveKeyArray& InChildrenCurveKeys, const TArray<BYTE>& IndexMapping, FCurveKeyArray& OutCurveKeys);
...
// FIRAXIS BEGIN - MHU - Helper to ensure features are utilized only when assets are ready
function bool ValidateChildrenAsAnimNodeSequences()
{
	local int i;
	local AnimBlendChild kNode;

	for (i = 0; i < Children.Length; i++)
	{
		kNode = Children[i];
		if (kNode.Anim == none ||
			AnimNodeSequence(kNode.Anim) == none ||
			AnimNodeSequence(kNode.Anim).AnimSeq == none)
			return false;
	}
