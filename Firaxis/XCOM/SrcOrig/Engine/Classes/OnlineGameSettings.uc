...
// FIRAXIS begin: Bubbling up implementation from the XComOnlineGameSettings to create an interface used by OnlineSubSystems -ttalley
struct native WriteClassType
{
	var string WriteType;
	var class<OnlineStatsWrite> WriteClass;
};
var array<WriteClassType> OnlineStatsWriteClasses;
var class<OnlineStatsWrite> OnlineStatsWriteClass; // Default write class

struct native ReadClassType
{
	var string ReadType;
	var class<OnlineStatsRead> ReadClass;
};
var array<ReadClassType> OnlineStatsReadClasses;
var class<OnlineStatsRead> OnlineStatsReadClass;  // Default read class

function class<OnlineStatsWrite> GetOnlineStatsWriteClass(string WriteType)
{
	local int Idx;
	for( Idx = 0; Idx < OnlineStatsWriteClasses.Length; ++Idx)
	{
		if( OnlineStatsWriteClasses[Idx].WriteType == WriteType )
		{
			return OnlineStatsWriteClasses[Idx].WriteClass;
		}
	}
	return OnlineStatsWriteClass;
}

function class<OnlineStatsRead> GetOnlineStatsReadClass(string ReadType)
{
	local int Idx;
	for( Idx = 0; Idx < OnlineStatsReadClasses.Length; ++Idx)
	{
		if( OnlineStatsReadClasses[Idx].ReadType == ReadType )
		{
			return OnlineStatsReadClasses[Idx].ReadClass;
		}
	}
	return OnlineStatsReadClass;
}
// FIRAXIS end: -ttalley
...
defaultproperties
{
	...
	OnlineStatsWriteClass=class'Engine.OnlineStatsWrite'
	OnlineStatsReadClass=class'Engine.OnlineStatsRead'
}