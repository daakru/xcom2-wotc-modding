...
/** Set the lightenvironment on this actor to all attached KActors */
var bool bPropagateDLEToAttachedInterpActors; // FIRAXIS
var() bool bUseDLEInHQ; // FIRAXIS
...
simulated event SetAnimPosition(name SlotName, int ChannelIndex, name InAnimSeqName, float InPosition, bool bFireNotifies, bool bLooping, bool bEnableRootMotion)
{
	...
	// BEGIN FIRAXIS
	if( SkeletalMeshComponent.bAllowSetAnimPositionWhenNotRendered )
	{
	...
	else if( !SkeletalMeshComponent.bRecentlyRendered )
	{
		return;
	}
	// END FIRAXIS
...
{
	SkeletalMeshComponent.UnMountCinematicFaceFX();
}
...
{
	// BEGIN - FIRAXIS CHANGE - Ryan Baker
	if (bPreviewOnly && !WorldInfo.IsPlayInEditor())
	{
		return none;
	}
	// END - FIRAXIS CHANGE - Ryan Baker
...
event bool PlayParticleEffect( const AnimNotify_PlayParticleEffect AnimNotifyData )
{
	...
	if( AnimNotifyData.bAttach == TRUE )
	{
		...
		{
			foreach SkeletalMeshComponent.AttachedComponentsOnBone( class'ParticleSystemComponent', PSC, AnimNotifyData.SocketName, AnimNotifyData.BoneName )
			{
		...
		AttachBoneIndex = SkeletalMeshComponent.MatchRefBone(AnimNotifyData.BoneName);
		AttachSocket = SkeletalMeshComponent.GetSocketByName(AnimNotifyData.SocketName);
...
defaultproperties
{
	...
	//	bEnabled=false
	//	bCastShadows=false // FIRAXIS ADDITION
