...
enum EProgressMessageType
{
	...
	/** Indicates that the connection to the server is experiencing problems, and may eventually timeout */
	PMT_ConnectionProblem,

	/** Indicates that a peer connection to another peer is experiencing problems, and may eventually timeout */
	PMT_PeerConnectionProblem,
};

/** Used to make sure the client is kept synchronized when in a spectator state */
var float LastSpectatorStateSynchTime;

// FIRAXIS addition: from Gears2 code -tsmith
...
// WWISEMODIF_END
	 ...
	 */
	virtual void SetControllingDirector( UInterpTrackInstDirector* NewControllingDirector );
	...
	virtual UBOOL IsInCinematicMode() {return bCinematicMode;}
}
...
native final function OnCameraFadeSet(bool bEnableFading, optional color FadeColor, optional vector2d FadeAlpha, optional float FadeTime, optional bool bFadeAudio, optional bool bDontResetFade);

reliable client event ClientSetCameraFade(bool bEnableFading, optional color FadeColor, optional vector2d FadeAlpha, optional float FadeTime, optional bool bFadeAudio, optional bool bDontResetFade)
{
	if (PlayerCamera != None)
	{
		`log("Setting Camera Fade",,'XCom_Content');
		`log(GetScriptTrace(),,'XCom_Content');
		...
		if (PlayerCamera.bEnableFading)
		{
			...
			if (!bDontResetFade || PlayerCamera.FadeTime > FadeTime || PlayerCamera.FadeTimeRemaining == 0)
			{
				PlayerCamera.FadeTime = FadeTime;
				PlayerCamera.FadeTimeRemaining = FadeTime;
			}
			if (FadeTime == 0.0f) // FIRAXIS BEGIN jboswell: Insta-fade
			{
		...
		OnCameraFadeSet(bEnableFading, FadeColor, FadeAlpha, FadeTime, bFadeAudio, bDontResetFade);
	}
}
...
event NotifyPeerDisconnectHost(UniqueNetId PeerNetId)
{
	...
	`Log(`location @ ": client peer lost connection to host" $" PeerNetId="$class'OnlineSubsystem'.static.UniqueNetIdToString(PeerNetId),,'DevNet');
...
`endif
	...
	{
		`Log(`location @ "migrating player as host" $" NetId="$class'OnlineSubsystem'.static.UniqueNetIdToString(PlayerReplicationInfo.UniqueId),,'DevNet');
...
function OnHostMigratedOnlineGame(name SessionName,bool bWasSuccessful)
{
	...
	{
		`Log(`location @ "migration failed for" $" NetId="$class'OnlineSubsystem'.static.UniqueNetIdToString(PlayerReplicationInfo.UniqueId),,'DevNet');
...
event PeerReceivedMigratedSession(UniqueNetId FromPeerNetId, name SessionName, class<OnlineGameSearch> SearchClass, byte PlatformSpecificInfo[80])
{
	...
	if (LP != None &&
		...
		OnlineSub.GameInterface != None)
	{
		`Log(`location @ "received migrated session to join" $" SessionName="$SessionName $" SearchClass="$SearchClass $" UniqueId="$OnlineSub.static.UniqueNetIdToString(PlayerReplicationInfo.UniqueId) $" FromPeerNetId="$OnlineSub.static.UniqueNetIdToString(FromPeerNetId) ,,'DevNet');
...
function PeerDesignatedAsClient(name SessionName)
{
	...
	{
		`Log(`location @ "traveling to joined,migrated session " $" SessionName="$SessionName $" URL="$URL,,'DevNet');
...
function OnJoinMigratedGame(name SessionName,bool bWasSuccessful)
{
	...
	if (!bWasSuccessful)
	{
		`Log(`location @ "failed joining migrated session "$" SessionName="$SessionName,,'DevNet');
...
// FIRAXIS addition: bUseShortConnectTimeout
reliable server function ServerShortTimeout(optional bool bUseShortConnectTimeout=true)
{
	local Actor A;

	// FIRAXIS addition: bUseShortConnectTimeout
	if (!bShortConnectTimeout || !bUseShortConnectTimeout)
	{
		// FIRAXIS addition: bUseShortConnectTimeout
		bShortConnectTimeOut = bUseShortConnectTimeout;
		ResetTimeMargin();
...
// NOTE: if this changes from an integration we may need to add changes to our XComPlayerController -tsmith 
/**
...
event InitInputSystem()
{
	...
	// dburchanowski - this should be coming from the ini's, but I can't figure out why they aren't loading the correct values
	// and I can't spend any more time on this with the amount of other things to do. Will fix correctly later as time permits.
	if(class'WorldInfo'.static.IsConsoleBuild(CONSOLE_PS3))
	{
		ForceFeedbackManagerClassName = "PS3Drv.PS3ForceFeedbackManager";
	}
...
// hook to allow game specific controllers to respond to AK events playing in matinee
...
//    `log("### ClientHearSound:"@ASound@SourceActor@SourceLocation@bStopWhenOwnerDestroyed@VSize(SourceLocation - Pawn.Location));
	...
	AC.bIsMusic = (ASound.SoundClass == 'Music'); // FIRAIXS - Make sure the music flag is correct
	AC.Play();
}
...
/** Uses Windows SAPI to speak text.*/
simulated private native function SayText_TTS(string SayText, bool bMaleVoice);
...
simulated function SpeakTTS( coerce string S, optional bool bMaleVoice )
{
	/** FIRAXIS RAM - Deprecated, not support on Win64
	local SoundCue Cue;
	local AudioComponent AC;
...
function RegisterOnlineDelegates()
{
	...
	if (OnlineSub != None && LP != None)
	{
		...
		{
			// FIRAXIS: Turning off the PlayerController handling of invites, instead pushing it onto the OnlineEventMgr -ttalley
...
event Destroyed()
{
	...
	// FIRAXIS begin: Cleaning up a hanging delegate -ttalley
...
function ServerMoveHandleClientError(float TimeStamp, vector Accel, vector ClientLoc)
{
	...
	if (ClientErr > MAXPOSITIONERRORSQUARED)
	{
		...
		else
		{
			...
			if( (InterpActor(Pawn.Base) != None) || DynamicBlockingVolume(Pawn.Base) != None )
			{
	...
	while( CurrentMove != None )
	{
		if( CurrentMove.TimeStamp <= CurrentTimeStamp )
		{
			...
			if( CurrentMove.TimeStamp == CurrentTimeStamp )
			{
				...
				FreeMoves.Clear();
				if( ((InterpActor(NewBase) != None) || DynamicBlockingVolume(NewBase) != None )
					&& (NewBase == CurrentMove.EndBase) )
				{
						...
						&& ((MoveActor.Physics == PHYS_Walking) || (MoveActor.Physics == PHYS_Falling)) )
					{
						...
						else if ( (VSizeSq(Velocity) < MAXNEARZEROVELOCITYSQUARED) && (VSizeSq(NewVelocity) < MAXNEARZEROVELOCITYSQUARED)
...
function bool SetPause( bool bPause, optional delegate<CanUnpause> CanUnpauseDelegate=CanUnpause, optional bool bFromLossOfFocus=false )
{
	...
	// FIRAXIS change: game can only be paused in single player. -tsmith 
	//if (WorldInfo.NetMode != NM_Client)
	if (WorldInfo.NetMode == NM_Standalone)
	{
		if (bPause)
...
event NotifyLoadedWorld(name WorldPackageName, bool bFinalDest)
{
	...
	SetViewTarget(self);
	foreach WorldInfo.AllActors(class'PlayerStart', P)
	{
...
simulated event GetPlayerViewPoint( out vector out_Location, out Rotator out_Rotation )
{
	...
	// FIRAXIS change: cameras dont need to be spawned on server for non-locally controlled players. -tsmith 
...
// FIRAXIS change: allow clients to call as well -tsmith 
exec function RemoteEvent(optional name EventName, optional bool bRunOnClient)
{
	ServerRemoteEvent(EventName);
	if(Role < ROLE_Authority && bRunOnClient)
	{
		InternalRemoteEvent(EventName);
	}
}
...
{
	// FIRAXIS: change, call helper function -tsmith 
	InternalRemoteEvent(EventName);
}

// FIRAXIS begin: helper function so clients can call this as well -tsmith
private function InternalRemoteEvent(name EventName)
{
...
// FIRAXIS addition: bDisabledOnClient -tsmith
/**
...
function OnToggleCinematicMode(SeqAct_ToggleCinematicMode Action)
{
	...
	if (Action.bDisabledOnClient && Role < ROLE_Authority)
	{
		`Warn("Not supported on client");
		return;
	}
	...
	SetCinematicMode(bNewCinematicMode, Action.bHidePlayer, Action.bHideHUD, Action.bDisableMovement, Action.bDisableTurning, Action.bDisableInput, Action.bDisabledOnClient, Action.bOverrideUserMusic);
}

function SetModalMode(bool bIsModal)
{
	IgnoreMoveInput(bIsModal);
	IgnoreLookInput(bIsModal);
}

// FIRAXIS addition: bDoClientRPC -tsmith
// FIRAXIS addition: bOverrideUserMusic -ejordan
/**
 ...
 * @param	bDoClientRPC        specify TRUE if the server should notify the client to toggle cinematics. useful to turn off if client code is authoritative over the on/off of cinematics. -tsmith
 * @param	bOverrideUserMusic  specify TRUE if the user's music should be disabled so as to not play over the cinematic audio. -ejordan
 */
function SetCinematicMode( bool bInCinematicMode, bool bHidePlayer, bool bAffectsHUD, bool bAffectsMovement, bool bAffectsTurning, bool bAffectsButtons, optional bool bDoClientRPC = true, optional bool bOverrideUserMusic = false )
{
	...
	{
		// MHU - Default state of the cursor should be hidden, and we reveal it only when pathing.
		if( Pawn != None && !Pawn.Controller.IsA('XComDemoSpectator') && !Pawn.IsA('XCom3DCursor'))  // Firaxis Change
		{
...
{
	if (OldPC.PlayerReplicationInfo != none)
	{
		// copy PRI data
...
{
	AttemptGameInviteAccepted(InviteResult, bWasSuccessful);
}
...
{
	StartAcceptGameInvite(); // FIRAXIS: Moved code to shared function -ttalley
}

// FIRAXIS begin: Combining like code into a shared function and allowing for easier overriding -ttalley
/**
 * Get's overridden in XComPlayerController
 */
function StartAcceptGameInvite()
{
	FinishAcceptGameInvite(true);
}

/**
 * Finishes Controller work and calls the Online Subsystem to continue the process.
 */
...
/**
 * Finishes the invite join process whenever the player actually joins the room; or fails.
 */
...
{
	`log(`location,,'DevOnline');
	ClientTravel(ResolvedURL, TRAVEL_Absolute);
	return true;
}
 ...
 */
// FIRAXIS addition: need to pass the session name as a string due to timing issue where the player replication info 
// has not been replicated and therefore cannot get the session name from it. -tsmith
reliable client function ClientStartOnlineGame(coerce string SessionNameString)
{
	...
	if (OnlineSub != None &&
		...
		IsPrimaryPlayer())
	{
		GameSettings = OnlineSub.GameInterface.GetGameSettings(name(SessionNameString));
		// Start the game if not already in progress
		if (GameSettings != None &&
			(GameSettings.GameState == OGS_Pending || GameSettings.GameState == OGS_Ended))
		{
			OnlineSub.GameInterface.StartOnlineGame(name(SessionNameString));
		}
	}
}
...
{
	// FIRAXIS BEGIN -cdoyle
	if( (!inAction.bAllowDuringCinematics && bCinematicMode) ||
		(PlayerCamera.GetStateName() == 'DebugView') )/*||
		(XComTacticalController(GetALocalPlayerController()).GetPres().CAMGetCurrentView().IsA('XGCameraView_Aiming')) )    broken code, keeping around for possible use later */
	{
		return;
	}
	// FIRAXIS END
 ...
 */ 
reliable server function ServerDestroyActor(Actor kDestroyActor)
{
	kDestroyActor.Destroy();
}
// FIRAXIS end -tsmith
