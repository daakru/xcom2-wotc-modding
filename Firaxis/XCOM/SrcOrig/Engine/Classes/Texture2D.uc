 ...
 * textures that are being re-imported over as they will have 
 * a linker but won't have been serialized from disk 
...
// FIRAXIS BEGIN
// This is used for accessing pixels in script/native (UTexture2D::GetPixel) and completely
// bypassing the mip streaming system.  This is faster than otherwise calling Lock()
// on a resident mip level, plus these textures are tiny and generally not streamed in for
// rendering anyways.
var() bool	                                                bStoreFirstMipInSystemMemory <DisplayName="Allow Script Access"|ToolTip="If true, this texture will be kept in system memory for access from script ( or the main thread ). Must be PF_A8R8G8B8, and should be relatively small.">;
var bool	                                                bProceduralStoreFirstMipInSystemMemory; //Used when constructing a texture on the fly to indicate that this texture should store its first mip
var private{private} array<byte>	                        FirstMipInSystemMemory;
// FIRAXIS END
...
public:
	...
	virtual void CompressNormalsUsingFPData(); // JMS
	...
	/**
	 * Returns whether or not the texture has LinearColor data from
	 * import time. */
	virtual UBOOL HasImportedMips() const;
	...
	/** Returns the system memory copy of the first mip */
	TArray<BYTE>* FetchFirstMipInSystemMemory();
}
...
//FIRAXIS BEGIN
/** Returns the pixel if this texture has been saved to system memory, using bStoreFirstMipInSystemMemory in the editor. */
native final function Color GetPixel(int X, int Y);
/** Frees the first mip in system memory (if available) and returns the number of bytes that were freed. Does nothing in editor/PIE. **/
native final function int FreeFirstMipInSystemMemory();
//FIRAXIS END

defaultproperties
{
	bStoreFirstMipInSystemMemory=false // FIRAXIS ADD
	StreamingIndex=-1
