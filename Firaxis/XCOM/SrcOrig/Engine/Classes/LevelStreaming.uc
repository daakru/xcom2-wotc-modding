...
var transient const	vector							OldOffset;

var() const			rotator							RotationOffset; // FIRAXIS
...
var bool bProgrammaticallyStreamed; // FIRAXIS Add - Levels that are ProgrammaticallyStreamed need there FStreamableTextureInstance's updated
var bool bForceNoDupe;				// FIRAXIS Add - Levels that should not be duplicated from the pristine even when they are programatically streamed

// Used for Avenger Rooms to be able to turn on/off dynamic shadow casting lights
var transient array<Light> DynamicShadowLights;

// Used to hide / unhide all the actors in a level
var transient bool bHiddenActorsSet;
var transient init array<Name> HiddenActors;

delegate LevelVisibleDelegate(name LevelPackageName, optional LevelStreaming LevelStreamedIn = new class'LevelStreaming');

event LevelVisible()
{
	LevelVisibleDelegate(PackageName, self);
	LevelVisibleDelegate = none; // Reset the delegate - FIRAXIS
}
