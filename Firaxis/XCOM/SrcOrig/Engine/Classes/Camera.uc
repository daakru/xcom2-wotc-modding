...
// FIRAXIS change, allow camera to read from ini file
	config(Camera)
	notplaceable
	native(Camera)
...
//FIRAXIS - Provides a mechanism for camera fade to be locked from code such that Kismet or other sequences cannot affect it. 
//Used to ensure fades are active during loads or other sequences where they are functional rather than artistic.
var bool bCodeLockFade; 
...
simulated function ResetConstrainAspectRatio()
{
	if( WorldInfo.IsConsoleBuild() )
	{
		bConstrainAspectRatio = true;
...
//	bConstrainAspectRatio = FALSE;      // FIRAXIS, Moose - commented out
...
//	ConstrainedAspectRatio	= ViewTarget.AspectRatio;   // FIRAXIS, Moose - commented out
...
simulated function DisplayDebug(HUD HUD, out float out_YL, out float out_YPos)
{
	...
	// FIRAXIS change, Moose, for debug
	Canvas.DrawText("   bConstrainAspectRatio:" $ bConstrainAspectRatio);
	out_YPos += out_YL;
	Canvas.SetPos(4,out_YPos);
...
// FIRAXIS_BEGIN
	// Use the camera's render viewpoint instead of its location. The location is not normally updated in XCom,
	// but the viewpoint is always correct since that is where the view will actually be rendered at
