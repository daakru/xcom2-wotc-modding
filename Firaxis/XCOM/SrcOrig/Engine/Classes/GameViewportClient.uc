...
var native EGameCursorType GameCursorType;
// FIRAXIS END
...
/** Suppress the cursor during cinematics */
var native transient bool bHideMouseCursorForCinematics; // FIRAXIS
...
/** Similar to above, but render with all RenderChannels set to false */
var bool bRenderEmptyScene;
...
/** FIRAXIS ADDITION
 * True if we should update the listener position based on the current view.
 * if false, the listener position should be updated by custom code somewhere else.
 * Use this to set custom positions for the listener if needed.
 */
var bool bSetListenerPositionBasedOnView;
...
cpptext
{
	...
	/** FIRAXIS ADDITION
	 * (audio) Updates the listener's position based on the current view
	 */
	virtual void SetListenerPositionFromView( FSceneView* View, INT PlayerIndex, FVector ViewLocation );
...
event PostRender(Canvas Canvas)
{
	...
	//DrawTransition(Canvas); // FIRAXIS REMOVAL - we display our own transitions
}
...
{
	/* Firaxis RAM - no crappy text at the center of the screen - we have a UI team
...
function DrawTransitionMessage(Canvas Canvas,string Message)
{
	...
	Canvas.SetDrawColor(103,232,237); //Firaxis: bsteiner: changing message color to our official cyan
	Canvas.DrawText( Message, false );
}
...
defaultproperties
{
	...
	bHideMouseCursorForCinematics=False // FIRAXIS
	...
	bSetListenerPositionBasedOnView=true		// FIRAXIS
}
