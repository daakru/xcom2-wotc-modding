...
struct native OnlineStatsRow
{
	...
	// FIRAXIS begin: allow subsystem to read the score value -tsmith
	/** The score used to rank the player */
	var const SettingsData Score;
	// FIRAXIS end: allow subsystem to read the score value -tsmith
...
/** set this to true to allow reading of non-leaderboard specific (rank/score) if the player is not in the leaderboard */
var bool bReadRawStatsIfNotInLeaderboard;
// FIRAXIS end -tsmith

// FIRAXIS begin -ttalley
enum EStatsReadState
{
	SRS_None,
	SRS_Reading,
	SRS_Finished,
...
/** Set this to SRS_Canceled to cancel the reading of the leaderboard and flush rows already read.  This will return a delegate call if in the middle of reading, otherwise it will do nothing. */
var EStatsReadState StatsReadState;
// FIRAXIS end -ttalley
...
// FIRAXIS begin -tsmith
/**
 * Searches the rows for the player and returns their score used to rank them on the leaderboard
 *
 * @param PlayerId the player to search for
 *
 * @return the score for the player, 0 if player not found.
 */
native function int GetScoreForPlayer(UniqueNetId PlayerId);

/**
 * Sets the score for the given player, necessary to if we need to do things like default/reset the player score
 *
 * @param PlayerId the player to search for
 * @param the score for the player
 */
native function SetScoreForPlayer(UniqueNetId PlayerId, int Score);
// FIRAXIS end -tsmith