...
/** Used to override the parent materials two-sided flag to TRUE. Mainly used with destroyed materials. */
var() const bool bOverrideParentTwoSided;
...
var duplicatetransient bool bHasStaticPermutationResource;
...
// FIRAXIS BEGIN
//var native int  m_nNumStaticPermutations; // needed to serialize TArray

//var const native duplicatetransient array<PlatformParameter> StaticParameterPermutations;  // platform structs defined in material.uc
//var const native duplicatetransient array<PlatformResource> StaticResourcePermutations;    //    ... 
// FIRAXIS END
...
//var private bool m_bTogglableSwitchPermutations; // FIRAXIS ADDITION

//var private int m_nCurrentSwitchPermutation; // FIRAXIS ADDITION
...
cpptext
{
	...
	*/
	FMaterialResource* AllocateResource(UBOOL bInOverrideParentTwoSided = FALSE);
	...
	*/
	...
	/**
	 * Checks if the given flag is set on the material
	 * @param Usage - The usage flag to check
	 * @return UBOOL - TRUE if the material uses the given type.
	 */
	...
	*/
	...
	/**
	* Sets the value and the override of the given static switch parameter.  If it is not found in this instance then
	*		the request is forwarded up the MIC chain.
	...
	*/
	void UpdateStaticPermutation(UBOOL bNoReattach=FALSE);

	/**
	* Updates static parameters and recompiles the static permutation resource if necessary
	*/
	void InitStaticPermutation(UBOOL bForceFlushExistingShaderMaps=FALSE);
	...
	*/
	void CacheResourceShaders(EShaderPlatform ShaderPlatform, UBOOL bFlushExistingShaderMaps = FALSE, UBOOL bForceAllPlatforms = FALSE, UBOOL bDebugDump = FALSE, UBOOL bForceUniformExpressionRegen=FALSE);
...
native function SetBooleanParameterValue(name ParameterName, bool Value); //FIRAXIS ADDITION
native function bool SetStaticSwitchParameterValueAndReattachShader(name ParameterName, bool Value, MeshComponent MeshComp); //FIRAXIS ADDITION
