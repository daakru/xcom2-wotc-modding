...
replication
{
	...
	if( bNetDirty )
		...
		TakeHitLocation, bUsedByMatinee, bFastAttachedMove;
...
simulated function FaceRotation(rotator NewRotation, float DeltaTime)
{
	...
	if (!InFreeCam())
	{
		...
		else if ( IsA('Xcom3dCursor') ) // 09/15/2009 MHU - Do not pitch the cursor.
		{
			NewRotation.Pitch = 0;
		}
...
event TakeDamage(int Damage, Controller InstigatedBy, vector HitLocation, vector Momentum, class<DamageType> DamageType, optional TraceHitInfo HitInfo, optional Actor DamageCauser)
{
	...
	// FIRAXIS change: XGAction_Death handles the force feedback so no need for the PC variable -tsmith
	//local PlayerController PC;
	local Controller Killer;
	...
	if ( Health <= 0 )
	{
		// FIRAXIS change: XGAction_Death handles the force feedback -tsmith
		//PC = PlayerController(Controller);
	...
	{
		// MHU - When units are hit, they teleport around the world. 
		//       Commenting this line out. Justin Boswell approved.
...
function bool Died(Controller Killer, class<DamageType> DamageType, vector HitLocation)
{
	...
	foreach LatentActions(Action)
	{
		...
		if ( Action.IsA('SeqAct_Interp') && SeqAct_Interp(Action).bIsPlaying )
		{
			SeqAct_Interp(Action).Stop();
			SeqAct_Interp(Action).ActivateOutputLink(0);
			SeqAct_Interp(Action).NativeDeActivated();
		}
		// FIRAXIS END
	...
	// FIRAXIS change: our XGAction_Death will call PlayDying on the Pawn -tsmith
...
{
	if( Physics != PHYS_RigidBody && Physics != PHYS_None )
	{
		SetPhysics(PHYS_Falling);
	}
}

// FIRAXIS change: make state and functions simulated because our pawns dont get torn off, and therefore dont
// become autoritative on clients when they die -tsmith
...
ignores Bump, HitWall, HeadVolumeChange, PhysicsVolumeChange, Falling, BreathTimer, FellOutOfWorld;
	...
	// Firaxis BEGIN
	// RAM - remove Destroy, bodies should remain in the world for the duration of the tactical game
	...
	simulated event TakeDamage(int Damage, Controller EventInstigator, vector HitLocation, vector Momentum, class<DamageType> DamageType, optional TraceHitInfo HitInfo, optional Actor DamageCauser)
	{
		...
		// cjcone@psyonix: obey rigid body physics
		SetDyingPhysics();
		/*
		SetPhysics(PHYS_Falling);
		*/
	...
	simulated event BeginState(Name PreviousStateName)
	{
		...
		// Firaxis BEGIN 
		// RAM - moved set timer outside bTearOff determination,
		SetTimer(1.0, false);
...
*/
simulated function PlayDying(class<DamageType> DamageType, vector HitLoc, optional Name AnimName='', optional vector Destination)
{
	GotoState('Dying');
...
defaultproperties
{
	...
	// Network
	// TODO: only do the cursors -tsmith
	RemoteRole=ROLE_SimulatedProxy
