...
//  Just like GetContentList but actually works for save games.
function EOnlineEnumerationReadState GetSaveGames(byte LocalUserNum, out array<OnlineSaveGame> SaveGames);
 ...
 */
function bool WriteSaveGameData(byte LocalUserNum,int DeviceId,string FriendlyName,string FileName,string SaveFileName,const out array<byte> SaveGameData, const out SaveGameHeader SaveHeader);
...
// FIRAXIS BEGIN
/**
 * @return true if the last write failed because storage is full
 */
