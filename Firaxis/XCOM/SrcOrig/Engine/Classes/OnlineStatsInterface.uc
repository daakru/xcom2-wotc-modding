 ...
 * @return TRUE if the call is successful, FALSE otherwise
 */
function bool ReadOnlineStatsByRankForFriendsList(byte LocalUserNum,OnlineStatsRead StatsRead);
 ...
 * 
 * @param ViewToLeaderboardNameMap - array that is the map
 */
function SetViewToLeaderboardNameMapping(const out array<ViewIdToLeaderboardName> ViewToLeaderboardNameMap);

// NOTE: took this from Steamworks and generalizing to all interfaces. the implementation MUST be compiled out for final release. -tsmith 
/**
...
{
	`log("DEFAULT IMPLEMENTATION OF RESET STATS OVERRIDE IN SUB-CLASSES!");
	return true;
}
// FIRAXIS end -tsmith