...
/*
 * If true, this event will execute in a manner that is safe for use with the x2 history and vis system. Additional restrictions will
 * be placed on the event that will be enforced in the editor and sanity checked at load time.
 */
var const bool bGameSequenceEvent;

// FIRAXIS begin -tsmith
/** 
 *  if true the action will fire on both client and server. 
 *  use for events that affects gameplay on both client and server (ex. the dynamic level generation)
 *  trumps the bClientSideOnly flag.
 */
var() const bool bClientAndServer;
// FIRAXIS end -tsmith
/**
