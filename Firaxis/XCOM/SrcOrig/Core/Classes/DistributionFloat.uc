...
struct native RawDistributionFloat extends RawDistribution
{
	structcpptext
	{
		 ...
		 */
		FLOAT GetValue(FLOAT F) const
		{
			return const_cast<FRawDistributionFloat&>(*this).GetValue(F);
		}
		FLOAT GetValue(FLOAT F=0.0f, UObject* Data=NULL, class FRandomStream* InRandomStream = NULL);

		//Firaxis Change, dbaker. Adding Varient to pass ptr in because this value must pass into the FPU pipe
		//somehow, therefore, this helps avoid a LHS
		FLOAT GetValuePTR(FLOAT *pF, UObject* Data=NULL, class FRandomStream* InRandomStream = NULL);
		//End Firaxis Change
