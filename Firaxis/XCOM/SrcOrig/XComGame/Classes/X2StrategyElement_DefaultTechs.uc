...
static function X2DataTemplate CreateHeavyPlasmaTemplate()
{
	...
	Template.PointsToComplete = 10000;
	Template.SortingTier = 1;
...
static function X2DataTemplate CreatePlasmaSniperTemplate()
{
	...
	Template.Requirements.SpecialRequirementsFn = IsPlasmaSniperTechVisible;
...
static function bool IsPlasmaSniperTechVisible()
{
...
static function X2DataTemplate CreatePoweredArmorTemplate()
{
	...
	Template.PointsToComplete = 14000;
	Template.SortingTier = 1;
	...
	Template.Requirements.SpecialRequirementsFn = IsPoweredArmorTechVisible;
	Template.Requirements.bVisibleIfTechsNotMet = true;
...
static function bool IsPoweredArmorTechVisible()
{
	local array<name> TechNames;

	TechNames.AddItem('Tech_Elerium');
...
static function X2DataTemplate CreateEleriumTemplate()
{
	...
	Template.Requirements.SpecialRequirementsFn = IsEleriumTechVisible;
	Template.Requirements.bVisibleIfTechsNotMet = true;
...
static function bool IsEleriumTechVisible()
{
	local array<name> TechNames;
	
	TechNames.AddItem('GaussWeapons');
...
static function IntelTechCompleted(XComGameState NewGameState, XComGameState_Tech TechState)
{
	local XComGameStateHistory History;
	...
	{
		IntelAmount = GetMinAdventDatapadIntel() + `SYNC_RAND_STATIC(GetMaxAdventDatapadIntel() - GetMinAdventDatapadIntel() + 1);
	}
	else if(TechState.GetMyTemplateName() == 'Tech_AlienDatapad')
	{
		IntelAmount = GetMinAlienDatapadIntel() + `SYNC_RAND_STATIC(GetMaxAlienDatapadIntel() - GetMinAlienDatapadIntel() + 1);
	}
...
static function ResistanceCommsCompleted(XComGameState NewGameState, XComGameState_Tech TechState)
{
	local XComGameStateHistory History;
...
//	local ArtifactCost Resources;
	...
	Template.bProvingGround = true;
	Template.bRandomHeavyWeapon = true;
	Template.bRepeatable = true;
	Template.SortingTier = 3;
	Template.ResearchCompletedFn = GiveDeckedItemReward;
...
static function GiveDeckedItemReward(XComGameState NewGameState, XComGameState_Tech TechState)
{
	local X2ItemTemplateManager ItemTemplateManager;
	local X2ItemTemplate ItemTemplate;	
...
static function GiveItems(XComGameState NewGameState, XComGameState_Tech TechState)
{
	local X2ItemTemplateManager ItemTemplateManager;
	local X2ItemTemplate ItemTemplate;
	local array<name> ItemRewards;
...
static function UpgradeItems(XComGameState NewGameState, XComGameState_Tech TechState)
{
	local X2ItemTemplateManager ItemTemplateManager;
	...
	ItemTemplateManager = class'X2ItemTemplateManager'.static.GetItemTemplateManager();
	TechState.ItemsUpgraded = ItemTemplateManager.GetAllItemsCreatedByTemplate(TechState.GetMyTemplateName());
}

static function bool AlwaysPriority()
{
	return true;
}

static function bool AdventOfficerPriority()
{
	local XComGameStateHistory History;
	local XComGameState_HeadquartersXCom XComHQ;
...
static function bool AlienBiotechPriority()
{
	local XComGameStateHistory History;
...
static function bool SkulljackPriority()
{
	local XComGameStateHistory History;
	local XComGameState_HeadquartersXCom XComHQ;
...
static function bool IsFacilityMissionAvailable()
{
	local XComGameStateHistory History;
...
static function FacilityLeadCompleted(XComGameState NewGameState, XComGameState_Tech TechState)
{
	...
	{
		FacilityMission = Missions[`SYNC_RAND_STATIC(Missions.Length)];
		FacilityMission = XComGameState_MissionSite(NewGameState.ModifyStateObject(class'XComGameState_MissionSite', FacilityMission.ObjectID));
...
static function int GetMinAdventDatapadIntel()
{
	return `ScaleStrategyArrayInt(default.MinAdventDatapadIntel);
}

static function int GetMaxAdventDatapadIntel()
{
	return `ScaleStrategyArrayInt(default.MaxAdventDatapadIntel);
}

static function int GetMinAlienDatapadIntel()
{
	return `ScaleStrategyArrayInt(default.MinAlienDatapadIntel);
}

static function int GetMaxAlienDatapadIntel()
{
	return `ScaleStrategyArrayInt(default.MaxAlienDatapadIntel);
}