	...
	foreach VisualizeGameState.IterateByClassType(class'XComGameState_EnvironmentDamage', DamageEvent)
	{
		foreach DamageEvent.DestroyedActors( ActorID )
		{
			...
			if (Destructible.ObjectID == DestructibleState.ObjectID)
			{
				...
				LookAtAction = X2Action_CameraFrameVisualizationActions(class'X2Action_CameraFrameVisualizationActions'.static.AddToVisualizationTree(ActionMetadata, VisualizeGameState.GetContext(), false, ActionMetadata.LastActionAdded));
				...
				class'X2Action_ApplyWeaponDamageToTerrain'.static.AddToVisualizationTree(ActionMetadata, VisualizeGameState.GetContext(), false, ActionMetadata.LastActionAdded);
