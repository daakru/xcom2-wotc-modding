...
function BuildVisualization(XComGameState GameState)
{
	...
	{
		Metadata.StateObject_OldState = EvacZone;
		Metadata.StateObject_NewState = EvacZone;
		class'X2Action_SyncVisualizer'.static.AddToVisualizationTree(Metadata, GameState.GetContext());
	}
}
...
{
	local ETeam Team;
	local XComGameState_EvacZone NewZone;
	...
	NewZone = class'XComGameState_EvacZone'.static.PlaceEvacZone(GameState, ActualSpawnLocation, Team, BlueprintMapOverride);
	NewZone.bMissionPlaced = true;
}
...
private function Vector GetSpawnLocation()
{
	...
	foreach `BATTLE.AllActors(class'XComGroupSpawn', Spawn)
	{
		...
		// Too far
		if (TilesFromSpawn > MaximumTilesFromLocation)
			continue;
