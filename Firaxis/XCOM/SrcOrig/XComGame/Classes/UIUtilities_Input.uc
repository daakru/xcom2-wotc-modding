...
const ICON_LBRB_L1R1        = "Icon_LBRB_L1R1"; // bsg-jrebar (5/23/17): Removing inlined buttons and adding ultrawide LBRB
...
static function string FindAbilityKey(string sSource, string sAction, int iActionEnum, out byte bKeyNotFound, XComKeybindingData kKeybinds, PlayerInput kPlayerInput, optional KeybindCategories category = eKC_Tactical )
{
	...
	return Repl( sSource, sAction, "" ); // bsg-jrebar (4/24/17): Remove the Action Keybind placeholder from source string
}
...
static function string InsertAbilityKeys(string sSource, out byte bKeyNotFound, XComKeybindingData kKeybinds, PlayerInput kPlayerInput)
{
	...
	sResult = FindAbilityKey(sResult, "%OPEN_CHOSEN_HUD", eTBC_OpenChosenHUD, bKeyNotFound, kKeybinds, kPlayerInput);
...
static function string InsertSteamIcons(string sSource) 
{
	...
	//O eKC_Tactical, eTBC_OpenChosenHUD
	SteamIcon = "steam_"$SteamMgr.GetSteamControllerActionOrigin(eKC_Tactical, eTBC_OpenChosenHUD);
	sResult = Repl(sResult, MarkForKeybinding("O"), class'UIUtilities_Text'.static.InjectImage(SteamIcon, IconSize, IconSize, 0));
