...
delegate bt_status BTLeafDelegate();

native static function bool FindBTDelegateByName(name strName, out delegate<BTLeafDelegate> dOutFn);
...
static function bool SplitNameAtHyphen(Name strName, out Name PreHyphen, out Name PostHyphen)
{
	local string strSearch;
	local int InStrResult;
	strSearch = string(strName);
	InStrResult = InStr(strSearch, "-");
	if (InStrResult > 0)
	{
		PreHyphen = Name(Left(strSearch, InStrResult));
		PostHyphen = Name(Mid(strSearch, InStrResult+1)); 
		return true;
	}
	return false;
}
