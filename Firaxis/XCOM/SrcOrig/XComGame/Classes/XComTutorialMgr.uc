...
simulated event StartReplay(int SessionStartStateIndex)
{
	...
	// 500 - Osei's first shot
	//1745 - end of the tutorial
	//1057 - Overwatch
	//TutorialStartFrame = 1057; //Way of quickly starting at an advanced frame in the tutorial
...
function ResetUnitIsMoving()
{
	ResetUnit.m_bIsMoving = false;
}
...
state WaitForPlayerInput
{
	simulated function bool HandleSubmittedGameStateContext(XComGameStateContext SubmittedContext)
	{
		...
		{
			//Not ideal, but lets this fix be very focused. Under normal circumstances, m_bIsMoving could never be false if the pathing system presented a valid path. However,
			//in the tutorial the player is allowed to explore with their path but really can only follow the on-rails directions.
			Unit = XGUnit(History.GetVisualizer(AbilityContext.InputContext.SourceObject.ObjectID));
			if (Unit != none)
			{
				ResetUnit = Unit;				
				SetTimer(0.1, false, nameof(ResetUnitIsMoving));
			}

			TacticalHUD = `Pres.m_kTacticalHUD;
			TacticalHUD.m_kAbilityHUD.bTutorialMgrInvalidSubmission = true;
