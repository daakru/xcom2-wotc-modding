...
//           in frame in exactly the same location as they did in matinee. For example, if the target
//           is two floors above the shooter, the camera will move down and rotate upward to keep both
//           heads correctly in frame. In addition to this, it provides some simple tethering logic to
//           smooth out the tracking of bobbing head animations. Note that "shooter" and "target" are
//           do not actually need to be shooting or a target, they merely indicate whose shoulder
//           the camera should sit over. You can specify any two units as the "shooter" and "target".
//           
//---------------------------------------------------------------------------------------
...
var private const config float FiringUnitHeadBlockedScorePenalty; // When scoring a shot for blockage, how many points are incurred when LOS to the shooter's head is blocked?
var private const config float FiringUnitWaistBlockedScorePenalty; // When scoring a shot for blockage, how many points are incurred when LOS to the shooter's waist is blocked?
...
var private const config float TargetedLocationBlockedScorePenalty; // When scoring a shot for blockage, how many points are incurred when LOS to the target's head (or targeting location, if not a unit) is blocked?
...
var private const config InterpCurveFloat RetargetLocationCurve; // Specifies an alpha curve to blend with when switching targets 
var private const config InterpCurveFloat RetargetRotationCurve; // Specifies an alpha curve to blend with when switching targets 
var private const config float RetargetDurationMin; // The minimum amount of time that a retargeting blend should last.
var private const config float RetargetDurationMax; // The maximum amount of time that a retargeting blend should last.
var private const config float RetargetDurationMaxTiles; // 
var private const config float TetherDuration; // How long should the tether take to fully catch up when the shooter or target ref point moves?
...
var private const config float MinTilesBetweenTraceSamples; // The number of tiles the camera needs to travel in matinee before we take another raytrace sample for blocking determination
...
var private const config float PriorityPenalty[MatineeSelectionPriority.EnumCount]; // Allows artists to assign an arbitrary penalty to a camera, and have that affect the scoring system.
...
// If the actor is none, will use this location as the target focus
...
/// </summary>
protected native function TPOV ComputeCameraLocationAndOrientationAtTime(X2MatineeInfo Matinee, XComUnitPawnNativeBase ShooterPawn, bool bUsePredictedHeadPos, Vector InTargetLocation, float Time);

/// <summary>
/// Returns the head location as close as we can easily predicted for the shooter when targeting the target location.
/// </summary>
static native function Vector GetPredictedHeadLocation(XComUnitPawnNativeBase ShooterPawn, Actor TargetOfShooter, const out vector InTargetLocation);
...
protected function Vector GetUnitFacing()
{
	...
	if (FiringUnit.GetDirectionInfoForPosition(TargetGroundLocation, OutVisibilityInfo, OutCoverIndex, OutPeekSide, OutCanSeeFromDefault, OutRequiresLean))
	{
...
function SetTargetLocation(vector NewTarget, optional Vector CurrentCameraLocation)
{
	TargetDirty = NewTarget != TargetLocation;
...
{	
	...
	if (FiringUnit != none && FiringUnit.GetPawn() != none)
	{
		`BATTLE.GetALocalPlayerController().GetPlayerViewPoint(CameraLocation, CameraRotation);

		UnitBounds = FiringUnit.GetPawn().Mesh.Bounds;
		outDitherParameters.DitherRadius = VSize(CameraLocation - UnitBounds.Origin);
		outDitherParameters.DitherFloor = UnitBounds.Origin.Z - UnitBounds.BoxExtent.Z + 5.0f;
		outDitherParameters.IgnorActors.AddItem(FiringUnit.GetPawn());

		if(XGUnit(TargetActor) != none)
			outDitherParameters.IgnorActors.AddItem(XGUnit(TargetActor).GetPawn());
		else
			outDitherParameters.IgnorActors.AddItem(TargetActor);

		return true;
	}
	else
	{
		return super.AllowProximityDither(outDitherParameters);
	}
}
