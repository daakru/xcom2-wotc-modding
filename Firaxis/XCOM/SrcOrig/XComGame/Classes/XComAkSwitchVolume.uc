...
// Entry and exit AkEvents make it possible to do things that AkStates can't, like alter audio effects (e.g. reverb).
// Array of akEvents that should fire when the camera enters this volume
var() array<AkEvent> EntryAkEvents;

// Array of akEvents that should fire when the camera exits this volume
var() array<AkEvent> ExitAkEvents;
...
simulated event Tick(float DeltaTime)
{
	...
	local AkEvent AkSwitchVolumeEvent;
	...
	if(VSizeSq(Camera.CameraCache.POV.Location - LastCheckLocation) > MinDistanceBetweenChecksSquared)
	{
		...
		if(WasInVolume && !IsInVolume)
		{
			...
			foreach ExitAkEvents(AkSwitchVolumeEvent)
			{
				SoundMgr.PlayAkEvent(AkSwitchVolumeEvent);
			}
		}
		else if(!WasInVolume && IsInVolume)
		{
			...
			SoundMgr = `XTACTICALSOUNDMGR;
			foreach EntryAkEvents(AkSwitchVolumeEvent)
			{
				SoundMgr.PlayAkEvent(AkSwitchVolumeEvent);
			}
		}
