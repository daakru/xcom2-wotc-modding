...
function X2MPCharacterTemplate FindMPCharacterTemplateByCharacterName(name CharacterTemplateName)
{
	local X2DataTemplate Template;
	local X2MPCharacterTemplate kCharacterTemplate;
	foreach IterateTemplates(Template, none)
	{
		kCharacterTemplate = X2MPCharacterTemplate(Template);
		if (kCharacterTemplate != None && kCharacterTemplate.CharacterTemplateName == CharacterTemplateName)
		{
			return kCharacterTemplate;
		}
	}
	return none;
}
