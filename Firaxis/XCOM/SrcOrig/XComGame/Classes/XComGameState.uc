...
structcpptext
{
	FStateObjectReference()
	{
		...
		static_assert(sizeof(FStateObjectReference) == sizeof(ObjectID), "FStateObjectReference size has changed. Please ensure all members are properly initialized");
		checkSlow(sizeof(*this) == sizeof(ObjectID));
		ObjectID = 0;
	}
...
cpptext
{
	...
	FORCEINLINE type_t* ModifyState( INT ObjectID )
	{
		return CastChecked<type_t>( ModifyStateObject( type_t::StaticClass( ), ObjectID ) );
	}

	template <class type_t>
	FORCEINLINE type_t* CreateNewState( class UX2DataTemplate* InitTemplate=NULL )
	{
		return CastChecked<type_t>( CreateNewStateObject( type_t::StaticClass( ), InitTemplate ) );
	}
...
var() privatewrite XComGameState ParentGameState;						// The GameState (if any) that was at the top of the stack of Submitted GameStates at the time that this GameState was submitted.
																		// This GameState is in essence the Parent/Instigator of this GameState having being created
...
//This map is for safer state object purging by ref-counting the creations/modification events
var private native Map_Mirror ObjectIDRefCount{ TMap<INT, INT> };
...
/// <param name="ObjectID">A parameter specifying the state object to update to a state object that already exists.</param>
native final function XComGameState_BaseObject ModifyStateObject(class StateClass, int ObjectID, optional int HACK_HistoryIndex = -1);
...
native final function XComGameState_BaseObject CreateNewStateObject(class StateClass, optional X2DataTemplate InitTemplate);
...
native final function XComGameState_BaseObject DebugGetGameStateForObjectIndex(int GameStateArrayIndex, optional EGameStateReturnType DesiredReturnType=eReturnType_Reference);
...
function SetParentGameState(XComGameState InParentGameState)
{
	ParentGameState = InParentGameState;
}

/// *******************************************************************************************************************************
/// MOD LEGACY APIs
///    - These functions should not be called by Firaxis Code.  They exist for backwards compatibilty of XCom2 Mods for use
...
/// </summary>
/// <param name="StateObject">The state object to add to this game state. MUST have been created by a corresponding ModifyStateObject</param>
