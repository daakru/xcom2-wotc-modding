...
//	ScanButton.OnSizeRealized = OnButtonSizeRealized;
	...
	if (Entity.Get2DLocation() != CenterWorld)
	{
		...
		Entity = XComGameState_WorldRegion(NewGameState.ModifyStateObject(class'XComGameState_GeoscapeEntity', Entity.ObjectID));
...
function UpdateFlyoverText()
{
	...
	local String RegionLabel, ChosenLabel;
	...
	if( ControllingChosen != none && ControllingChosen.bMetXCom && !ControllingChosen.bDefeated && RegionState.HaveMadeContact() )
	{
		...
		ChosenLabel = RegionState.GetChosenRegionText();
	}
	RegionLabel = class'UIUtilities_Text'.static.GetColoredText(RegionState.GetMyTemplate().DisplayName @ ChosenLabel, GetRegionLabelColor());
...
function GenerateTooltip(string tooltipHTML)
{
	...
	if(!(RegionState.bCanScanForContact || RegionState.bCanScanForOutpost))
	{
		...
		super.GenerateTooltip(strTooltip);
	}
}
