...
function Activated(TPOV CurrentPOV, X2Camera PreviousActiveCamera, X2Camera_LookAt LastActiveLookAtCamera)
{
	...
	if(SkipIfPathAlreadyInSafeZone)
	{
		...
		if(MoveAbility.InputContext.MovementPaths.Length > 0)
		{
		...
		{
			`Redscreen("X2Camera_FollowMovingUnit was given a move ability with no paths: " $ MoveAbility.ToString());
		}
...
function UpdateCamera(float DeltaTime)
{
	...
	// We don't care about the Vis Points from the beginning of movement. We already take care of what we need for that.
