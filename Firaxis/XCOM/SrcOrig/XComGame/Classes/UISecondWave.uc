...
var localized array<string> m_arrGameplayToggleTitle;
var localized array<string> m_arrGameplayToggleDesc;
...
simulated function UpdateData()
{
	...
	for( i = 0; i < eGO_MAX; i++ )
	{
		...
		if( !`XPROFILESETTINGS.Data.IsGameplayToggleUnlocked(i) )
			continue;
		...
		if(m_bViewOnly)
			kCheckbox.bChecked = Movie.Pres.IsGameplayOptionEnabled(i);
		else
			kCheckbox.bChecked = `XPROFILESETTINGS.Data.GetGameplayOption(i); //This query is only valid in the shell! 		
...
simulated function UpdateToggle()
{
	...
	eOption = CheckboxIndexToGameplayOptionMap[m_hWidgetHelper.GetCurrentSelection()];
