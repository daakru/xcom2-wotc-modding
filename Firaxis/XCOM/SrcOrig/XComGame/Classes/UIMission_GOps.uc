...
simulated function UpdateButtons()
{
	...
	if (GetNumMissions() > 0 && arrMissions[0].GetReference() == MissionRef)
	{
		Navigator.SetSelected(Button1);
		UpdateStickyButton(Button1);
	}

	if (GetNumMissions() > 1 && arrMissions[1].GetReference() == MissionRef)
	{
		Navigator.SetSelected(Button2);
		UpdateStickyButton(Button2);
	}

	if (GetNumMissions() > 2 && arrMissions[2].GetReference() == MissionRef)
	{
