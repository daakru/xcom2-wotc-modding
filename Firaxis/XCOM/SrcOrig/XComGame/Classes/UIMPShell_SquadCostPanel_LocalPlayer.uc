...
function InitLocalPlayerSquadCostPanel(X2MPShellManager kShellManager, XComGameState kLoadoutGameState)
{
	...
	PRI = GetALocalPlayerController().PlayerReplicationInfo;
	kEventMgr.m_kMPLastMatchInfo.m_kLocalPlayerInfo.m_kUniqueID = PRI.UniqueId;
	kEventMgr.FillPRIFromLastMatchPlayerInfo(kEventMgr.m_kMPLastMatchInfo.m_kLocalPlayerInfo, XComPlayerReplicationInfo(PRI));
	m_kLocalPRI = XComMPLobbyPRI(PRI);
	if (m_kLocalPRI != none)
	{
	...
	super.InitSquadCostPanel(maxSquadCost, m_kLoadoutGameState, PRI.PlayerName);
}
...
function XComGameState CreateUpdateGameStatePlayer()
{
	...
	{
		NewPlayerState = XComGameState_Player(NewGameState.ModifyStateObject(class'XComGameState_Player', m_kLocalPlayerGameStateRef.ObjectID));
