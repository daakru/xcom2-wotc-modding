...
{
	var deprecated bool bAITakenDamage;  // Tracked for Reinforcement triggers.
	var deprecated bool bAIDeath;		// Tracked for Reinforcement triggers.
	var deprecated bool bEnemyDamaged;	// Tracked for Reinforcement triggers.

	var deprecated array<int> TakenDamageUnitIDs; // Keeping track of units that have taken damage since last turn.  Reset in OnEndTurn
	var deprecated array<int> TakenFireUnitIDs;   // Keeping track of units that have been unsuccessfully attacked since last turn. Reset in OnEndTurn
...
var config array<int> YellCooldownDuration;
...
function OnBeginTacticalPlay(XComGameState NewGameState)
{
	...
	`Log("OnBeginTacticalPlay called for XComGameState_AIPlayerData class.");
}
...
function EventListenerReturn OnObjectiveComplete(Object EventData, Object EventSource, XComGameState GameState, Name Event, Object CallbackData)
{
	...
	if( !bConvergedToObjective && BattleData.AllStrategyObjectivesCompleted() )
	{
		...
		if( `CHEATMGR != None && `CHEATMGR.bAllPodsConvergeOnMissionComplete )
		{
			...
			{
				Group = XComGameState_AIGroup(NewGameState.ModifyStateObject(class'XComGameState_AIGroup', GroupRef.ObjectID));
		...
		PlayerData = XComGameState_AIPlayerData(NewGameState.ModifyStateObject(class'XComGameState_AIPlayerData', ObjectID));
...
function bool HasEnemyVIP(optional out XComGameState_Unit VIP_out)
{
	...
	if( TacticalMissionMgr.GetVIPCharacterTemplate(VIPTemplate) )
	{
		foreach History.IterateByClassType(class'XComGameState_Unit', UnitState)
		{
			if( UnitState.GetTeam() != eTeam_Alien && UnitState.GetMyTemplateName() == VIPTemplate.DataName )
			{
				VIP_out = UnitState;
				break;
			}
...
function bool IsEnemyVIP(int TargetObjectID)
{
	local X2CharacterTemplate VIPTemplate;
	local XComGameStateHistory History;
	local XComGameState_Unit UnitState;
	...
	if (TacticalMissionMgr.GetVIPCharacterTemplate(VIPTemplate))
	{
		UnitState = XComGameState_Unit(History.GetGameStateForObjectID(TargetObjectID));
		if (UnitState.GetTeam() != eTeam_Alien && UnitState.GetMyTemplateName() == VIPTemplate.DataName)
		{
			return true;
		}
...
function UpdateGroupData(XComGameState NewGameState)
{
	...
	`LogAI("UpdateGroupData::Added "$iGroupCount@"groups from NewGameState.");
	...
	`LogAI("UpdateGroupData::Added "$iGroupCount@"groups from History.");
}
...
function UpdateForMindControlledUnit(XComGameState NewGameState, XComGameState_Unit MindControlledUnit, StateObjectReference InstigatorRef)
{
	...
	{
		`LogAI("Failed to update mind-controlled unit - no associated group found to remove from.");
	}
	...
	if (InstigatorRef.ObjectID > 0)
	{
		...
		GroupState = XComGameState_AIGroup(NewGameState.ModifyStateObject(class'XComGameState_AIGroup', iGroupID));
		GroupState.AddUnitToGroup(MindControlledUnit.ObjectID, NewGameState);
		if (iGroupID <= 0)
		{
			`LogAI("Failed to find group for instigator of mind-control!  Adding both instigator and mind-controlled unit to new group!");
			GroupState.AddUnitToGroup(InstigatorRef.ObjectID, NewGameState);
		}
	...
	{
		`LogAI("Failed to update mind-controlled unit - no valid instigator found.");
	}
}
...
function UpdateForMindControlRemoval(XComGameState NewGameState, XComGameState_Unit MindControlledUnit)
{
	...
	// Remove unit from instigator group.
...
function bool RemoveFromCurrentGroup(StateObjectReference UnitRef, XComGameState NewGameState, bool bAddToDisplaced=false)
{
	...
	{
		GroupState = XComGameState_AIGroup(NewGameState.ModifyStateObject(class'XComGameState_AIGroup', iGroupID));
		GroupState.RemoveUnitFromGroup(UnitRef.ObjectID, NewGameState);
...
function bool TransferUnitToGroup(StateObjectReference GroupRef, StateObjectReference UnitRef, XComGameState NewGameState)
{
	...
	{
		GroupState = XComGameState_AIGroup(NewGameState.ModifyStateObject(class'XComGameState_AIGroup', GroupRef.ObjectID));
		if( GroupState != None )
		{
			GroupState.AddUnitToGroup(UnitRef.ObjectID, NewGameState);
			UpdateGroupData(NewGameState);
			return true;
		}
	}
...
{
	return `ScaleTacticalArrayInt(YellCooldownDuration);
}
...
static function int GetNumEngagedEnemies(out StateObjectReference OutEngagedUnitRef, bool bIncludeLost=false)
{
	local XComGameState_Unit UnitState;
	local XComGameStateHistory History;
	...
	foreach History.IterateByClassType(class'XComGameState_Unit', UnitState)
	{
		...
		if( UnitState.IsAbleToAct() && (UnitTeam == eTeam_Alien || (bIncludeLost && UnitTeam == eTeam_TheLost)) )
		{
			...
			if( !UnitState.IsUnrevealedAI() )
			{
				...
				{
					++NumEngagedEnemies;
					OutEngagedUnitRef = UnitState.GetReference();
				}
				else
				{
...
function UpdateFightMgrStats( XComGameState NewGameState )
{
	...
	StatsData.NumEngagedAI = GetNumEngagedEnemies(EngagedUnitRef);
...
function CreateNewAIUnitGameStateIfNeeded(int UnitID, XComGameState NewGameState)
{
	...
	{
		AIUnitData = XComGameState_AIUnitData(NewGameState.CreateNewStateObject(class'XComGameState_AIUnitData'));
...
function AddKismetJob(Name JobName, int Priority=0, int TargetID=0)
{
	...
	NewAIPlayerDataState = XComGameState_AIPlayerData(NewGameState.ModifyStateObject(class'XComGameState_AIPlayerData', ObjectID));
