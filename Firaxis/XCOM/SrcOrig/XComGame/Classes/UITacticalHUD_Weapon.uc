	...
	if(SelectedAction.AbilityObjectRef.ObjectID > 0)
	{
		...
		if( m_kWeapon == AbilityState.GetSourceWeapon() )
		{
			...
			foreach AbilityTemplate.AbilityCosts(AbilityCost)
			{
				...
				{
					return X2AbilityCost_Ammo(AbilityCost).CalcAmmoCost(AbilityState, m_kWeapon, none);
				}
			}
		}
	}
...
simulated function AS_X2SetAmmo( int ammoCurrent, int ammoMax, int ammoHighlight, bool hasBullets, int soldierID )
{
