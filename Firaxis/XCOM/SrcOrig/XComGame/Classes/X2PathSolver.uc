...
{	
	...
	public:
		...
		const struct FHazardousTilesCache* ConcealmentCache; // concealment cache for the unit, if any
		const UX2PathSolver& DefaultPathSolverObject; // for getting configurable cost bias values

	private:
		const struct FHazardousTilesCache* GetConcealmentCacheForUnit(class UXComGameState_Unit* InUnit);
...
native static function bool IsUnitPotentiallyTrapped(XComGameState_Unit Unit, TTile StartTile, optional int MinTileDistance = 6);