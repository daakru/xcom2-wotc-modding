...
var localized string m_strNoNationality;
...
var localized string m_strNoClassVariants;
...
simulated function XComGameState_Unit GetUnit()
{
	if(Movie.Pres.m_kCustomizeManager != none)
	{
		...
		bIsXPACSoldier = Unit.IsChampionClass();
		bDisableVeteranOptions = false; // !Unit.IsVeteran() && !InShell();
	}
...
{
	class'UIUtilities'.static.DisplayUI3D(DisplayTag, name(CameraTag), bInMP? 0.0f : `HQINTERPTIME);
...
{
	return NewUnit.IsSoldier() && !NewUnit.IsDead() && !NewUnit.IsOnCovertAction();
}
