...
var config array<int> DefenseMatrixSabotageReduction;
...
static function bool PowerCoreNeedsAttention(StateObjectReference FacilityRef)
{
	local XComGameStateHistory History;
	local XComGameState_HeadquartersXCom XComHQ;
...
static function InterruptCheckpowerCore(StateObjectReference FacilityRef)
{
	local XComGameStateHistory History;
	local XComGameState_HeadquartersXCom XComHQ;
...
static function LeavePowerCoreWithoutResearchPopup()
{
...
static function OnLeavePowerCoreWithoutResearchPopupCallback(Name eAction)
{
	local XComGameStateHistory History;
	local XComGameState NewGameState;
	local XComGameState_FacilityXCom FacilityState;
...
static function bool IsResearchProjectActive(StateObjectReference FacilityRef)
{
	local XComGameState_HeadquartersXCom XComHQ;
...
static function string GetResearchQueueMessage(StateObjectReference FacilityRef)
{
	local XComGameState_HeadquartersXCom XComHQ;
...
static function X2DataTemplate CreateHangarTemplate()
{
	...
	// XPack soldier intro map
	MapInfo.MapName = "CIN_XP_SoldierIntros";
	MapInfo.InitiallyVisible = true;
	Template.AuxMaps.AddItem(MapInfo);
...
static function bool ArmoryNeedsAttention(StateObjectReference FacilityRef)
{
	local XComGameStateHistory History;
	local XComGameState_HeadquartersXCom XComHQ;
	...
	return XComHQ.HasSoldiersToPromote();
}

static function string GetHangarQueueMessage(StateObjectReference FacilityRef)
{
	local XComGameState_HeadquartersXCom XComHQ;
	...
	iAvailable = XComHQ.GetNumberOfDeployableSoldiers(true);
...
static function X2DataTemplate CreateBarMemorialTemplate()
{
	...
	Template.FillerSlots.AddItem('Soldier');
	Template.FillerSlots.AddItem('Soldier');
	Template.FillerSlots.AddItem('Griever');
	Template.FillerSlots.AddItem('Griever');
	Template.FillerSlots.AddItem('Griever');
	Template.FillerSlots.AddItem('Griever');
	Template.FillerSlots.AddItem('Griever');
...
static function OnLaboratoryBuilt(StateObjectReference FacilityRef)
{
	...
	XComHQ.ModifyBreakthroughTechTimer(class'XComGameState_HeadquartersXCom'.default.BreakthroughLaboratoryModifier);

	if (XComHQ.bLabBonus)
	{
		XComHQ.ResearchEffectivenessPercentIncrease += `ScaleStrategyArrayInt(class'X2StrategyElement_DefaultContinentBonuses'.default.PursuitOfKnowledgeBonus);		
	}
...
static function OnLaboratoryRemoved(StateObjectReference FacilityRef)
{
	...
	NewXComHQ.ModifyBreakthroughTechTimer(1.0 / class'XComGameState_HeadquartersXCom'.default.BreakthroughLaboratoryModifier);

	if (NewXComHQ.bLabBonus)
	{
		NewXComHQ.ResearchEffectivenessPercentIncrease -= `ScaleStrategyArrayInt(class'X2StrategyElement_DefaultContinentBonuses'.default.PursuitOfKnowledgeBonus);
	}
...
static function X2DataTemplate CreateWorkshopTemplate()
{
	...
	// Staff Slots
	StaffSlotDef.StaffSlotTemplateName = 'WorkshopStaffSlot';
	Template.StaffSlotDefs.AddItem(StaffSlotDef);
...
static function X2DataTemplate CreatePowerRelayTemplate()
{
	...
	// Staff Slots
	StaffSlotDef.StaffSlotTemplateName = 'PowerRelayStaffSlot';
	Template.StaffSlotDefs.AddItem(StaffSlotDef);
...
static function X2DataTemplate CreateAdvancedWarfareCenterTemplate()
{
	...
	Template.OnFacilityBuiltFn = OnAWCBuilt;
	Template.OnFacilityRemovedFn = OnAWCRemoved;
	Template.BaseMinFillerCrew = 1;
	...
	Template.FacilityCompleteNarrative = "";
	Template.FacilityUpgradedNarrative = "";
	Template.ConstructionStartedNarrative = "X2NarrativeMoments.Avenger_Tutorial_Infirmary_Construction";

	Template.Upgrades.AddItem('Infirmary_RecoveryChamber');

	// Staff Slots
	StaffSlotDef.StaffSlotTemplateName = 'AWCScientistStaffSlot';
	Template.StaffSlotDefs.AddItem(StaffSlotDef);

	StaffSlotDef.StaffSlotTemplateName = 'InfirmarySoldierStaffSlot';
	Template.StaffSlotDefs.AddItem(StaffSlotDef);
...
static function OnAWCBuilt(StateObjectReference FacilityRef)
{
	local XComGameStateHistory History;
	local XComGameState_HeadquartersXCom XComHQ;
	local XComGameState NewGameState;
	...
	XComHQ = XComGameState_HeadquartersXCom(NewGameState.ModifyStateObject(class'XComGameState_HeadquartersXCom', XComHQ.ObjectID));
	XComHQ.bCanCureNegativeTraits = true;

	`XCOMGAME.GameRuleset.SubmitGameState(NewGameState);
}

static function OnAWCRemoved(StateObjectReference FacilityRef)
{
	local XComGameState NewGameState;
	local XComGameState_HeadquartersXCom NewXComHQ;
	...
	NewGameState = class'XComGameStateContext_ChangeContainer'.static.CreateChangeState("On AWC Removed");
	RemoveFacility(NewGameState, FacilityRef, NewXComHQ);
	NewXComHQ.bCanCureNegativeTraits = false;
	NewXComHQ.ResetNegativeTraitRecovery(NewGameState);
...
static function X2DataTemplate CreateShadowChamberTemplate()
{
	...
	// Staff Slots
	StaffSlotDef.StaffSlotTemplateName = 'ShadowChamberShenStaffSlot';
	StaffSlotDef.bStartsLocked = true;
	Template.StaffSlotDefs.AddItem(StaffSlotDef);

	StaffSlotDef.StaffSlotTemplateName = 'ShadowChamberTyganStaffSlot';
	Template.StaffSlotDefs.AddItem(StaffSlotDef);
...
static function X2DataTemplate CreateOfficerTrainingSchoolTemplate()
{
	...
	Template.SoldierUnlockTemplates.AddItem('MeditationPreparationUnlock');
	Template.SoldierUnlockTemplates.AddItem('ParkourUnlock');
	Template.SoldierUnlockTemplates.AddItem('InfiltrationUnlock');
	
	// Staff Slots
	StaffSlotDef.StaffSlotTemplateName = 'OTSStaffSlot';
	Template.StaffSlotDefs.AddItem(StaffSlotDef);

	Template.MaxFillerCrew = 6 - Template.StaffSlotDefs.Length;
...
static function OnGTSRemoved(StateObjectReference FacilityRef)
{
	...
	RemoveFacility(NewGameState, FacilityRef, NewXComHQ);
	NewXComHQ.ClearGTSSoldierUnlockTemplates();
...
static function X2DataTemplate CreateUFODefenseTemplate()
{
	...
	// Staff Slots
	StaffSlotDef.StaffSlotTemplateName = 'UFODefenseStaffSlot';
	Template.StaffSlotDefs.AddItem(StaffSlotDef);
...
static function OnDefenseFacilityBuilt(StateObjectReference FacilityRef)
{
	...
	XComHQ.SabotageChanceModifier += `ScaleStrategyArrayInt(default.DefenseMatrixSabotageReduction);
...
static function OnDefenseFacilityRemoved(StateObjectReference FacilityRef)
{
	...
	NewXComHQ.SabotageChanceModifier -= `ScaleStrategyArrayInt(default.DefenseMatrixSabotageReduction);
...
static function X2DataTemplate CreateResistanceCommsTemplate()
{
	...
	// Staff Slots
	StaffSlotDef.StaffSlotTemplateName = 'ResCommsStaffSlot';
	Template.StaffSlotDefs.AddItem(StaffSlotDef);

	StaffSlotDef.StaffSlotTemplateName = 'ResCommsBetterStaffSlot';
	StaffSlotDef.bStartsLocked = true;
	Template.StaffSlotDefs.AddItem(StaffSlotDef);
...
static function X2DataTemplate CreatePsiChamberTemplate()
{
	...
	StaffSlotDef.StaffSlotTemplateName = 'PsiChamberSoldierStaffSlot';
	Template.StaffSlotDefs.AddItem(StaffSlotDef);
...
static function SelectFacility(StateObjectReference FacilityRef, optional bool bForceInstant = false)
{
	...
	`GAME.GetGeoscape().m_kBase.m_kAmbientVOMgr.DisableChosenAmbientVO();
...
protected static function RemoveFacility(XComGameState NewGameState, StateObjectReference FacilityRef, optional out XComGameState_HeadquartersXCom NewXComHQ)
{
	local XComGameStateHistory History;
	local XComGameState_HeadquartersXCom XComHQ;
	local XComGameState_FacilityXCom FacilityState;
...
protected static function EmptyFacilityProjectStaffSlots(StateObjectReference FacilityRef)
{
	local XComGameState_FacilityXCom FacilityState;
	local XComGameState_StaffSlot SlotState;
	local int idx;
