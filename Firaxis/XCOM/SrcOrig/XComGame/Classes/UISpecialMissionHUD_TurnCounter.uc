...
simulated function OnTurnChange(bool bLocalTurn)
{
	...
	if ((Timer != none) && !Timer.bIsChallengeModeTimer)
	{
		if(bLocalTurn)
		{
			...
			AS_SetLabel( m_strPlayerTurnLabel);
		}
		else
		{
			SetUIState(eUIState_Bad);
			AS_SetLabel( m_strOpponentTurnLabel);
		}
		AS_SetSubLabel(" ");
...
simulated function RefreshCounter()
{
	...
	if(( Timer != none ) && !Timer.bIsChallengeModeTimer)
	{
		// TTP#13126: always going to set the player label in MP as the edge/turn events are not getting fired on the first turn.
...
function EventListenerReturn PlayerTurnBegin(Object EventData, Object EventSource, XComGameState GameState, Name Event, Object CallbackData)
{
