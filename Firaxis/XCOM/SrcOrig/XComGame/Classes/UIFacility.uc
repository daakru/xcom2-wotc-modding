...
var UIText m_kTitle;
var UIFacility_CovertOpsStatus m_kCovertOpsStatus;
...
simulated function RealizeNavHelp()
{
	...
	NavHelp.AddSelectNavHelp(); //bsg-jrebar (4.6.17): reordering navehlp for consistency 
...
simulated function CreateStaffSlotContainer()
{
	if (m_kStaffSlotContainer == none)
	{
		...
		m_kStaffSlotContainer.AnchorTopCenter(); //bsg-crobinson (5.5.17): Fix to offset staff slots in facility view
...
simulated function CreateCovertOpsStatus()
{
	if( m_kCovertOpsStatus == none )
	{
		m_kCovertOpsStatus = Spawn(class'UIFacility_CovertOpsStatus', self).InitCovertOpsStatus();
	}
}
...
simulated function CloseScreen()
{
	...
	if(`ISCONTROLLERACTIVE == true && Movie.Stack.GetCurrentClass() != class'UIFacilitySummary')
		`HQPRES.LookAtSelectedRoom(GetFacility().GetRoom(), `HQINTERPTIME);

	`HQPRES.ClearQueuedListExpansions();
}
