...
var config array<name> BeginnerVOObjectives;
var config name BeginnerVONonTutorialObjective;
var config name XPackNarrativeObjective;
var config name XPackTutorialNarrativeObjective;
var config name XPackNonTutorialNarrativeObjective;
var config name XPackNonNarrativeObjective;
...
event OnCreation(optional X2DataTemplate Template)
{
	...
	m_Template = X2ObjectiveTemplate(Template);
...
static function SetUpObjectives(XComGameState StartState)
{
	...
	if (CampaignState.bTutorialEnabled)
	{
		...
		// Always start the XPack Narrative content when the base game Tutorial is turned on
		StartObjectiveByName(StartState, default.XPackNarrativeObjective);
		StartObjectiveByName(StartState, default.XPackTutorialNarrativeObjective);

		// Always turn on beginner VO objectives when Tutorial is enabled
		for (idx = 0; idx < default.BeginnerVOObjectives.Length; idx++)
		{
			StartObjectiveByName(StartState, default.BeginnerVOObjectives[idx]);
		}
	}
	else
	{
		...
		// The XPack narrative can be enabled independently of the base game Tutorial
		if (CampaignState.bXPackNarrativeEnabled)
		{
			StartObjectiveByName(StartState, default.XPackNarrativeObjective);
			StartObjectiveByName(StartState, default.XPackNonTutorialNarrativeObjective);
			bBeginnerVOEnabled = true; // Force beginner VO on when XPack narrative is enabled
		}
		else
		{
			StartObjectiveByName(StartState, default.XPackNonNarrativeObjective);
			bBeginnerVOEnabled = !CampaignState.bSuppressFirstTimeNarrative;
		}
		...
		foreach StartState.IterateByClassType(class'XComGameState_Objective', ObjectiveState)
		{
			...
			// Complete beginner VO objectives if it is not turned on
			if(!bBeginnerVOEnabled)
			{
				if (default.BeginnerVOObjectives.Find(ObjectiveState.GetMyTemplateName()) != INDEX_NONE ||
		...
		// If beginner VO is enabled, start the objectives
		if (bBeginnerVOEnabled)
		{
			for (idx = 0; idx < default.BeginnerVOObjectives.Length; idx++)
			{
...
function StartObjective(XComGameState NewGameState, optional bool bForceStart=false)
{
	...
	if( SubObjectives.Length > 0 )
	{
		...
		{
			ObjectiveState = XComGameState_Objective(NewGameState.ModifyStateObject(class'XComGameState_Objective', SubObjectives[0].ObjectID));
		}
...
function CompleteObjective(XComGameState NewGameState)
{
	...
	if( IsMainObjective() )
	{
		...
		for( idx = 0; idx < SubObjectives.Length; idx++ )
		{
			...
			if( ObjectiveState == none )
			{
				ObjectiveState = XComGameState_Objective(NewGameState.ModifyStateObject(class'XComGameState_Objective', SubObjectives[idx].ObjectID));
			}
	...
	else
	{
		...
		if( ObjectiveState == None )
		{
			ObjectiveState = XComGameState_Objective(NewGameState.ModifyStateObject(class'XComGameState_Objective', MainObjective.ObjectID));
...
static function BuildVisualizationForNarrative(XComGameState VisualizeGameState)
{
	...
	foreach VisualizeGameState.IterateByClassType(class'XComGameState_Objective', ObjectiveState)
	{
		for (i = 0; i < ObjectiveState.PendingNarrativeTriggers.Length; i++)
		{
			...
			ActionMetadata.StateObject_OldState = ObjectiveState;
			ActionMetadata.StateObject_NewState = ObjectiveState;
					}
	}

}

function EventListenerReturn OnNarrativeEventTrigger(Object EventData, Object EventSource, XComGameState GameState, Name EventID, Object CallbackData)
{
	...
	if (Pres != none)
	{
		return Pres.OnNarrativeEventTrigger(self, EventData, EventSource, GameState, EventID);
	}
	else
	{
		return DoNarrativeEventTrigger(EventData, EventSource, GameState, EventID);
	}
}
...
function EventListenerReturn DoNarrativeEventTrigger(Object EventData, Object EventSource, XComGameState GameState, Name EventID)
{
	...
	if( EventData != None )
	{
		if (EventData.IsA('XComGameState_BaseObject'))
		{
			TriggeringTemplateName = XComGameState_BaseObject(EventData).GetMyTemplateName();
		}
		else if (EventData.IsA('X2DataTemplate'))
		{
			TriggeringTemplateName = X2DataTemplate(EventData).DataName;
		}
	}

	// before doing any history modifications, make sure we actually have some event that is valid
	// and can fire. Modifications are expensive.
	if( TheTemplate.CompletionEvent != EventID && TheTemplate.RevealEvent != EventID )
	{
		AnyEventsAreValid = false;
		foreach TheTemplate.NarrativeTriggers(EventTrigger)
		{
			...
			{
				AnyEventsAreValid = true;
				break;
			}
		}

		if(!AnyEventsAreValid)
		{
			return ELR_NoInterrupt;
		}
	}
	...
	ObjectiveState = XComGameState_Objective(NewGameState.ModifyStateObject(class'XComGameState_Objective', ObjectID));
...
//---------------------------------------------------------------------------------------
function bool ShouldCompleteWithSimCombat()
{
	if(GetMyTemplate().TacticalCompletion && GetStateOfObjective() == eObjectiveState_InProgress)
	{
		if(GetMyTemplate().ShouldCompleteWithSimCombatFn != none)
		{
			return GetMyTemplate().ShouldCompleteWithSimCombatFn();
		}
