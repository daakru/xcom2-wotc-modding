...
simulated function InitLaunchButton()
{
	...
	class'UIUtilities_Input'.static.GetGamepadIconPrefix() $ class'UIUtilities_Input'.const.ICON_X_SQUARE, 26, 26, -5) @ m_strConfirm); //bsg-jneal (2.2.17): fix for InjectImage calls not using platform prefix for button icons
	
	LaunchButton.AnchorBottomRight();
	LaunchButton.SetVisible(false);
	LaunchButton.ShowBG(true);
	Navigator.RemoveControl(LaunchButton);
}
...
simulated function UpdateNavHelpInGamepadMode()
{
	...
	if(m_kSquadLoadout != none)
	{
		...
		IntegratedNavHelp.AddCenterHelp(m_strRenameSet, class'UIUtilities_Input'.const.ICON_LSCLICK_L3); // bsg-jrebar (04/12/17): Adding LStick press for rename
	...
	switch( cmd )
	{
		...
		case class'UIUtilities_Input'.const.FXS_BUTTON_Y:
			if(m_kSquadLoadout != none)
				CloneButtonCallback();
			return true;
		// bsg-jrebar (04/12/17): Adding LStick press for rename
		case class'UIUtilities_Input'.const.FXS_BUTTON_L3:
			if(m_kSquadLoadout != none)
				RenameButtonCallback();
			return true;
		// bsg-jrebar (04/12/17): end
...
function OnDisplayConfirmDeleteDialogAction(Name eAction)
{
...
{
	local UIMechaListItem Loadout;
	if(SquadList.NumChildren() > 0)
	{
		Loadout = UIMechaListItem(SquadList.GetItem(0));
		if (Loadout != none)
		{
			m_kSquadLoadout = XComGameState(Loadout.metadataObject);
		}
		UpdateSquadInfoPanel();
	}
}
...
simulated function OnDisplayConfirmMPAccountDialogAction(Name eAction)
{
	if (eAction == 'eUIAction_Accept')
	{
		//Nothing happens
