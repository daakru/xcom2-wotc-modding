...
simulated function UpdateHistoryListTurns()
{
	...
	{
		PlayerState = XComGameState_Player(CachedHistory.GetGameStateForObjectID(BattleData.PlayerTurnOrder[PlayerIndex].ObjectID));
		if( PlayerState != None )
		{
			PlayerStateObjects.AddItem(PlayerState);
		}
	}
	...
	{		
		...
		if( GameRuleState.GameRuleType == eGameRule_PlayerTurnBegin )
		{
			...
			PlayerIndex = (PlayerIndex + 1) % PlayerStateObjects.Length;
		}
	}
	...
	case 0:				
		...
		{
			TurnNumber = TurnIndex / PlayerStateObjects.Length;
	...
	case 2:
		...
		for( HistoryIndex = 0; HistoryIndex < NumGameStates; HistoryIndex++ )
		{
			XComGameStateData.strName = string(AssociatedGameStateFrame.DebugGetGameStateForObjectIndex(HistoryIndex).Name);
			XComGameStateData.iValue = HistoryIndex;
			GameStatesList.AddItem( XComGameStateData );
		}
		break;
...
function UpdateListItem()
{
	...
	case eFilterType_Turns:		
		...
		case 2:
			...
			AssociatedGameStateFrame = CachedHistory.GetGameStateFromHistory(HistoryIndex, eReturnType_Reference);
			TextString = AssociatedGameStateFrame.DebugGetGameStateForObjectIndex(m_kDepthSelection[m_kListDepth]).ToString();
			m_kStateDetailText.SetText( TextString );
			break;
		}
		break;
