...
// If true, don't advance time on the matinee
var privatewrite bool Paused;

var float m_fDitherRadiusDefault;

var bool	m_bAllowProximityDither;

var array<XComUnitPawnNativeBase> m_kUnitsInMatinee;
...
function bool AllowProximityDither(out DitherParameters outDitherParameters)
{
	...
	local int i;
	local float WorldFloor;

	if (m_bAllowProximityDither == false && (MatineeInfo.Matinee == none || MatineeInfo.Matinee.EnableDitherEffectInMatinee == false))
		return false;

	// I would rather these all be combined into a single array that I am grabbing actors from, but it is a little late in the project
	// to rework this stuff, potentially causing bugs. As for now, having duplicates in array is ok and this is a safer fix.

	for (i = 0; i < m_kUnitsInMatinee.Length; ++i)
	{
		//Duplicates are ok here.
		outDitherParameters.IgnorActors.AddItem(m_kUnitsInMatinee[i]);
	}

	for (i = 0; i < ActorsToIgnoreForBlockingDetermination.Length; ++i)
	{
		//Duplicates are ok here.
		outDitherParameters.IgnorActors.AddItem(ActorsToIgnoreForBlockingDetermination[i]);
	}

	if (MatineeFocusActor != none)
	{
		`BATTLE.GetALocalPlayerController().GetPlayerViewPoint(CameraLocation.Location, CameraLocation.Rotation);

		if (XGUnit(MatineeFocusActor) != none)
		{
			UnitBounds = XGUnit(MatineeFocusActor).GetPawn().Mesh.Bounds;					
		}
		else if (SkeletalMeshActor(MatineeFocusActor) != none)
		{
			UnitBounds = SkeletalMeshActor(MatineeFocusActor).SkeletalMeshComponent.Bounds;
		}
		else if (XComUnitPawn(MatineeFocusActor) != none)
		{
			UnitBounds = XComUnitPawn(MatineeFocusActor).Mesh.Bounds;
		}
		else
		{
			UnitBounds.Origin = MatineeFocusActor.Location;
			UnitBounds.BoxExtent = vect(0, 0, 0);
		}		

		outDitherParameters.DitherRadius = VSize(CameraLocation.Location - UnitBounds.Origin);

		WorldFloor = `XWORLD.GetFloorZForPosition(UnitBounds.Origin);
		outDitherParameters.DitherFloor = Max(WorldFloor, UnitBounds.Origin.Z - UnitBounds.BoxExtent.Z);
		outDitherParameters.DitherFloor = outDitherParameters.DitherFloor + 5.0f;

		outDitherParameters.IgnorActors.AddItem(MatineeFocusActor);
	}
	else
	{
		outDitherParameters.DitherRadius = m_fDitherRadiusDefault;
		outDitherParameters.DitherFloor = 1.0f;
	}
	return true;
}
...
defaultproperties
{
	...
	Paused=false
	m_fDitherRadiusDefault=200.0
	m_bAllowProximityDither=true
}