...
simulated function InitScreen(XComPlayerController InitController, UIMovie InitMovie, optional name InitName)
{
	...
	if (NeedResearchReportPopup(TechRefs))
	{
		...
		for (idx = 0; idx < TechRefs.Length; idx++)
		{
			...
			{
				`HQPRES.UIBreakthroughResearchComplete(TechRefs[idx]);
			}
			else
			{
...
simulated function OnCancel()
{
	...
	if( Movie.Pres.ScreenStack.bCinematicMode ) return;
...
simulated function OnLeaveFacilityWithoutStaffPopupCallback(Name eAction)
{
...
simulated function OnLeaveFacilityWithoutProjectPopupCallback(Name eAction)
{
	if (eAction == 'eUIAction_Accept')
		`HQPRES.UIChooseResearch();
	else
...
simulated function RealizeNavHelp()
{
	...
	NavHelp.AddSelectNavHelp(); // BSG-jrebar (4.5.17): Add Select prompt to main research screen
