...
simulated function float GetHitRange(const XComGameState_Ability Ability)
{
	...
	local float MultiTargetHitRange;
	Template = Ability.GetMyTemplate();
	if (Template.AbilityMultiTargetStyle != None)
	{
		MultiTargetHitRange = Template.AbilityMultiTargetStyle.GetHitRange(Ability);
		if (MultiTargetHitRange > 0)
		{
			return MultiTargetHitRange;
		}
	}
	return `METERSTOUNITS(GetCursorRangeMeters(Ability));
}
