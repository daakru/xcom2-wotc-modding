...
function SetHoverMaterialValue(float Value)
{
	...
	for( i = 0; i < NUM_TILES; ++i)
	{
		...
		if(MICMat != none)
		{
			if(NumTimesHovered == 0)
			{
				MICMat = new(self) class'MaterialInstanceConstant';
				MICMat.SetParent(OverworldMeshs[i].GetMaterial(0));
				MICMat.SetScalarParameterValue('Hover', Value);
				OverworldMeshs[i].SetMaterial(0, MICMat);
			}
			else
			{
				MICMat.SetScalarParameterValue('Hover', Value);
