...
var private vector LastCameraLocation;
var private bool PausedGC;
...
function TPOV GetCameraLocationAndOrientation()
{
	...
	LocationDelta = EffectiveTPOV.Location - LastCameraLocation;

	if( VSize(LocationDelta) > 0.01 )
	{
		if( !PausedGC )
		{
			`XENGINE.PauseGC();
	...
	LastCameraLocation = EffectiveTPOV.Location;


	return EffectiveTPOV;
}

function bool IsCameraInStack( X2Camera TestCamera )
{
	local int i;

	for( i = 0; i < Cameras.Length; i++ )
	{
		if( Cameras[i] == TestCamera )
		{
			return true;
		}
	}
...
function X2Camera FindCameraWithTag(name CameraTag)
{
	local int Index;
	...
	if( CameraTag != '' )
	{
		for( Index = 0; Index < Cameras.Length; Index++ )
		{
			...
			{
				if( CameraCheck.CameraTag == CameraTag )
				{
					return CameraCheck;
				}
...
private function CleanupCameraAnims()
{
	...
	Battle = `TACTICALGRI != none ? `BATTLE : none;
	if(Battle == none) return;
...
function OnVisTreeEndMarkerCompleted(X2Action_MarkerTreeInsertEnd EndNode)
{
	local X2Camera CameraIterator;
	local X2Camera_FrameAbility FrameAbilityCamera;
	...
	for (Index = 0; Index < Cameras.Length; Index++)
	{
		...
		while (CameraIterator != none)
		{
			FrameAbilityCamera = X2Camera_FrameAbility(CameraIterator);
			if (FrameAbilityCamera != none)
			{
				AbilityContext = XComGameStateContext_Ability(EndNode.StateChangeContext);
				if (AbilityContext != none)
				{
					FrameAbilityCamera.OnMarkerEndNodeCompleted(AbilityContext);
				}
			}
			CameraIterator = CameraIterator.ChildCamera;
		}
...
{
	outDitherParameters.DitherRadius = 0.0f;
	outDitherParameters.DitherFloor = 1.0f;
	return ActiveCamera != none ? ActiveCamera.AllowProximityDither(outDitherParameters) : false;
}
