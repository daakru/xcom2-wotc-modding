...
simulated function UIArmory_PromotionItem InitPromotionItem(int InitRank)
{
	...
	AbilityIcon1.bDisableSelectionBrackets = !`ISCONTROLLERACTIVE; //bsg-jneal (3.17.17): show selection brackets when using controller if receiving focus
	...
	AbilityIcon2.bDisableSelectionBrackets = !`ISCONTROLLERACTIVE; //bsg-jneal (3.17.17): show selection brackets when using controller if receiving focus
...
simulated function RealizeInfoButtons()
{
	if(`ISCONTROLLERACTIVE)
	{
		...
		// bsg-nlong (1.26.17): Hiding the info buttons on console
		InfoButton1.MC.SetBool("_visible", !`ISCONSOLE && bIsFocused && !bIsDisabled && !bEligibleForPromotion && !ClassIcon.bIsVisible && AbilityIcon1.bIsFocused);
		InfoButton2.MC.SetBool("_visible", !`ISCONSOLE && bIsFocused && !bIsDisabled && !bEligibleForPromotion && AbilityIcon2.bIsFocused);
		// bsg-nlong (1.26.17): end
	}
	else
	{
