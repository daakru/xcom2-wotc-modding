...
static function VisualizeSecondaryFires(XComGameState GameState)
{
	...
	foreach GameState.IterateByClassType(class'XComGameState_WorldEffectTileData', TileDataStateObject)
	{
		...
		if (TileDataStateObject.StoredTileData.Length > 0)
		{
			...
			if (ApplyEffect != none)
			{
				class'X2Action_WaitForWorldDamage'.static.AddToVisualizationTree(ActionMetadata, GameState.GetContext());
				ApplyEffect.AddX2ActionsForVisualization(GameState, ActionMetadata, 'AA_Success');
			}
			else
			{
...
native function bool IsAffectedDestructibleActor( ActorIdentifier ActorID );
