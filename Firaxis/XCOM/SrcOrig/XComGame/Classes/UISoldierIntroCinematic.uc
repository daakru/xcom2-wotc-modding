...
simulated function HideExistingPawn()
{
	...
	XComUnitPawn(ExistingPawn).Mesh.SetBlockRigidBody(false);
}
...
simulated function DismissScreen()
{
	...
	GermanVOIsTooLong = (GetLanguage() == "DEU" || GetLanguage() == "ITA" || GetLanguage() == "ESN")
		&& (StartEventBase == "CIN_ArmorIntro_RageSuit" || StartEventBase == "CIN_ArmorIntro_IcarusSuit" || StartClassIntro == 'CIN_ClassIntro_Reaper');

	// there is no audio playing during the Spark intro, so do not cut off any conversations when it ends
	// in case ambient VO triggers during the matinee
	bSparkIntro = (StartClassIntro == 'CIN_ClassIntro_Spark');

	if(!GermanVOIsTooLong && !bSparkIntro)
	{
