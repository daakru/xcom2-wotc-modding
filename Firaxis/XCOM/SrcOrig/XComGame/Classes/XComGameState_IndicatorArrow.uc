...
static private function XComGameState_IndicatorArrow CreateArrow(XComGameState_Unit InUnit, Vector InLocation, float InOffset, EUIState InColor, int InCounterValue, string InIcon)
{
	...
	if(InUnit != none)
	{
		...
		PointToUnit = XComGameState_Unit( NewGameState.ModifyStateObject(class'XComGameState_Unit', InUnit.ObjectID) );
	}

	Arrow = XComGameState_IndicatorArrow(NewGameState.CreateNewStateObject(class'XComGameState_IndicatorArrow'));
