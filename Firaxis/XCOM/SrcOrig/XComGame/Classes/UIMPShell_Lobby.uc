...
simulated function InitScreen(XComPlayerController InitController, UIMovie InitMovie, optional name InitName)
{
	...
	{
		StartButton.SetText(class'UIUtilities_Text'.static.InjectImage(class'UIUtilities_Input'.static.GetGamepadIconPrefix() $ class'UIUtilities_Input'.const.ICON_Y_TRIANGLE, 26, 26, -10) @ class'UIStartScreen'.default.m_sPressStartPC);
		// bsg-jrebar (5/5/17): Use Gamepad Icon Prefix
		ReadyButton = Spawn(class'UILargeButton', self).InitLargeButton('ContinueButton', class'UIUtilities_Text'.static.InjectImage(
				class'UIUtilities_Input'.static.GetGamepadIconPrefix() $ class'UIUtilities_Input'.const.ICON_X_SQUARE, 26, 26, -10) @ Caps(m_strReadyButtonText));
...
function UpdateNavHelp()
{
	...
	{		
		...
		if(m_kLocalPlayerInfo.GetPlayerReady())
		{
			...
			// bsg-jrebar (5/5/17): Use Gamepad Icon Prefix
			ReadyButton.SetText(class'UIUtilities_Text'.static.InjectImage(class'UIUtilities_Input'.static.GetGamepadIconPrefix() $ class'UIUtilities_Input'.const.ICON_X_SQUARE, 26, 26, -10) @ m_strMPLoadout_CancelReady);
		...
		{
			// bsg-jrebar (5/5/17): Use Gamepad Icon Prefix
			ReadyButton.SetText(class'UIUtilities_Text'.static.InjectImage(class'UIUtilities_Input'.static.GetGamepadIconPrefix() $ class'UIUtilities_Input'.const.ICON_X_SQUARE, 26, 26, -10) @ m_strMPLoadout_Ready);
		}
	}
	else
	{
...
simulated function OnInit()
{
    ...
    OnlineSub = class'GameEngine'.static.GetOnlineSubsystem(); 
	// bsg-jrebar (5/16/17): Init after load and update data delay
	InitWidgets();
	UpdateData();
...
function InitWidgets()
{
	...
	UpdateButtons(); 	// bsg-jrebar (5/16/17): Update buttons after info item creation
}
...
function SetupMapData(XComGameState StrategyStartState, XComGameState TacticalStartState)
{
	...
	MPMission = XComGameState_MissionSite(StrategyStartState.CreateNewStateObject(class'XComGameState_MissionSite'));
	...
	MPMission = XComGameState_MissionSite(TacticalStartState.ModifyStateObject(class'XComGameState_MissionSite', MPMission.ObjectID));
...
function OnDisplayConfirmExitDialogAction(Name eAction)
{
...
simulated function UpdateData(optional bool bFillSquad)
{
	...
	UIList_SquadEditor(m_kSlotList).SelectFirstListItem(); // bsg-jrebar (5/16/17): Ensure first item selection after update
	UpdateButtons();
}
