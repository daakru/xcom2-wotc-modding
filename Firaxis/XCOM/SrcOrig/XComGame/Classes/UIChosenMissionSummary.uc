...
simulated function OnChosenCapture()
{
	...
	SoldierClass = AffectedUnit.GetSoldierClassTemplate();

	MC.BeginFunctionOp("SetSoldierCapture");
	MC.QueueString(SoldierCapturedLabel);
	MC.QueueString(SoldierClass.IconImage);
	MC.QueueString(class'UIUtilities_Image'.static.GetRankIcon(AffectedUnit.GetRank(), SoldierClass.DataName));
	MC.QueueString(`GET_RANK_STR(AffectedUnit.GetRank(), SoldierClass.DataName));
	MC.QueueString(AffectedUnit.GetName(eNameType_RankFull));
	MC.QueueString(SoldierClass.DisplayName);
