...
{
	m_iReticuleMode = reticleEnum - 1;
	MC.FunctionNum("setMode", m_iReticuleMode);
}

simulated function SetReaperLabels(string hitChance, string rifleManufa)
{
	MC.BeginFunctionOp("setReaperLabels");
	MC.QueueString(hitChance);
	MC.QueueString(rifleManufa);
	MC.EndOp();
}
...
simulated function SetLoc(float NewX, float NewY)
{
	...
	local LinearColor scopeModePosition;
	...
	if (m_iReticuleMode == eUIReticle_Vektor-1) // UI enum doesnt have a NONE
	{
		scopeModePosition.R = -Loc.X;
		scopeModePosition.G = -Loc.Y;
		`Pres.SetPostProcessEffectVectorValue(class'X2SoldierClass_DefaultChampionClasses'.default.ScopePostProcessOn, 'TargetPos', scopeModePosition);
		`Pres.SetPostProcessEffectVectorValue(class'X2SoldierClass_DefaultChampionClasses'.default.ScopePostProcessOff, 'TargetPos', scopeModePosition);
		`Pres.SetPostProcessEffectVectorValue(class'X2SoldierClass_DefaultChampionClasses'.default.ShadowScopePostProcessOn, 'TargetPos', scopeModePosition);
		`Pres.SetPostProcessEffectVectorValue(class'X2SoldierClass_DefaultChampionClasses'.default.ShadowScopePostProcessOff, 'TargetPos', scopeModePosition);
	}
}
