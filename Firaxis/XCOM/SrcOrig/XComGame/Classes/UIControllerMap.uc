...
//bsg-jedwards (5.11.17) : Added localization for zoom in and zoom out
var localized string m_sZoomIn;
var localized string m_sZoomOut;
//bsg-jedwards (5.11.17) : end
...
var localized string m_sViewChosenDatabase; // bsg-hlee (05.16.17): String for L3 on geoscape controller map.
//</workshop>
...
var UINavigationHelp NavHelp; //bsg-crobinson (5.25.17): Make NavHelp a member instead of local
// Psuedo CTOR
simulated function InitScreen(XComPlayerController InitController, UIMovie InitMovie, optional name InitName)
{
	...
	//bsg-crobinson (5.25.17): NavHelp fixes
	if (`TACTICALGRI != none)
	{
...
//bsg-crobinson (5.25.17): NavHelp Functions
...
simulated function LoadDefaultBattlescape()
{
	if( class'UIUtilities_Input'.static.IsAdvanceButtonSwapActive() )
	{
		...
		UIGamePad[2].label = m_sTakeAction; //bsg-crobinson (5.9.17): Changed to "Take Action" from "Select/Scan"
	}
	else
	{	
		UIGamePad[0].label = m_sTakeAction; //bsg-crobinson (5.9.17): Changed to "Take Action" from "Select/Scan"
...
simulated function LoadDefaultGeoscape()
{
	...
	UIGamePad[9].label = m_sPrevious;
	UIGamePad[10].label = m_sNext;
	UIGamePad[11].label = m_sZoomOut; //bsg-jedwards (5.11.17) : Adding zoom in and zoom out text for Geoscape's controller map
	UIGamePad[12].label = m_sZoomIn;
	UIGamePad[13].label = m_sViewChosenDatabase; //bsg-hlee (05.16.17): Adding in the string for L3 to view Chosen Database.
