...
function TestBondIcon()
{
	local UIBondIcon BondIcon, BondIconDepth;

	BondIconDepth = Spawn(class'UIBondIcon', self);
	BondIconDepth.InitBondIcon('BondIcon0_Depth', 2 );
	BondIconDepth.SetPosition(100, 100);


	BondIcon = Spawn(class'UIBondIcon', self);
	BondIcon.InitBondIcon('BondIcon0', 0);
	BondIcon.SetPosition(130, 120);

	//BondIconDepth.MoveToHighestDepth();
	
	BondIcon = Spawn(class'UIBondIcon', self);
	BondIcon.InitBondIcon('BondIcon1', 1, ClickBondIconTest);
	BondIcon.SetPosition(100, 200);

	BondIcon = Spawn(class'UIBondIcon', self);
	BondIcon.InitBondIcon('BondIcon2', 2, ClickBondIconTest);
	BondIcon.SetPosition(100, 300);

}
function ClickBondIconTest(UIBondIcon Icon)
{
	`log("Bond Icon Loaded: " @ Icon.MCName);
}

function UIChosenInfo()
{
	local UIChosenInfo TempScreen;

	TempScreen = Spawn(class'UIChosenInfo', self);
...
function TestIconInjection()
{
	local UIText Value;
	local string txt;
	txt = "This is a sample of button icons. %KEY:P% and another %KEY:Q%";
	...
	Value.SetHTMLText(class'UIUtilities_Input'.static.InsertPCIcons(txt));
}
...
simulated function TestIconSelector()
{
	local UIIconSelector IconSelector;

	IconSelector = Spawn(class'UIIconSelector', self);
	IconSelector.IniticonSelector(, 100, 100, , , , TestImage_OnSelect, TestImage_OnAccept);
}

simulated function TestImageSelector()
{
	local UIImageSelector ImageSelector;

	ImageSelector = Spawn(class'UIImageSelector', self);
	ImageSelector.InitImageSelector(, , , , , , TestImage_OnSelect, TestImage_OnAccept);
}

simulated function TestImage_OnSelect(int iIndex)
{
	`log( "You selected image #"$iIndex$". Pretend like this previewed the image on the target." );
}
simulated function TestImage_OnAccept(int iIndex)
{
	`log( "You confirmed image #"$iIndex$". Pretend like this confirmed the change and updated image on the target." );
}
...
simulated function bool OnUnrealCommand(int cmd, int arg)
{
	...
	switch( cmd )
	{
		...
		case class'UIUtilities_Input'.const.FXS_KEY_7 :
			Movie.Pres.Notify("Regular MESSAGE!", class'UIUtilities_IMage'.const.UnitStatus_StasisLanced);
			//Movie.Pres.Notify("Another normal MESSAGE!", class'UIUtilities_IMage'.const.UnitStatus_StasisLanced);
			break;


		case class'UIUtilities_Input'.const.FXS_KEY_8 :
			Movie.Pres.NotifyBanner("BANNER MESSAGE!", class'UIUtilities_IMage'.const.UnitStatus_StasisLanced, "Subtitle Area", "77", eUIState_Normal);
			Movie.Pres.Notify("Regular MESSAGE!", class'UIUtilities_IMage'.const.UnitStatus_StasisLanced);
			Movie.Pres.NotifyBanner("Regular MESSAGE!", class'UIUtilities_IMage'.const.UnitStatus_StasisLanced);
			//Movie.Pres.NotifyBanner("ANOTHER BANNER MESSAGE!", class'UIUtilities_IMage'.const.UnitStatus_Berserk, "Subtitle Area with more info", "88", eUIState_Bad);
			break;
