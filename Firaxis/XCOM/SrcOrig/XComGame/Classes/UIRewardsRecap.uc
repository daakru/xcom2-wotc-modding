...
simulated function InitScreen(XComPlayerController InitController, UIMovie InitMovie, optional name InitName)
{
	...
	{
		`XCOMGRI.DoRemoteEvent(FactionState.GetFactionLeaderCinEvent());
		FactionRecapEvents();
	}
	else
	{
...
function FactionRecapEvents()
{
	...
	{		
		if (BattleData.bToughMission)
		{
			`XEVENTMGR.TriggerEvent(FactionState.GetNegativeFeedbackEvent(), , , NewGameState);
		}
		else if (BattleData.bGreatMission)
		{
			`XEVENTMGR.TriggerEvent(FactionState.GetPositiveFeedbackEvent(), , , NewGameState);
		}
		else
		{
			bSubmitGameState = false;
...
{
	local int i, iSlot;
	local array<string> arrRewardStrings, EffectsData;
	local string MissionName, MissionLocation, MissionSummary, GlobalEffects, RewardLabel;
	local bool bFoundRewards;
	...
	// Reward Panel	
	arrRewardStrings = GetRewardStrings();
	iSlot = 0;
	for (i = 0; i < arrRewardStrings.Length; i++)
	{
		if (arrRewardStrings[i] != "")
		{
			AS_UpdateCouncilAfterActionRewards(iSlot, arrRewardStrings[i]);
			iSlot++;
			bFoundRewards = true;
		}
	}
	
	// Only display the rewards label string if there are rewards
	RewardLabel = "";
	if (bFoundRewards)
	{
		RewardLabel = m_strReward;
	}
	MC.FunctionString("UpdateCouncilAfterActionRewardLabel", RewardLabel);
...
simulated function AS_UpdateCouncilAfterActionRewards(int slotIndex, string strRewardValue)
{
	MC.BeginFunctionOp("UpdateCouncilAfterActionRewards");
	MC.QueueNumber(slotIndex);
...
simulated function array<String> GetRewardStrings()
{
	return RESHQ().RecapRewardsStrings;
}
...
simulated function XComGameState_ResistanceFaction GetResistanceFaction()
{
	return GetMission().GetResistanceFaction();
}
...
simulated function Remove()
{
	...
	`XCOMGRI.DoRemoteEvent('CIN_HideReaperLeader');
	`XCOMGRI.DoRemoteEvent('CIN_HideSkirmisherLeader');
	`XCOMGRI.DoRemoteEvent('CIN_HideTemplarLeader');
}
