...
simulated function UIIcon LoadIcon(string newPath)
{
	...
	{
		ImagePath = newPath;
		mc.FunctionString("loadIcon", class'UIUtilities_Image'.static.ValidateImagePath(ImagePath));
	}
	return self;
}
...
simulated function OnLoseFocus()
{
	...
	if( bIsNavigable )
	{
		if( bEnableMouseAutoColor && !IsDisabled)
		{
			SetForegroundColor(DefaultForegroundColor);
			SetBGColor(DefaultBGColor);
		}
	...
	{
		//bsg-jneal (3.17.17): show selection brackets when using controller if receiving focus
		if (`ISCONTROLLERACTIVE && !bDisableSelectionBrackets)
		{
			mc.FunctionVoid("hideSelectionBrackets");
		}
