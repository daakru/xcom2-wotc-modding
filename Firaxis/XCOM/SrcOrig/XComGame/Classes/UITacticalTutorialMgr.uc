...
{	
	...
	if( bInitializeMessage || m_bShowingHoverHelp)
	{
			...
			!m_kActiveUnit.CanSatisfyHoverRequirements())
		{
			...
			XComPresentationLayer(Owner).QueueWorldMessage( m_strCursorHelpNoFuel,
																	  messageV3, 
																	  ,
																	  eColor_Xcom, 
...
{	
	...
	if( bInitializeMessage || m_bShowingFlyHelp)
	{
		if( CanFly() )
		{
			...
			XComPresentationLayer(Owner).QueueWorldMessage( m_strCursorHelpFlyActive,
																	  messageV3, 
																	  ,
																	  eColor_Xcom, 
																	  class'UIWorldMessageMgr'.const.FXS_MSG_BEHAVIOR_STEADY, 
...
{	
	...
	if( bInitializeMessage || m_bShowingDashHelp)
	{
		if( ShouldShowDashMessage() )
		{
			...
			XComPresentationLayer(Owner).QueueWorldMessage( dashMessage,
																	  messageV3,
																	  ,
																	  eColor_Xcom, 
																	  class'UIWorldMessageMgr'.const.FXS_MSG_BEHAVIOR_STEADY, 
	...
	if( XComUnitPawn(kTouchingActor) != none )
	{
		...
		if( kUnit != none )
		{
			...
			if( kUnit.IsAlive() && kUnit.IsCriticallyWounded() && kUnit.m_bStunned)
			{
				...
				XComPresentationLayer(Owner).QueueWorldMessage( m_strCursorHelpStunned,
																		  messageV3,
																		  ,
																		  eColor_Xcom, 
																		  class'UIWorldMessageMgr'.const.FXS_MSG_BEHAVIOR_STEADY, 
			...
			else if( kUnit.IsAlive() && kUnit.IsCriticallyWounded() )
			{
				...
				XComPresentationLayer(Owner).QueueWorldMessage( sStabilize,
																		  messageV3, 
																		  ,
																		  eColor_Xcom, 
																		  class'UIWorldMessageMgr'.const.FXS_MSG_BEHAVIOR_STEADY, 
			...
			else if( kUnit.IsDead() && !kUnit.m_bForceHidden )
			{
				...
				XComPresentationLayer(Owner).QueueWorldMessage( kUnit.SafeGetCharacterName() @ sDeadString,
																		  messageV3,
																		  ,
																		  eColor_Xcom, 
																		  class'UIWorldMessageMgr'.const.FXS_MSG_BEHAVIOR_STEADY, 
