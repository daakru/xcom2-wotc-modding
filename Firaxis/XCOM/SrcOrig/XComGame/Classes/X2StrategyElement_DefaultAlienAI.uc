...
static function array<X2DataTemplate> CreateTemplates()
{
	...
	// XPACK
	AIComponents.AddItem(CreateChosenTakeActionTemplate());
	AIComponents.AddItem(CreateChosenShouldTakeActionTemplate());

	AIComponents.AddItem(CreateSearchingForFavoredChosenTemplate());
	AIComponents.AddItem(CreateNotSearchingForFavoredChosenTemplate());
	AIComponents.AddItem(CreateMetAllChosenTemplate());
	AIComponents.AddItem(CreateChosenAlreadyFavoredTemplate());
	AIComponents.AddItem(CreateNoChosenFavoredTemplate());
	AIComponents.AddItem(CreateSearchForFavoredTimerCompleteTemplate());
	AIComponents.AddItem(CreateStartSearchForFavoredChosenTemplate());
...
static function X2DataTemplate CreateChosenTakeActionTemplate()
{
	local X2AlienStrategyActionTemplate Template;

	Template = new class'X2AlienStrategyActionTemplate';
	Template.SetTemplateName('AlienAI_ChosenTakeAction');
	Template.PerformActionFn = ChosenTakeAction;

	// Conditions
	Template.Conditions.AddItem('AlienAICondition_ChosenShouldTakeAction');
	Template.Conditions.AddItem('AlienAICondition_NotInFlightMode');

	return Template;
}
static function ChosenTakeAction()
{
	local XComGameStateHistory History;
	local XComGameState NewGameState;
	local XComGameState_AdventChosen ChosenState;
	local XComGameState_ChosenAction ActionState;
	local int OldKnowledge, NewKnowledge;

	History = `XCOMHISTORY;

	foreach History.IterateByClassType(class'XComGameState_AdventChosen', ChosenState)
	{
		if(ChosenState.CanTakeAction())
		{
			ActionState = XComGameState_ChosenAction(History.GetGameStateForObjectID(ChosenState.CurrentMonthAction.ObjectID));
			if(ActionState.GetMyTemplate().bPauseGeoscapeBeforeActivation)
			{
				`GAME.GetGeoscape().Pause();
			}
			...
			ChosenState.KnowledgeFromLastAction = 0;
			if(ActionState.KnowledgeGain != 0)
			{
				OldKnowledge = ChosenState.GetKnowledgeScore(true);
				ChosenState.ModifyKnowledgeScore(NewGameState, ChosenState.GetScaledKnowledgeDelta(ActionState.KnowledgeGain));
				NewKnowledge = ChosenState.GetKnowledgeScore(true);
				ChosenState.KnowledgeFromLastAction = NewKnowledge - OldKnowledge;
			}
...
static function X2DataTemplate CreateChosenShouldTakeActionTemplate()
{
	local X2AlienStrategyConditionTemplate Template;

	Template = new class'X2AlienStrategyConditionTemplate';
	Template.SetTemplateName('AlienAICondition_ChosenShouldTakeAction');
	Template.IsConditionMetFn = ChosenShouldTakeAction;

	return Template;
}
static function bool ChosenShouldTakeAction()
{
	local XComGameStateHistory History;
	local XComGameState_HeadquartersAlien AlienHQ;
	local XComGameState_AdventChosen ChosenState;
...
static function X2DataTemplate CreateSearchingForFavoredChosenTemplate()
{
	local X2AlienStrategyConditionTemplate Template;

	Template = new class'X2AlienStrategyConditionTemplate';
	Template.SetTemplateName('AlienAICondition_SearchingForFavoredChosen');
	Template.IsConditionMetFn = SearchingForFavoredChosen;

	return Template;
}
static function bool SearchingForFavoredChosen()
{
	local XComGameStateHistory History;
	local XComGameState_HeadquartersAlien AlienHQ;
	...
	return AlienHQ.bSearchingForFavoredChosen;
}

static function X2DataTemplate CreateNotSearchingForFavoredChosenTemplate()
{
	local X2AlienStrategyConditionTemplate Template;

	Template = new class'X2AlienStrategyConditionTemplate';
	Template.SetTemplateName('AlienAICondition_NotSearchingForFavoredChosen');
	Template.IsConditionMetFn = NotSearchingForFavoredChosen;

	return Template;
}
static function bool NotSearchingForFavoredChosen()
{
	return !SearchingForFavoredChosen();
}

static function X2DataTemplate CreateMetAllChosenTemplate()
{
	local X2AlienStrategyConditionTemplate Template;

	Template = new class'X2AlienStrategyConditionTemplate';
	Template.SetTemplateName('AlienAICondition_MetAllChosen');
	Template.IsConditionMetFn = MetAllChosen;

	return Template;
}
static function bool MetAllChosen()
{
	local XComGameStateHistory History;
	local XComGameState_HeadquartersAlien AlienHQ;
...
static function X2DataTemplate CreateChosenAlreadyFavoredTemplate()
{
	local X2AlienStrategyConditionTemplate Template;

	Template = new class'X2AlienStrategyConditionTemplate';
	Template.SetTemplateName('AlienAICondition_ChosenAlreadyFavored');
	Template.IsConditionMetFn = ChosenAlreadyFavored;

	return Template;
}
static function bool ChosenAlreadyFavored()
{
	local XComGameStateHistory History;
	local XComGameState_HeadquartersAlien AlienHQ;
	local array<XComGameState_AdventChosen> AllChosen;
...
static function X2DataTemplate CreateNoChosenFavoredTemplate()
{
	local X2AlienStrategyConditionTemplate Template;

	Template = new class'X2AlienStrategyConditionTemplate';
	Template.SetTemplateName('AlienAICondition_NoChosenFavored');
	Template.IsConditionMetFn = NoChosenFavored;

	return Template;
}
static function bool NoChosenFavored()
{
	return !ChosenAlreadyFavored();
}

static function X2DataTemplate CreateSearchForFavoredTimerCompleteTemplate()
{
	local X2AlienStrategyConditionTemplate Template;

	Template = new class'X2AlienStrategyConditionTemplate';
	Template.SetTemplateName('AlienAICondition_SearchForFavoredTimerComplete');
	Template.IsConditionMetFn = SearchForFavoredTimerComplete;

	return Template;
}
static function bool SearchForFavoredTimerComplete()
{
	local XComGameStateHistory History;
	local XComGameState_HeadquartersAlien AlienHQ;
	...
	return (class'X2StrategyGameRulesetDataStructures'.static.LessThan(AlienHQ.StartFavoredChosenSearchTime, `STRATEGYRULES.GameTime));
}

static function X2DataTemplate CreateStartSearchForFavoredChosenTemplate()
{
	local X2AlienStrategyActionTemplate Template;

	Template = new class'X2AlienStrategyActionTemplate';
	Template.SetTemplateName('AlienAI_StartSearchForFavoredChosen');
	Template.PerformActionFn = StartSearchForFavoredChosen;

	// Conditions
	Template.Conditions.AddItem('AlienAICondition_NotSearchingForFavoredChosen');
	Template.Conditions.AddItem('AlienAICondition_SearchForFavoredTimerComplete');
	Template.Conditions.AddItem('AlienAICondition_MetAllChosen');
	Template.Conditions.AddItem('AlienAICondition_NoChosenFavored');

	return Template;
}
static function StartSearchForFavoredChosen()
{
	local XComGameStateHistory History;
	local XComGameState NewGameState;
	local XComGameState_HeadquartersAlien AlienHQ;
	...
	History = `XCOMHISTORY;
	AlienHQ = XComGameState_HeadquartersAlien(History.GetSingleGameStateObjectForClass(class'XComGameState_HeadquartersAlien'));
	NewGameState = class'XComGameStateContext_ChangeContainer'.static.CreateChangeState("AIAction: ActivateAllChosen");
	AlienHQ = XComGameState_HeadquartersAlien(NewGameState.ModifyStateObject(class'XComGameState_HeadquartersAlien', AlienHQ.ObjectID));
	AlienHQ.StartSearchForFavoredChosen();
	`XCOMGAME.GameRuleset.SubmitGameState(NewGameState);
}