...
var() const bool bTacticalTransient; // this type of gamestate should be ignored when creating an archive during the tactical->strategy transition

/// <summary>
/// Constructor for a new gamestate object from the provided template
/// </summary>
...
/// <summary>
/// Returns the immediately preceding version of this object in the history
/// </summary>
native final function XComGameState_BaseObject GetPreviousVersion(optional EGameStateReturnType DesiredReturnType=eReturnType_Reference);
...
final event BeginTacticalPlay(XComGameState NewGameState)
{
	if( !bInPlay && `TACTICALRULES != none && `TACTICALRULES.TacticalGameIsInPlay() )
	{
		if (!`ONLINEEVENTMGR.bInitiateValidationAfterLoad)
		{
			...
			OnBeginTacticalPlay(NewGameState);
		}
		bInPlay = true;
	}
}
...
final event EndTacticalPlay(XComGameState NewGameState)
{
	if( bInPlay && !`TACTICALRULES.TacticalGameIsInPlay() )
	{
		OnEndTacticalPlay(NewGameState);
		bInPlay = false;
	}
}
...
{
	return "[" $ GetMyTemplateName() $ ", " $ GetVisualizer() $ ", " $ ObjectID $ "]";
}
