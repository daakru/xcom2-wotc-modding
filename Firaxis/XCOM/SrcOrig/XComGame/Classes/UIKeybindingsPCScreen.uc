...
simulated function bool RawInputHandler(Name Key, int ActionMask, bool bCtrl, bool bAlt, bool bShift)
{
	...
	// bsg-jrebar (4/25/17): Adding item to Changed inputs list so that the keybind gets updated at bottom of function.
	else
	{
		ChangedInputs.AddItem(GetPlayerInput());
	}
	// bsg-jrebar (4/25/17): end
...
function OnDisplayConfirmResetBindingsDialogAction(Name eAction)
{
...
function OnDisplayConfirmCancelDialog(Name eAction)
{
	if (eAction == 'eUIAction_Accept')
	{
		ReloadPlayerInputBindings();
...
function OnDisplayConflictingKeyDialog(Name eAction)
{
	local PlayerInput kPlayerInput;
	local PlayerController kPlayerController;
	local KeyBind kBind;
