...
class XComStrategyMap extends Actor native(Strategy)
	config(GameData);
...
//}
//
//simulated private function ResistanceOpsAppearedCallback(Name eAction)
//{
//	`GAME.GetGeoscape().Resume();
//
...
// Geoscape visuals
// #######################################################################
event UIStrategyMapItem GetMapItemNative(XComGameState_GeoscapeEntity Entity)
{
	if (UIMap != None) //only update visuals when the map is actually being displayed
	{
		return UIMap.GetMapItem(Entity);
	}
...
{	
	if (UIMap != None) //only update visuals when the map is actually being displayed
	{		
		//Calls UpdateVisuals on the individual UIStrategyMapItems that are visible / relevant to the geoscape
		UpdateMapItemVisuals(`EARTH);
...
native function UpdateMapItemVisuals(XComEarth EarthActor);
