...
function OnSpawnComplete(const out EffectAppliedData ApplyEffectParameters, StateObjectReference NewUnitRef, XComGameState NewGameState, XComGameState_Effect NewEffectState)
{
	...
	{
		CivUnitGameState = XComGameState_Unit(NewGameState.ModifyStateObject( class'XComGameState_Unit', ApplyEffectParameters.TargetStateObjectRef.ObjectID ));
	}
										...
										XComGameState_Unit EffectTargetUnit, optional out VisualizationActionMetadata EffectTargetUnitTrack)
{
	...
	UnitChangeAction = X2Action_UnitToFacelessChangeForm(class'X2Action_UnitToFacelessChangeForm'.static.AddToVisualizationTree(EffectTargetUnitTrack, Context));
	...
	class'X2Action_WaitForAbilityEffect'.static.AddToVisualizationTree(SpawnedUnitTrack, Context);

	if (SpawnedUnit.IsUnitAffectedByEffectName(class'X2Effect_ScanningProtocol'.default.EffectName))
	{
		OutlineAction = X2Action_UpdateScanningProtocolOutline(class'X2Action_UpdateScanningProtocolOutline'.static.AddToVisualizationTree(SpawnedUnitTrack, Context));
		OutlineAction.bEnableOutline = true;
	}

	FacelessAction = X2Action_FacelessChangeForm(class'X2Action_FacelessChangeForm'.static.AddToVisualizationTree(SpawnedUnitTrack, Context));
