...
var Material		m_kAOEDamageMaterialMasked;
...
simulated function SetFOW(bool bNewFOW)
{
	...
	local X2SitRepEffect_PerfectVisibility SitRepEffect;
	...
	if(bNewFOW)
	{
		// fow is only enabled if no sitrep is currently disabling it
		History = `XCOMHISTORY;
		...
		{
			foreach class'X2SitRepTemplateManager'.static.IterateEffects(class'X2SitRepEffect_PerfectVisibility', SitRepEffect, BattleDataState.ActiveSitReps)
			{
				`XWORLD.InitializeAllViewersToHaveSeenFog(true);
				bNewFOW = false;
				break;
			}
		}
...
simulated function LoadDynamicAOEMaterials()
{
	...
	m_kAOEDamageMaterialMasked = Material(DynamicLoadObject("UI_3D.Targeting.M_HitByAOE_Masked", class'Material'));
	m_kAOEDamageMaterial_FriendlyDestructible = Material(DynamicLoadObject("UI_3D.Targeting.M_FriendlyHitByAOE", class'Material'));
}


native function MaterialInterface GetAOEMaterialToUse(MaterialInterface ParentMaterial, optional MaterialInterface OverideAOEMaterial, optional bool bUseFriendly=false);
