...
{
	return ConvertEarthToWorldNative(inCoords, bWrap);
}

static function native vector ConvertEarthToWorldNative(vector2D inCoords, optional bool bWrap = true);
...
static function Vector ConvertUVToWorld(vector inCoords)
{
	local Vector vLoc;

	vLoc.x = inCoords.X * 384.0f;
	vLoc.y = (inCoords.Y * 234.0f) - 21.0f;
