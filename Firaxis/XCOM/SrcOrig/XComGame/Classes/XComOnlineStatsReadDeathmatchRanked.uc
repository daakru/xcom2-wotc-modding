...
function UpdateToPRI(XComPlayerReplicationInfo PRI)
{
	...
	if(!GetIntStatValueForPlayer(PRI.UniqueId, STATS_COLUMN_EW_DEATHMATCHALLTIMERANKED_MATCHES_WON, PRI.m_iRankedDeathmatchMatchesWon))
	{
		`log(self $ "::" $ GetFuncName() @ PRI.PlayerName @ "Stats not found for matches won", true, 'XCom_Online');
		PRI.m_iRankedDeathmatchMatchesWon = 0;
	}
	if(!GetIntStatValueForPlayer(PRI.UniqueId, STATS_COLUMN_EW_DEATHMATCHALLTIMERANKED_MATCHES_LOST, PRI.m_iRankedDeathmatchMatchesLost))
	{
...
function int GetRankedMatchesWonForPlayer(UniqueNetId PlayerId)
{
	local int iValue;
	iValue = -1;
	if(!GetIntStatValueForPlayer(PlayerId, STATS_COLUMN_EW_DEATHMATCHALLTIMERANKED_MATCHES_WON, iValue))
	{
...
function int GetRankedMatchesLostForPlayer(UniqueNetId PlayerId)
{
	local int iValue;
	iValue = -1;
	if(!GetIntStatValueForPlayer(PlayerId, STATS_COLUMN_EW_DEATHMATCHALLTIMERANKED_MATCHES_LOST, iValue))
	{
...
function int GetDisconnectsForPlayer(UniqueNetId PlayerId)
{
	local int iValue;
	iValue = -1;
	if(!GetIntStatValueForPlayer(PlayerId, STATS_COLUMN_EW_DEATHMATCHALLTIMERANKED_MATCHES_DISCONNECTED, iValue))
	{
...
function int DidPlayerCompleteLastRankedmatch(UniqueNetId PlayerId)
{
	local int iValue;
	iValue = -1;
	if(!GetIntStatValueForPlayer(PlayerId, STATS_COLUMN_EW_DEATHMATCHALLTIMERANKED_MATCH_STARTED, iValue))
	{
...
function string ToString_ForPlayer(string strPlayerName, UniqueNetId PlayerId)
{
	...
	if(!GetIntStatValueForPlayer(PlayerId, STATS_COLUMN_EW_DEATHMATCHALLTIMERANKED_MATCHES_WON, iMatchesWon))
	{
		`log(self $ "::" $ GetFuncName() @ `ShowVar(strPlayerName) @ "Stats not found for matches won", true, 'XCom_Online');
	}
	strRep @= "MatchesWon=" $ iMatchesWon;
	if(!GetIntStatValueForPlayer(PlayerId, STATS_COLUMN_EW_DEATHMATCHALLTIMERANKED_MATCHES_LOST, iMatchesLost))
	{
	...
	if(!GetIntStatValueForPlayer(PlayerId, STATS_COLUMN_EW_DEATHMATCHALLTIMERANKED_MATCHES_DISCONNECTED, iMatchesDisconnected))
	{
	...
	if(!GetIntStatValueForPlayer(PlayerId, STATS_COLUMN_EW_DEATHMATCHALLTIMERANKED_MATCH_STARTED, iLastMatchStarted))
	{
...
defaultproperties
{
	...
	ViewId= STATS_VIEW_EW_DEATHMATCHALLTIMERANKED

	// the stats we read -tsmith 
	ColumnIds(0)=STATS_COLUMN_EW_DEATHMATCHALLTIMERANKED_MATCHES_WON
	ColumnIds(1)=STATS_COLUMN_EW_DEATHMATCHALLTIMERANKED_MATCHES_LOST
	ColumnIds(2)=STATS_COLUMN_EW_DEATHMATCHALLTIMERANKED_MATCHES_DISCONNECTED
	ColumnIds(3)=STATS_COLUMN_EW_DEATHMATCHALLTIMERANKED_MATCH_STARTED

	// The metadata for the columns
	ColumnMappings(0)=(Id=STATS_COLUMN_EW_DEATHMATCHALLTIMERANKED_MATCHES_WON,Name="MatchesWon")
	ColumnMappings(1)=(Id=STATS_COLUMN_EW_DEATHMATCHALLTIMERANKED_MATCHES_LOST,Name="MatchesLost")
	ColumnMappings(2)=(Id=STATS_COLUMN_EW_DEATHMATCHALLTIMERANKED_MATCHES_DISCONNECTED,Name="MatchesDisconnected")
	ColumnMappings(3)=(Id=STATS_COLUMN_EW_DEATHMATCHALLTIMERANKED_MATCH_STARTED,Name="MatchStarted")
}
