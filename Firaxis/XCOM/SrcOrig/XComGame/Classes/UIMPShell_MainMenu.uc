...
var localized string m_strChallengeModeButtonText;
var localized string m_strMPHeaderText;
var localized string m_strCancel;
var localized string m_strRankedDisabled;
var localized string m_strDisableModsForRanked;
var localized string m_strDisableDLCForRanked;
var localized string m_strDisableGameDataForRanked;
var localized string m_strDisableConsoleForRanked;
...
var int     m_iAllRankedGameDataValid; // -1 not checked, 0 not valid, 1 valid
...
simulated function BuildMenu()
{
	...
	/*m_optChallengeMode = iCurrent++;
	UIListItemString(List.CreateItem()).InitListItem(m_strChallengeModeButtonText);*/
...
{	
	...
	case m_optLeaderboards:
		...
		break;
	/*case m_optChallengeMode:
		ChallengeModeButtonCallback();
		break;*/
...
function ChallengeModeButtonCallback()
{
	`log(`location,, 'uixcom_mp');
...
{
	local string DisabledRankedText;
	...
	`log("UpdateConnectivityDependentButtons" @ `ShowVar(m_iAllRankedGameDataValid), , 'XCom_Online');
	/*if (m_iAllRankedGameDataValid == -1)
	{*/
		m_iAllRankedGameDataValid = class'Helpers'.static.NetAllRankedGameDataValid() ? 1 : 0;
		...
		m_iAllRankedGameDataValid = (!bAreModsInstalled && bAreInstalledDLCsRankedFriendly && bGameDataHashesAreValid && !bIsDevConsoleEnabled) ? 1 : 0;

		`log("UpdateConnectivityDependentButtons" @ `ShowVar(m_iAllRankedGameDataValid) @ `ShowVar(bAreModsInstalled) @ `ShowVar(bAreInstalledDLCsRankedFriendly) @ `ShowVar(bGameDataHashesAreValid) @ `ShowVar(bIsDevConsoleEnabled), , 'XCom_Online');

		if (m_iAllRankedGameDataValid == 0)
		{
			if (bAreModsInstalled)
			{
				DisabledRankedText $= m_strDisableModsForRanked;
			}
			if (!bAreInstalledDLCsRankedFriendly)
			{
				DisabledRankedText $= ((Len(DisabledRankedText) > 0) ? ", " : "");
				DisabledRankedText $= m_strDisableDLCForRanked;
			}
			if (!bGameDataHashesAreValid)
			{
				DisabledRankedText $= ((Len(DisabledRankedText) > 0) ? ", " : "");
				DisabledRankedText $= m_strDisableGameDataForRanked;
			}
			if (bIsDevConsoleEnabled)
			{
				DisabledRankedText $= ((Len(DisabledRankedText) > 0) ? ", " : "");
				DisabledRankedText $= m_strDisableConsoleForRanked;
			}
			DisabledRankedText = m_strRankedDisabled @ DisabledRankedText;
			`log("UpdateConnectivityDependentButtons" @ `ShowVar(DisabledRankedText), , 'XCom_Online');
			UIListItemString(List.GetItem(m_optRanked)).SetTooltipText(DisabledRankedText);
		}
	//}
	...
	//UIListItemString(List.GetItem(m_optChallengeMode)).EnableListItem();
	...
	if(m_kMPShellManager.m_bPassedNetworkConnectivityCheck)
	{
		...
		{
			UIListItemString(List.GetItem(m_optRanked)).DisableListItem(DisabledRankedText);
			UIListItemString(List.GetItem(m_optCasual)).DisableListItem();
			UIListItemString(List.GetItem(m_optLeaderboards)).DisableListItem();
			//UIListItemString(List.GetItem(m_optChallengeMode)).DisableListItem();
		}
		else if(m_iAllRankedGameDataValid == 0)
		{
			UIListItemString(List.GetItem(m_optRanked)).DisableListItem(DisabledRankedText);
		}
	}
