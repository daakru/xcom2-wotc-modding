...
public function Update( optional bool bForceUpdate = false )
{
	...
	else if( bForceUpdate || (kActiveUnit != none && kActiveUnit != m_kCurrentUnit) )
	{
		...
		{
			m_kWeapon.SetWeaponAndAmmo(kPrimaryWeapon, kGameStateUnit);
			AS_SetWeaponName(class'UIUtilities_Text'.static.CapsCheckForGermanScharfesS(kPrimaryWeapon.GetMyTemplate().GetItemFriendlyName(kPrimaryWeapon.ObjectID)), kGameStateUnit.ObjectID);
			Show();
		}
		else
		{
			AS_SetWeaponName("", kGameStateUnit.ObjectID);
			Hide();
		}
	}
...
simulated function AS_SetWeaponName( string displayString, int soldierID)
{
