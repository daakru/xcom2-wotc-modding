...
event OnAnimNotifyCinescript(string EventLabel)
{
	...
	PC = XComTacticalController(GetALocalPlayerController());

	//Disallow cinescript camera cuts while the shot HUD is up. 
	//Ozzy bug push special case. If additional screens need this, a more general method should be devised.	
	if (PC.m_bInputInShotHUD == false)
	{
