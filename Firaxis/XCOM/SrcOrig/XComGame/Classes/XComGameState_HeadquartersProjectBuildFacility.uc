...
function SetProjectFocus(StateObjectReference FocusRef, optional XComGameState NewGameState, optional StateObjectReference AuxRef)
{
	...
	InitialProjectPoints = FacilityState.GetMyTemplate().GetPointsToComplete();
	ProjectPointsRemaining = Round(float(FacilityState.GetMyTemplate().GetPointsToComplete()) * `ScaleStrategyArrayFloat(class'X2StrategyGameRulesetDataStructures'.default.BuildFacilityProject_TimeScalar));
...
function int CalculateWorkPerHour(optional XComGameState StartState = none, optional bool bAssumeActive = false)
{
	...
	if(Room != none)
	{
		...
		if(SlotState != none)
		{
			if(bHaveState)
			{
				Builder = XComGameState_Unit(StartState.GetGameStateForObjectID(SlotState.AssignedStaff.UnitRef.ObjectID));
			}

			if(Builder == none)
			{
				Builder = XComGameState_Unit(History.GetGameStateForObjectID(SlotState.AssignedStaff.UnitRef.ObjectID));
			}
	...
	return Round(iTotalWork * ResHQ.GetFacilityBuildScalar());
}
