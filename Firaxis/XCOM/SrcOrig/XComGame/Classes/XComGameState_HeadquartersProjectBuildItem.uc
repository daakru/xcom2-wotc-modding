...
function SetProjectFocus(StateObjectReference FocusRef, optional XComGameState NewGameState, optional StateObjectReference AuxRef)
{
	...
	InitialProjectPoints = ItemState.GetMyTemplate().GetPointsToComplete();
	bInstant = (InitialProjectPoints == 0);
	ProjectPointsRemaining = Round(float(ItemState.GetMyTemplate().GetPointsToComplete()) * `ScaleStrategyArrayFloat(class'X2StrategyGameRulesetDataStructures'.default.BuildItemProject_TimeScalar));
