...
simulated function InitMemorial(StateObjectReference UnitRef)
{
	...
	else
	{
		...
		SoldierPicture = `XENGINE.m_kPhotoManager.GetHeadshotTexture(SettingsState.GameIndex, UnitReference.ObjectID, 512, 512);
...
simulated function SetEpitaphText(string newText)
{
	...
	UnitChange = XComGameState_Unit(NewGameState.ModifyStateObject(class'XComGameState_Unit', UnitReference.ObjectID));
...
simulated function PopulateStatsData()
{
	...
	AStat.Value = Analytics.GetUnitValueAsString( "ACC_UNIT_SUCCESSFUL_ATTACKS", UnitReference );
	UnitStats.AddItem( AStat );

	AStat.Label = m_statLabels[ 6 ];
	AStat.Value = Analytics.GetUnitValueAsString( "ACC_UNIT_DEALT_DAMAGE", UnitReference );
	UnitStats.AddItem( AStat );

	AStat.Label = m_statLabels[ 7 ];
	AStat.Value = Analytics.GetUnitValueAsString( "ACC_UNIT_ABILITIES_RECIEVED", UnitReference );
	UnitStats.AddItem( AStat );
