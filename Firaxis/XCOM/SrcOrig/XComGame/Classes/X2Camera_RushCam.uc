...
// context of the ability that this camera should be following
...
var private Vector UnitDestination; // Cached location that the unit is running to
var private TPOV CachedCameraLocation; // cached locaiton of our camera, so we only compute it once in the update

var private float TargetOffset; // If the rush cam would be blocked, this will be filled out with a nearby unblocked location to move the camera to
var private float CurrentOffset; // If TargetOffset is not 0, this is the current interpolated offset. Prevents jarring cuts when the camera becomes blocked.

// Determines if anything the camera cares about is blocking the los between the specified point.
// This can probably be replaced with the common IsLineOfSightBlockedToLocation() function in the base class.
...
function Added()
{
	...
	// Determine who we are following, and where they are going
	History = `XCOMHISTORY;
...
function UpdateCamera(float DeltaTime)
{
	...
	// Interpolate to the unblocked offset
