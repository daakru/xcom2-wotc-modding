...
// Item to reward if this objective is completed with Sim Combat
var Name					SimCombatItemReward;
...
// Call during Sim Combat to check if this objective should be completed (for tactical completion objectives, defaults to true)
var Delegate<ShouldCompleteWithSimCombatDelegate> ShouldCompleteWithSimCombatFn;
...
delegate bool ShouldCompleteWithSimCombatDelegate();
...
function XComGameState_Objective CreateInstanceFromTemplate(XComGameState NewGameState)
{
	...
	ObjectiveState = XComGameState_Objective(NewGameState.CreateNewStateObject(class'XComGameState_Objective', self));
