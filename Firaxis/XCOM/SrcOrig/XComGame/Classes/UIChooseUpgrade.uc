...
simulated function bool CanBuildUpgrade(X2FacilityUpgradeTemplate UpgradeTemplate)
{
	...
	local int FacilityDiscount;

	XComHQ = class'UIUtilities_Strategy'.static.GetXComHQ();
	FacilityDiscount = XComHQ.GetPendingFacilityDiscountPercent( UpgradeTemplate.DataName );

	return XComHQ.MeetsRequirmentsAndCanAffordCost(UpgradeTemplate.Requirements, UpgradeTemplate.Cost, XComHQ.FacilityUpgradeCostScalars, FacilityDiscount);
}
...
function int GetPowerRequirement(X2FacilityUpgradeTemplate UpgradeTemplate)
{
	...
	if (FacilityState.BuiltOnPowerCell())
	{
		...
		{
			// If the upgrade provdes power, show the power value
			power = UpgradeTemplate.iPower;
		}
		else
		{
			// No power cost when building on a coil
			power = 0;
		}
	}
	else
...
simulated function UpdateSelection(UIList list, int itemIndex)
{
	...
	XComHQ = `XCOMHQ;
	facilitydiscount = XComHQ.GetPendingFacilityDiscountPercent( m_selectedUpgrade.DataName );
	Requirements $= class'UIUtilities_Strategy'.static.GetStrategyCostString(m_selectedUpgrade.Cost, XComHQ.FacilityBuildCostScalars, facilitydiscount);
...
simulated function StartUpgradeFacility()
{
	...
	FacilityDiscount = XComHQ.GetPendingFacilityDiscountPercent( UpgradeTemplate.DataName );
	XComHQ.RemovePendingFacilityDiscountPercent( UpgradeTemplate.DataName );

	XComHQ.PayStrategyCost(NewGameState, UpgradeState.GetMyTemplate().Cost, XComHQ.FacilityUpgradeCostScalars, FacilityDiscount);
			
	FacilityState = XComGameState_FacilityXCom(NewGameState.ModifyStateObject(class'XComGameState_FacilityXCom', m_FacilityRef.ObjectID));
...
DefaultProperties
{
	...
	InputState = eInputState_Consume; //bsg-crobinson (5.17.17): Allow this state to consume input so user cant cycle through facility list
}
