...
event Activated()
{
	...
	foreach GetWorldInfo().AllControllers(class'XComTacticalController', kTacticalController)
	{
		...
		if( kUnit == none )
		{
			XComPresentationLayer(kTacticalController.Pres).QueueWorldMessage(class'UIUtilities_Input'.static.InsertGamepadIcons(Message), vLocation,,,INT(Steady),MessageBoxIdentifier,,,,DisplayTime);
		}
		else
		{ 
			XComPresentationLayer(kTacticalController.Pres).QueueWorldMessage(class'UIUtilities_Input'.static.InsertGamepadIcons(Message), kUnit.GetLocation(), kUnit.GetVisualizedStateReference() ,,INT(Steady),MessageBoxIdentifier,,,,DisplayTime);
		}
		break;
	}
}
