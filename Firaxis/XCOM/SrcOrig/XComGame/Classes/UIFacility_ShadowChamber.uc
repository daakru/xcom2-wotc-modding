...
function TriggerResearchReport()
{
	...
	for(idx = 0; idx < TechRefs.Length; idx++)
	{
		...
		{
			`HQPRES.ShadowChamberResearchReportPopup(TechRefs[idx]);
		}
	}
}
...
simulated function RealizeNavHelp()
{
	...
	if(XComHQ.HasActiveShadowProject())
	{
		if( `ISCONTROLLERACTIVE == false )
			NavHelp.AddRightHelp(m_strPauseResearch, class'UIUtilities_Input'.static.GetAdvanceButtonIcon(), class'XComHQPresentationLayer'.static.PauseShadowProjectPopup);
