...
function SetProjectFocus(StateObjectReference FocusRef, optional XComGameState NewGameState, optional StateObjectReference AuxRef)
{
	...
	InitialProjectPoints = UpgradeState.GetMyTemplate().GetPointsToComplete();
	bInstant = (InitialProjectPoints == 0);
	ProjectPointsRemaining = Round(float(UpgradeState.GetMyTemplate().GetPointsToComplete()) * `ScaleStrategyArrayFloat(class'X2StrategyGameRulesetDataStructures'.default.UpgradeFacilityProject_TimeScalar));
