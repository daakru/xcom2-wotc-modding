...
event Activated()
{
	...
	ObjectiveList = XComGameState_ObjectivesList(NewGameState.ModifyStateObject(class'XComGameState_ObjectivesList', 
	...
	{
		if(!ObjectiveList.GetObjectiveDisplay(MissionType, DisplayLabel, DisplayInfo, TextPoolIndex, LineIndex, GroupID))
		{
			// there is no current line for this mission text yet, so add a new one
			DisplayInfo.MissionType = MissionType;			
		}

		// Has this objective previously been updated?
		// We just got (or created a new) DisplayInfo, so check how the ShowCompleted and ShowFailed are currently set.
		bPreviouslySet = DisplayInfo.ShowCompleted || DisplayInfo.ShowFailed;

		// update the status
		DisplayInfo.DisplayLabel = DisplayLabel;		//	always update in case language has changed
		...
		DisplayInfo.TextPoolIndex = TextPoolIndex;
		DisplayInfo.LineIndex = LineIndex;
