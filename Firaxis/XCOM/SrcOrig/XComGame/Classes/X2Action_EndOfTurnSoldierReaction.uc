...
static function bool AddReactionToBlock(XComGameStateContext_TacticalGameRule Context)
{
	...
	{
		if(UnitIterator.GetTeam() == eTeam_XCom && UnitIterator.IsAlive() && !UnitIterator.IsBleedingOut() )
	...
	ActionMetadata.StateObject_OldState = SelectedUnit;
	ActionMetadata.StateObject_NewState = SelectedUnit;
	ActionMetadata.VisualizeActor = SelectedUnit.GetVisualizer();
	Action = X2Action_EndOfTurnSoldierReaction(class'X2Action_EndOfTurnSoldierReaction'.static.AddToVisualizationTree(ActionMetadata, Context, false, ActionMetadata.LastActionAdded));
