...
function OnBeginTacticalPlay(XComGameState NewGameState)
{
	...
	{
		RemainingMissionCooldown = RemainingMissionCooldown - 1;
	}
}
...
private function BuildVisualization(XComGameState VisualizeGameState)
{
	...
	{
		ActionMetadata.StateObject_OldState = NarrativeActorState;
		ActionMetadata.StateObject_NewState = NarrativeActorState;
		class'X2Action_PlayWorldNarrative'.static.AddToVisualizationTree(ActionMetadata, VisualizeGameState.GetContext(), false, ActionMetadata.LastActionAdded);
			}
}
...
private function EventListenerReturn OnPlayerTurnBegun(Object EventData, Object EventSource, XComGameState GameState, Name Event, Object CallbackData)
{
	...
	if(XComGameState_Player(EventSource).GetTeam() == eTeam_XCom && RemainingTurnCooldown > 0)
	{
		...
		NewTrackerState = XComGameState_WorldNarrativeTracker(NewGameState.ModifyStateObject(class'XComGameState_WorldNarrativeTracker', ObjectID));
...
private function EventListenerReturn OnUnitMoveFinished(Object EventData, Object EventSource, XComGameState GameState, Name Event, Object CallbackData)
{
	...
	if(BestNarrativeActorState != none) // we have selected a narrative moment to play
	{
		...
		NarrativeActorState = XComGameState_WorldNarrativeActor(NewGameState.ModifyStateObject(class'XComGameState_WorldNarrativeActor', BestNarrativeActorState.ObjectID));
