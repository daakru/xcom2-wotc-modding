...
simulated function UpdateNavHelp()
{
	...
	if(`ISCONTROLLERACTIVE)
	{
		...
		NavHelp.AddCenterHelp(m_strTabNavHelp, class'UIUtilities_Input'.static.GetGamepadIconPrefix() $ class'UIUtilities_Input'.const.ICON_LBRB_L1R1); // bsg-jrebar (5/23/17) Removed button inlining
	}
}
...
simulated function ManufactureItem(UIList ContainerList, int ItemIndex)
{
	...
	if (CanBuildItem(ItemTemplate))
	{
		...
		if (ItemTemplate.GetPointsToComplete() > 0)
		{
			// If the item has a build time, create a project for it
			FacilityState = XComGameState_FacilityXCom(NewGameState.ModifyStateObject(class'XComGameState_FacilityXCom',
