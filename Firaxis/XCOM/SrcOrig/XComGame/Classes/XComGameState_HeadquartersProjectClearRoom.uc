...
function SetProjectFocus(StateObjectReference FocusRef, optional XComGameState NewGameState, optional StateObjectReference AuxRef)
{
	...
	InitialProjectPoints = Room.GetSpecialFeature().GetPointsToComplete() * Room.BuildSlots.Length;

	if (Room.GridRow == 0) // If in the first row, apply an additional time scalar
	{
		InitialProjectPoints *= `ScaleStrategyArrayFloat(class'X2StrategyGameRulesetDataStructures'.default.ClearRoomProjectFirstRow_TimeScalar);
	}

	ProjectPointsRemaining = Round(float(InitialProjectPoints) * `ScaleStrategyArrayFloat(class'X2StrategyGameRulesetDataStructures'.default.ClearRoomProject_TimeScalar));
...
function int CalculateWorkPerHour(optional XComGameState StartState = none, optional bool bAssumeActive = false)
{
	...
	return Round(float(ConstructionRate * NumBuildSlots) * ResHQ.GetExcavateScalar());
}
