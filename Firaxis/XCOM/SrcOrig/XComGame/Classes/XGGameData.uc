...
struct native SpawnDistributionListEntry
{
	var name Template;
	var int MinForceLevel; 
	var int MaxForceLevel; 
	var int MaxCharactersPerGroup;
	...
	structcpptext
	{
		FSpawnDistributionListEntry()
		{
			appMemzero(this, sizeof(FSpawnDistributionListEntry));
			MinForceLevel=-1;
			MaxForceLevel=-1;
			SpawnWeight=1;
		}

		FSpawnDistributionListEntry(EEventParm)
		{
			appMemzero(this, sizeof(FSpawnDistributionListEntry));
		}
	}
...
struct native SpawnDistributionList
{
	// The name of this list
	var name				ListID;

	// The list of names to be Included or Excluded
	var array<SpawnDistributionListEntry> SpawnDistribution;

	structcpptext
	{
		FLOAT GetSpawnWeight(FName CharacterTemplateName) const;
		INT GetMaxCharactersPerGroup(const class UX2CharacterTemplate& CharacterTemplate) const;
	}
};
...
struct native MissionSchedule
{
	...
	// If the encounter doesn't specify a leader list, this is the encounter list that will be used
	var name DefaultEncounterLeaderSpawnList;

	// If the encounter doesn't specify a follower list, this is the encounter list that will be used
	var name DefaultEncounterFollowerSpawnList;

	// Spawn list that will be used for selecting turrets
	var name TurretSpawnList;

	// Spawn list that will be used for selecting civilians
	var name CivilianSpawnList;
	...
	structdefaultproperties
	{
		...
		TurretSpawnList = "DefaultTurrets"
		CivilianSpawnList = "DefaultCivilians"
	}
};
...
struct native MissionIntroDefinition
{
	...
	var bool bSilenceMissionBriefingAtStart; //If true, the skyranger briefing will be stopped when the intro starts ( prevents VO collisions )
};
...
struct native PlotTypeDefinition
{
	...
	var bool OverrideDefaultMissionIntro; // if true, will override the default and mission specific intros with MissionIntroOverride
	var MissionIntroDefinition MissionIntroOverride; 
	var string FlashlightPSOverride;
	var bool FlashlightsIgnoreTOD;

	var string ATTMatineeOverride;

	var array<name> ForcedSitReps; // Some plot types force on certain SitReps (e.g. The Lost, or Low Light)
...
// Also update arrMPMapFriendlyNames array in the localization files
...
struct native PlotDefinition
{
	...
	var bool OverrideDefaultMissionIntro; // if true, will override the default, mission and plot type specific intros with MissionIntroOverride
	var MissionIntroDefinition MissionIntroOverride; 
...
struct native GeneratedMissionData
{
	...
	var array<name>             SitReps; // names of the sitrep templates in use for this mission
