...
var native private array_mirror VisibilityFrames{ TArray<FVisibilityInfoFrame *> };
...
// this is a thread safe variable set if nullptr in the GetFunction
var private{private} GameRulesCache_VisibilityInfo X2ConditionCachedVisInfo;
...
{	
	...
	const FVisibilityInfoFrame* GetInfoFrameInternal(INT HistoryIndex = -1, INT *InfoFrameIndex = NULL) const;

	FGameRulesCache_VisibilityInfo *GetX2ConditionCachedVisInfo() const;
	void SetX2ConditionCachedVisInfo(const FGameRulesCache_VisibilityInfo *);
...
function RegisterForNewGameStateEvent()
{
	...
	ActorVisibilityMgr.RegisterEvent();
...
function UnRegisterForNewGameStateEvent()
{
	...
	ActorVisibilityMgr.UnregisterEvents();
}
...
// <summary>
// This is a specialized function that looks for units that would be visible to a source unit, if that source unit were at a remote location
// (SourceTileLocation).  Does not assume all sight ranges are equal, and ignores the Visibility cache, since the source unit is not at that tile.
native function GetAllVisibleUnitsToSource_Remote(int SourceStateObjectID,
											 TTile SourceTileLocation,
											 out array<GameRulesCache_VisibilityInfo> VisibleToSource,
											 optional int StartAtHistoryIndex = -1,
											 optional array<X2Condition> RequiredConditions,
											 optional ETeam Team);
...
/// exists, FALSE if not.
/// </summary>
native function bool GetVisibilityInfoFromRemoteLocation(int SourceStateObjectID, const out TTile RemoteTile, int TargetStateObjectID, out GameRulesCache_VisibilityInfo OutVisInfo, optional int HistoryIndex = -1);

native function bool EvaluateConditions(const out GameRulesCache_VisibilityInfo VisInfo, int HistoryIndex, array<X2Condition> RequiredConditions);
...
/// Returns information about the optimal configuration for Source to see Target
/// </summary>
native function bool GetDirectionInfoForTargetEx(XComGameState_Unit Source,
												 XComGameState_Unit Target,
												 out GameRulesCache_VisibilityInfo VisInfo,
												 out int Direction,
												 out UnitPeekSide PeekSide,
												 out int bCanSeeDefaultFromDefault,
												 out int bRequiresLean,
