...
function XComGameState_Unit SpawnUnit(optional XComGameState GameState)
{
	...
	local bool bGeneratingNewGameState;
	...
	else if( SpawnedUnit.IsChosen() )
	{
		if( GameState == None )
		{
			GameState = class'XComGameStateContext_ChangeContainer'.static.CreateChangeState("Spawning new unit via Cinematic Spawn");
			bGeneratingNewGameState = true;
		}

		SpawnedUnit = XComGameState_Unit(GameState.ModifyStateObject(class'XComGameState_Unit', SpawnedUnit.ObjectID));
		NewGroupState = XComGameState_AIGroup(GameState.ModifyStateObject(class'XComGameState_AIGroup', SpawnedUnit.GetGroupMembership(GameState).ObjectID));
		NewGroupState.bSummoningSicknessCleared = true;
		Rules.RemoveGroupFromInitiativeOrder(NewGroupState, GameState);
		SpawnedUnit.SetUnitFloatValue(class'XComGameState_AdventChosen'.default.ChosenActivatedStateUnitValue, class'XComGameState_AdventChosen'.const.CHOSEN_STATE_ENGAGED);

		if( bGeneratingNewGameState )
		{
			Rules.SubmitGameState(GameState);
		}
	}
...
defaultproperties
{
	...
	Begin Object Class=SpriteComponent Name=Sprite
		Sprite=Texture2D'LayerIcons.Editor.cinematic_spawn'
		HiddenGame=True
