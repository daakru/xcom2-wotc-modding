...
simulated state Executing
{
	...
	{	
		...
		if (syncWithOtherAction)
		{
			...
			{
				`XWORLD.UpdateVolumeEffects_PartialUpdate(XComGameState_WorldEffectTileData(Metadata.StateObject_NewState), PendingTiles, true, FireParticleSystems, bCenterTile);
		...
		else if (endSync)
		{
			...
			`XWORLD.UpdateVolumeEffects_PartialUpdate(XComGameState_WorldEffectTileData(Metadata.StateObject_NewState), ActivatedTiles, false, FireParticleSystems, bCenterTile);
			CompleteAction();
		}
		else
		{
			`XWORLD.UpdateVolumeEffects(XComGameState_WorldEffectTileData(Metadata.StateObject_NewState), FireParticleSystems, bCenterTile);		
			CompleteAction();
		}
	}

Begin:
	//`XWORLD.UpdateVolumeEffects(XComGameState_WorldEffectTileData(Metadata.StateObject_NewState), FireParticleSystems, false);
	//CompleteAction();
}

defaultproperties
{
	InputEventIDs.Add( "Visualizer_TileData" )
	syncWithOtherAction = false;
	endSync = false;
	bCenterTile = false;
}
