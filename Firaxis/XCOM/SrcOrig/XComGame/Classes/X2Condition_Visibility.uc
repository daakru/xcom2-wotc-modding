...
var(VisibilityType) bool bRequireLOS;
var(VisibilityType) bool bRequireBasicVisibility; //LOS + in range
var(VisibilityType) bool bRequireGameplayVisible; //LOS + in range + meets situational conditions in interface method UpdateGameplayVisibility
var(VisibilityType) bool bAllowSquadsight;        //LOS + any squadmate can see the target, if unit has Squadsight (overrides bRequireGameplayVisible)
var(VisibilityType) bool bActAsSquadsight;        //LOS + any squadmate can see the target, regardless of if the unit has Squadsight (overrides bRequireGameplayVisible)
var(VisibilityType) bool bVisibleToAnyAlly;       //any squadmate can see the target, regardless of if the unit has Squadsight (overrides bRequireGameplayVisible)
var(VisibilityType) bool bDisablePeeksOnMovement; //If TRUE, peeks cannot be used if bTargetMoved is true in the vis info
var(VisibilityType) bool bExcludeGameplayVisible; //condition will FAIL if there is GameplayVisibility FROM the target TO the source
