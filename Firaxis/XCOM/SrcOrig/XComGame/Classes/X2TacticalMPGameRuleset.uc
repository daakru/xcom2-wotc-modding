...
//******** Debug Variables *******************************
var array<string> WaitingLocations;
//********************************************************
...
struct MPPlayerGroup
{
	var ETeam GroupTeam;
	var XComGameState_AIGroup PlayerGroup;
};

function string DEBUG_SetLocationString(string LocationString)
{
	WaitingLocations.AddItem("[H-IDX:"$ CachedHistory.GetCurrentHistoryIndex() $"][C-OID:" $ GetCachedUnitActionPlayerRef().ObjectID $ "]" @ LocationString);
	if (WaitingLocations.Length >= 255)
	{
		WaitingLocations.Remove(0, 128);
	}
...
function EndReplay()
{
	`RedScreen(`location @ " - Is called while not in the PerformingReplay state! BUG @ttalley");
}
...
{
	local XComGameState_BaseObject UnitActionObject;
	UnitActionObject = CachedHistory.GetGameStateForObjectID(GetCachedUnitActionPlayerRef().ObjectID);
	return GameStatePlayerOrGroupIsRemote(UnitActionObject);
}

// Turns are aligned [Player A] [Player A's Group 1] [Player A's Group 2] ... [Player A's Group N] [Player B] [Player B's Group 1], etc.
// This is only really useful for determining the remote status of turn 0, since a specific player's full turn spans multiple turn orders.
simulated function bool IsTurnRemote(int TurnNum)
{
	local XComGameState_BaseObject TurnOrderObject;
	local XComGameState_BattleDataMP BattleData;
	...
	TurnOrderObject = CachedHistory.GetGameStateForObjectID(BattleData.PlayerTurnOrder[TurnNum].ObjectID);
	return GameStatePlayerOrGroupIsRemote(TurnOrderObject);
}

simulated function bool GameStatePlayerOrGroupIsRemote(XComGameState_BaseObject GameStateObject)
{
	...
	PlayerState = XComGameState_Player(GameStateObject);
	if (PlayerState == None)
	{
		PlayerGroup = XComGameState_AIGroup(GameStateObject);
		if (PlayerGroup == None)
		{
			`log(`location @ "WARNING! Invalid PlayerState/PlayerGroup!!" @ `ShowVar(GameStateObject.ObjectID, ObjectID));
			`assert(false);
			return true;
		}
		else
		{
			foreach CachedHistory.IterateByClassType(class'XComGameState_Player', PlayerState, eReturnType_Reference)
			{
				if (PlayerState.GetTeam() == PlayerGroup.TeamName)
				{
					break;
				}
			}
...
simulated function bool IsNextInitiativeGroupRemote()
{
	local XComGameState_BaseObject CheckRemoteObject;
	local XComGameState_BattleData BattleData;
	local int NextUnitActionInitiativeIndex;

	BattleData = XComGameState_BattleData(CachedHistory.GetGameStateForObjectID(CachedBattleDataRef.ObjectID));

	NextUnitActionInitiativeIndex = UnitActionInitiativeIndex + 1;
	if (NextUnitActionInitiativeIndex < 0 || NextUnitActionInitiativeIndex >= BattleData.PlayerTurnOrder.Length)
	{
		NextUnitActionInitiativeIndex = 0;
	}

	if (BattleData.InterruptingGroupRef.ObjectID > 0)
	{
		CheckRemoteObject = CachedHistory.GetGameStateForObjectID(BattleData.InterruptingGroupRef.ObjectID);
	}
	else
	{
		CheckRemoteObject = CachedHistory.GetGameStateForObjectID(BattleData.PlayerTurnOrder[NextUnitActionInitiativeIndex].ObjectID);
	}
	return GameStatePlayerOrGroupIsRemote(CheckRemoteObject);
}

simulated function bool UnitActionPlayerIsAI()
{
	// This will need to be properly implemented if we need an AI turn in MP. Until then, assuming all players are Human.
	return false;
}
...
simulated function string GetStateDebugString()
{
	...
	DebugString $= GetReplayMgr().GetStateDebugString();
	DebugString $= "\n\n";

	DebugString $= "    UnitActionInitiativeIndex:" @ UnitActionInitiativeIndex;
	DebugString $= "    CachedUnitActionInitiativeRef:" @ CachedUnitActionInitiativeRef.ObjectID;
	DebugString $= "    CachedUnitActionPlayerRef:" @ CachedUnitActionPlayerRef.ObjectID;
	DebugString $= "    InterruptingUnitActionPlayerRef:" @ InterruptingUnitActionPlayerRef.ObjectID;
	DebugString $= "    UnitActionUnitRef:" @ UnitActionUnitRef.ObjectID;
	DebugString $= "\n\n";

	DebugString $= "    WaitingForNewStatesTime:" @ WaitingForNewStatesTime;
	DebugString $= "    bLoadingSavedGame:" @ bLoadingSavedGame;
	DebugString $= "    bSkipAutosaveAfterLoad:" @ bSkipAutosaveAfterLoad;
	DebugString $= "    bSkipRemainingTurnActivty:" @ bSkipRemainingTurnActivty;
	DebugString $= "\n\n";
	...
	if( PlayerState != none )
	{
		PlayerVisualizer = PlayerState.GetVisualizer();
		DebugString @= string(PlayerVisualizer) @ " (" @ PlayerState.ObjectID @ ") (" @ (XGPlayer(PlayerVisualizer).IsRemote() ? "REMOTE" : "") @ ")\n";
	}
	else
	{
	...
	for (i = 0; i < BattleData.PlayerTurnOrder.Length; ++i)
	{
		DebugString $= "Player(" $ i $ "): " $ (IsTurnRemote(i) ? "REMOTE" : "LOCAL") @ "(" $ BattleData.PlayerTurnOrder[i].ObjectID $ ")    ";
	}
	DebugString $= "\n\n";
	...
	DebugString $= "Waiting Location History:\n";

	for (i = WaitingLocations.Length - 1; i > WaitingLocations.Length - 20 && i >= 0; --i)
	{
		DebugString $= "  ("$i$") --> " $ WaitingLocations[i] $ "\n";
	}
...
function StartStatePositionUnits()
{
	...
	if( StartState != none )
	{
		...
		{
			if (PlayerState == none || PlayerState.TeamFlag == eTeam_XCom || PlayerState.TeamFlag == eTeam_Alien || PlayerState.TeamFlag == eTeam_Neutral || PlayerState.TeamFlag == eTeam_Resistance)
			{
...
function FailsafeCheckForTacticalGameEnded()
{
	...
	// defer this check if latent game state submission is still underway
	if (!BuildingLatentGameState && !HasTacticalGameEnded())
	{
...
simulated function StartStateInitializeSquads(XComGameState StartState)
{
	local XComGameState_Unit UnitState;
	local XComGameState_AIGroup PlayerGroup;
	local array<MPPlayerGroup> Groups;
	local int GroupIdx;

	foreach StartState.IterateByClassType(class'XComGameState_Unit', UnitState)
	{
		for (GroupIdx = 0; GroupIdx < Groups.Length; ++GroupIdx)
		{
			if (Groups[GroupIdx].GroupTeam == UnitState.GetTeam())
			{
				break;
			}
		}
		if (GroupIdx >= Groups.Length)
		{
			...
			Groups[GroupIdx].GroupTeam = UnitState.GetTeam();
			Groups[GroupIdx].PlayerGroup = PlayerGroup;
		}
		Groups[GroupIdx].PlayerGroup.AddUnitToGroup(UnitState.ObjectID, StartState);
	}
}
...
simulated state TurnPhase_MPGameStart
{
	...
	simulated function EndPhase()
	{
		...
		if( !(NetworkMgr.bPauseGameStateSending && IsTurnRemote(0)) )
		{
...
simulated state TurnPhase_BeginPlayerTurn extends TurnPhase_UnitActions
{
	...
	simulated function string GetStateDebugString()
	{
		...
		DebugString $= global.GetStateDebugString();
...
Begin:
	`SETLOC("Waiting for latent game states before Sending Partial History");
	while (BuildingLatentGameState)
		Sleep(0.0f);
...
simulated state TurnPhase_MPUnitActions extends TurnPhase_UnitActions
{
	...
	simulated function string GetStateDebugString()
	{
		...
		DebugString $= "    ActionsAvailable: " $ ActionsAvailable();
		DebugString $= "    HasTacticalGameEnded: " $ HasTacticalGameEnded();
		DebugString $= "    HasTimeExpired: " $ HasTimeExpired();
		DebugString $= "\n\n";
...
Begin:
	...
	do
	{
		...
		{
			`SETLOC("Check for Available Actions: Waiting for latent game states before Sending Partial History");
			while (BuildingLatentGameState)
				Sleep(0.0f);
			...
			while (BuildingLatentGameState || (bWaitingForNewStates && (!HasTacticalGameEnded()) && (!HasTimeExpired())))
			{
				sleep(0.0);
			}
		...
		`SETLOC("Checking the remote status of the next initiative group");
		if (IsNextInitiativeGroupRemote())
		{
			`SETLOC("Ending Loop, skipping next initiative group");
			// If the next group is remote, then break out of the loop and let the TurnPhase_EndTurn process the NextInitiativeGroup.
			break;
		}

		`SETLOC("Processing the next initiative group");
	// Otherwise, the group is local, just process the next group while this client has priority.
	} until(!NextInitiativeGroup());
...
simulated state TurnPhase_EndPlayerTurn extends TurnPhase_UnitActions
{
	...
	{
		// Intentionally overridden to keep the NextInitiativeGroup() call from advancing further than intended
	}
...
simulated state EndTacticalGame
{
	simulated event BeginState(name PreviousStateName)
	{
		`log(self $ "::" $ GetStateName() $ "::" $ GetFuncName() @ `ShowVar(PreviousStateName),,'WTF');
	...
	function OnEndTacticalGame_SetupStateChange(XComGameState SetupState)
	{
		...
		for( PlayerIdx = 0; PlayerIdx < BattleData.PlayerTurnOrder.Length; ++PlayerIdx)
		{
			...
			if (Player != none)
			{
				...
				NewPlayer = XComGameState_Player(SetupState.ModifyStateObject(Player.Class, Player.ObjectID));
	...
	function Local_UpdateBattleData()
	{
		...
		NewBattleData = XComGameState_BattleDataMP(NewGameState.ModifyStateObject(class'XComGameState_BattleDataMP', CachedBattleDataRef.ObjectID));
	...
	function OnRemoteCommand(string Command, array<byte> RawParams)
	{
		if( Command ~= "HistoryReceived" )
		{
			if (bWaitingForEndTacticalGame)
			{
				SendRemoteCommand("EndTacticalGame");
			}
			else
			{
				// Got a previous history, wait for the end-game history to come through.
				bSendingPartialHistory = false;
			}
		}
...
simulated function bool PlayersAreWaitingForATurn()
{
	local XComGameState_BattleDataMP BattleData;
	BattleData = XComGameState_BattleDataMP(CachedHistory.GetGameStateForObjectID(CachedBattleDataRef.ObjectID));
	return (UnitActionInitiativeIndex < BattleData.PlayerTurnOrder.Length);
}
