...
var privatewrite int TotalScanHours; // The total amount of time it will take to complete this scan. Used for UI display.
...
function AddScanDays(int NumDays)
{
	ScanHoursRemaining += (NumDays * 24);
	PausedDaysRemaining += NumDays;
}
...
function bool IsScanRepeatable()
{
	return bScanRepeatable;
}
...
{
	MinDays = `ScaleStrategyArrayInt(default.MinScanDays);
	MaxDays = `ScaleStrategyArrayInt(default.MaxScanDays);
}
...
private function CheckForEventOverlaps()
{
	...
	foreach XComHQ.Projects(ProjectRef)
	{
		...
		// Some projects can overlap since they don't give popups on the Geoscape
		if(!ProjectState.bNoInterruptOnComplete && ProjectState.CompletionDateTime == ScanEndDateTime)
		{
