...
protected function ContextBuildVisualization()
{
	...
	if (CheatMan.bDebugXp)
	{
		...
		{
			History.GetCurrentAndPreviousGameStatesForObjectID(IterateXpMan.ObjectID, ActionMetadata.StateObject_OldState, ActionMetadata.StateObject_NewState, eReturnType_Reference, AssociatedState.HistoryIndex);
			OldXpMan = XComGameState_XpManager(ActionMetadata.StateObject_OldState);
			NewXpMan = XComGameState_XpManager(ActionMetadata.StateObject_NewState);
			...
			else
			{
				...
				DebugFlyover = X2Action_PlaySoundAndFlyOver(class'X2Action_PlaySoundAndFlyOver'.static.AddToVisualizationTree(ActionMetadata, self));
