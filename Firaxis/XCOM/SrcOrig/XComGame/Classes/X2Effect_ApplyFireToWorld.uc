...
static simulated event AddLDEffectToTiles( name EffectName, XComGameState NewGameState, array<TilePosPair> Tiles, array<int> TileIntensities )
{
	...
	for (Index = 0; Index < Tiles.Length; ++Index)
	{
		...
		InitialFireTileData.NumTurns = TileIntensities[Index] >= 0 ? TileIntensities[Index] : 1;
...
static simulated function bool TileEffectsPreventFire( TTile Tile, optional XComWorldData WorldData )
{
	if (WorldData == none)
		WorldData = `XWORLD;

	return WorldData.IsWaterTile( Tile ) || WorldData.TileContainsFire( Tile ) || WorldData.TileContainsAcid( Tile );
}
...
static simulated event AddEffectToTiles(Name EffectName, X2Effect_World Effect, XComGameState NewGameState, array<TilePosPair> Tiles, vector TargetLocation, float AbilityRadius, float AbilityCoverage, optional XComGameState_Unit SourceStateObject, optional XComGameState_Item SourceItemState, optional bool bUseFireChance)
{
	...
	{
		//   NonFlammable Tile Occupiers.
		TileActors = WorldData.GetActorsOnTile( Tiles[Index].Tile, true );

		foreach TileActors(TileActor)
		{
			PossibleFuel = XComDestructibleActor( TileActor );
			if (PossibleFuel != none)
				break;
		}
		...
		//   Water Tiles
		if (TileEffectsPreventFire(Tiles[Index].Tile, WorldData))
		{
			Tiles.Remove( Index, 1 );
			NumToRemove--;
			Index--;
		...
		if(X2Effect_ApplyFireToWorld(Effect).bCheckForLOSFromTargetLocation)
		{
			if(`XWORLD.VoxelRaytrace_Tiles(TopTile, Tiles[Index].Tile, VisInfo) == true)
			{
				if(VisInfo.BlockedTile != Tiles[Index].Tile)
				{
					...
					FuelOnTile = false;
					TileActors = `XWORLD.GetActorsOnTile(TestTile, true);
					foreach TileActors( TileActor )
					{
						if (TileActor == PossibleFuel)
							FuelOnTile = true;
					}

					if((PossibleFuel == none) || !FuelOnTile)
					{
						Tiles.Remove(Index, 1);
						NumToRemove--;
						Index--;
...
event AddWorldEffectTickEvents( XComGameState NewGameState, XComGameState_WorldEffectTileData TickingWorldEffect )
{
	...
	local XComDestructibleActor TileDestructible;
	local Actor TileActor;
	local array<Actor> TileActors;
	...
	if (DestroyedTiles.Length > 0)
	{
		...
		{
			TileActors = `XWORLD.GetActorsOnTile( Tile, true  );

			DamageEvent.DamageTiles.AddItem( Tile );

			foreach TileActors( TileActor )
			{
				TileDestructible = XComDestructibleActor( TileActor );

				if (TileDestructible == none)
					continue;

				if (TileDestructible.Toughness != none)
				{
					if (TileDestructible.Toughness.bInvincible)
						continue;

					if (TileDestructible.Toughness.bNonFlammable)
						continue;
				}

				DamageEvent.DestroyedActors.AddItem( TileDestructible.GetActorID() );
			}
