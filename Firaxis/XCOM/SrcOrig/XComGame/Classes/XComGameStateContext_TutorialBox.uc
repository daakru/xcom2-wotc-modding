...
protected function ContextBuildVisualization()
{
	...
	class'X2Action_ShowTutorialPopup'.static.AddToVisualizationTree(Metadata, self);
	
	// the visualizer complains if there is no associated state for a track, so just put it on a player state
	History = `XCOMHISTORY;
	foreach History.IterateByClassType(class'XComGameState_Player', PlayerState)
	{
		Metadata.StateObject_OldState = PlayerState;
		Metadata.StateObject_NewState = PlayerState;
		break;
	}
}
