...
protected function int GetHitChance(XComGameState_Ability kAbility, AvailableTarget kTarget, optional out ShotBreakdown m_ShotBreakdown, optional bool bDebugLog = false)
{
	...
	AddModifier(BaseChance, class'XLocalizedData'.default.BaseChance, m_ShotBreakdown, eHit_Success, bDebugLog);

	UnitState = XComGameState_Unit(`XCOMHISTORY.GetGameStateForObjectID(kAbility.OwnerStateObject.ObjectID));
	if (UnitState != None)
	{
		AddModifier(UnitState.GetCurrentStat(StatToRoll), class'X2TacticalGameRulesetDataStructures'.default.m_aCharStatLabels[StatToRoll], m_ShotBreakdown, eHit_Success, bDebugLog);
	}
