...
var const bool m_bEnableDynamicCapture;
...
simulated event SpawnSplashEmitter()
{
	...
	if(m_bEnableDynamicCapture)
		m_kSplashEmitter.LightEmitterMaterialConstant.SetTextureParameterValue('DynamicOrthoDepth', m_kDynamicDepthTexture );
	else
		m_kSplashEmitter.LightEmitterMaterialConstant.SetTextureParameterValue('DynamicOrthoDepth', m_kStaticDepthTexture);
...
simulated function ToggleRain()
{
	if( m_bInitialized )
	{
		...
		m_kDynamicDepthCapture.SetEnabled(m_bEnableDynamicCapture);
...
simulated function SetupRenderTextures()
{
	...
	if( m_kStaticDepthTexture == none || (m_kDynamicDepthTexture == none && m_bEnableDynamicCapture))
	{
...
simulated event UpdateStaticRainDepth()
{
	...
	{
		m_kStaticDepthCapture.SetEnabled(true);
		ReattachComponent(m_kStaticDepthCapture);
	}
}
