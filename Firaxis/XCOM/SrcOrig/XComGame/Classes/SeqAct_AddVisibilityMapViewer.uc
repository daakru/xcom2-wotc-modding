...
function ModifyKismetGameState(out XComGameState GameState)
{
	...
	{
		SquadViewer = XComGameState_SquadViewer(GameState.CreateNewStateObject(class'XComGameState_SquadViewer'));
...
function BuildVisualization(XComGameState GameState)
{
	...
	{
		ActionMetadata.StateObject_OldState = SquadViewer;
		ActionMetadata.StateObject_NewState = SquadViewer;
		...
		{
			ActionMetadata.VisualizeActor = SquadViewer.FindOrCreateVisualizer();
		}

		FOWAction = X2Action_UpdateFOW( class'X2Action_UpdateFOW'.static.AddToVisualizationTree( ActionMetadata, GameState.GetContext() ) );
