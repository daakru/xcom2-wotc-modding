...
simulated state DebugView
{
	...
	simulated function PostProcessInput()
	{
		...
		if(XCTC == none || !XCTC.bLockDebugCamera) {
			...
			CameraCache.POV.Rotation.Yaw += (PCOwner.PlayerInput.aTurn * fRotationUnitsPerSecond * fMoveSpeedMultipier);
			CameraCache.POV.Rotation.Pitch += (PCOwner.PlayerInput.aLookUp * fRotationUnitsPerSecond * fMoveSpeedMultipier);
...
simulated state TransitioningToStrategy extends InTransitionBetweenGames
{
	simulated function UpdateViewTarget(out TViewTarget OutVT, float DeltaTime)
	{
		...
		if (!bInitComplete)
		{
			...
			EnablePostProcessEffect('ShadowModeOn', false);
		}
	}
