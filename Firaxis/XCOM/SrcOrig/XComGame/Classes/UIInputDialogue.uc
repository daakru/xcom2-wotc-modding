...
var UIMechaListItem	 m_CrashDumpCheck;

var localized string m_CrashDumpLabel;
...
delegate TextInputClosedCallback_Report(string newTitle, string newText, bool bSendCrashDump);
delegate TextInputAcceptedCallback_Report(string newTitle, string newText, bool bSendCrashDump);
...
{
	local string CrashLabel;
	super.InitScreen(InitController, InitMovie, InitName);

	if (m_kData.DialogType == eDialogType_Report)
	{
		m_CrashDumpCheck = Spawn(class'UIMechaListItem', self);
		m_CrashDumpCheck.InitListItem('CrashDumpCheckMC');

		if(`ISCONTROLLERACTIVE)
		{
			CrashLabel = m_CrashDumpLabel @ class'UIUtilities_Text'.static.InjectImage(class'UIUtilities_Input'.static.GetGamepadIconPrefix() $ class'UIUtilities_Input'.const.ICON_X_SQUARE, 26, 26, -5);
		}
		else
		{
			CrashLabel = m_CrashDumpLabel;
		}

		m_CrashDumpCheck.UpdateDataCheckbox(CrashLabel, "", false);
		m_CrashDumpCheck.SetPosition(550, 770);
	}
...
simulated function bool RawInputHandler(Name Key, int ActionMask, bool bCtrl, bool bAlt, bool bShift)
{
	...
	if( Key == 'Enter' && m_kData.DialogType == eDialogType_SingleLine )
		...
		OnCancel();
	else if (Key == 'XboxTypeS_X' && ActionMask == 1)//on press
		m_CrashDumpCheck.Checkbox.SetChecked(!m_CrashDumpCheck.Checkbox.bChecked);
...
simulated function bool OnAccept( optional string strOption = "" )
{
	...
	bSendCrashDump = m_CrashDumpCheck.Checkbox.bChecked;
	...
	if(m_kData.DialogType == eDialogType_Report)
	{
		...
		if(OnClosedReport != none)
			OnClosedReport(ResultTitle, Result, bSendCrashDump);
	}
	else
	{
	...
	if(m_kData.DialogType == eDialogType_Report)
	{
		...
		if(OnAcceptedReport != none)
			OnAcceptedReport(ResultTitle, Result, bSendCrashDump);
	}
	else
	{
...
simulated function bool OnCancel( optional string strOption = "" )
{
	...
	if(m_kData.DialogType == eDialogType_Report)
	{
		OnClosedReport = m_kData.fnCallback_report;
		if(OnClosedReport != none)
			OnClosedReport(m_kData.strTitle, m_kData.strInputBoxText, false);
	}
	else
	{
		OnClosed = m_kData.fnCallback;
	...
	if(m_kData.DialogType == eDialogType_Report)
	{
		...
		if(OnCancelledReport != none)
			OnCancelledReport(m_kData.strTitle, m_kData.strInputBoxText, false);
	}
	else
	{
