...
var() int					WorkPerHour;
var() bool					bInstant;
var() bool					bProgressesDuringFlight; // Determines if the project pauses or not during flight
var() bool					bNoInterruptOnComplete; // Flag for if the project throws a popup upon completion
...
var() int					ProjectTimeBeforePausesHours; // for analytics computations
...
private function CheckForProjectOverlap()
{
	...
	{
		if (ActionState.bStarted && ActionState.EndDateTime == CompletionDateTime)
		{
			// If the projects' completion date and time is the same as a covert action time in the event queue, subtract an hour so the events don't stack
			// An hour is subtracted instead of added so the total "Days Remaining" does not change because of the offset.
			class'X2StrategyGameRulesetDataStructures'.static.AddHours(CompletionDateTime, -1);
			bModified = true;
		}
...
function PauseProject(optional int iCurrentWorkPerHour = -1)
{
	...
	ProjectTimeBeforePausesHours += class'X2StrategyGameRulesetDataStructures'.static.DifferenceInHours( `STRATEGYRULES.GameTime, StartDateTime );
...
//---------------------------------------------------------------------------------------
// Updates the project to save and refresh current points remaining. Assume the project is not being paused.
function UpdateProject(optional int iCurrentWorkPerHour = -1)
{
	// First store points remaining
	if (iCurrentWorkPerHour != -1)
	{
		UpdateProjectPointsRemaining(iCurrentWorkPerHour);
	}
	...
	// Then update the saved hours data and new start date
	ProjectTimeBeforePausesHours += class'X2StrategyGameRulesetDataStructures'.static.DifferenceInHours(`STRATEGYRULES.GameTime, StartDateTime);
	StartDateTime = `STRATEGYRULES.GameTime;
}
