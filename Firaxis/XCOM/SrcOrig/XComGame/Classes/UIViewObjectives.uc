...
simulated function bool ShouldShowGoodState(int ItemIndex)
{
	...
	ObjectiveState = XComGameState_Objective(History.GetGameStateForObjectID(m_arrObjectives[ItemIndex].ObjectID));
	return (ObjectiveState.GetStateOfObjective() == eObjectiveState_Completed);
}
...
{
	super.OnLoseFocus();
	`HQPRES.m_kAvengerHUD.NavHelp.ClearButtonHelp(); // bsg-jrebar (4/18/17): removing select button from objectives
}

simulated function OnReceiveFocus()
{
	super.OnReceiveFocus();
	UpdateNavHelp();  // bsg-jrebar (4/18/17): removing select button from objectives
}

// bsg-jrebar (4/18/17): removing select button from objectives
