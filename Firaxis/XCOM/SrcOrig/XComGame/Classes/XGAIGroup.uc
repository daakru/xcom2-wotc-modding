...
event InitializePostSpawn(XComGameState NewGameState, bool bRefresh=false)
{
	...
	local XComGameState_Unit LeadUnit;
	if (m_arrUnitIDs.Length > 0)
	{
		History = `XCOMHISTORY;
		RefreshMembers();
		if ( !bRefresh )
		{
			LeadUnit = XComGameState_Unit(History.GetGameStateForObjectID(m_arrUnitIDs[0]));
			m_kPlayer = XGAIPlayer(History.GetVisualizer(LeadUnit.ControllingPlayer.ObjectID));

			if (m_kPlayer != None)
			{
				if (m_kPlayer.m_eTeam == eTeam_Alien)
				{
				...
				if (bDisableFallback)
				{
					...
					SetFallbackStatus(false, , NewGameState);
				}
			}
		}
	}
...
function BaseInit(array<int> arrObjIDs, bool bRefresh)
{
	m_arrUnitIDs = arrObjIDs;
	InitializePostSpawn( none, bRefresh );
}
//------------------------------------------------------------------------------------------------
...
function InitTurn()
{
	...
	if( m_arrMember.Length > 0 && m_arrMember[0].GetTeam() != eTeam_XCom )
	{
...
function SetFallbackStatus(bool bFallback, optional StateObjectReference FallbackGroupRef, optional XComGameState NewGameState)
{
	local XComGameState_AIGroup GroupState;
	GroupState = GetGroupState();
	GroupState.SetFallbackStatus(bFallback, FallbackGroupRef, , NewGameState);
}
