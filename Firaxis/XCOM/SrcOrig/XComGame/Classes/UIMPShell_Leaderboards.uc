...
enum ELeaderboardSortType
{
	...
	eLeaderboardSortType_Score,
	eLeaderboardSortType_Time,
	eLeaderboardSortType_Replay,
	eLeaderboardSortType_SIZE
};

const LEADERBOARDS_SORT_SIZE = 5; //bsg-jneal (5.9.17): new enum entries extended size for checks so making new const to check against instead
...
var ELeaderboardSortType m_eSortOrder[LEADERBOARDS_SORT_SIZE]; //bsg-jneal (5.9.17): new enum entries extended size for checks so making new const to check against instead
...
simulated function InitScreen(XComPlayerController InitController, UIMovie InitMovie, optional name InitName)
{
	...
	m_kScreenHeader.InitScreenHeader('MPLeaderboardHeader');
	...
	m_kHeader = Spawn(class'UIPanel', self).InitPanel('', 'ChallengeLeaderboardHeader');
...
{
	if(m_eLeaderboardType == eMPLeaderboard_TopPlayers && m_LeaderboardsData.Length == 20)
	{
		m_iTopPlayersRank += 20;
...
{
	return UILeaderboard_ListItem(m_kList.CreateItem(class'UILeaderboard_ListItem')).InitListItem();
}

simulated function UpdateData()
{
	local TLeaderboardEntry kLeaderboardEntry;
	local int i;
	local UILeaderboard_ListItem kListItem;

	if (!m_kList.bIsInited)
	{
		SetTimer(0.1f, false, nameof(UpdateData));
		return;
	}
	...
	{
		kLeaderboardEntry = m_LeaderboardsData[i];
		kListItem = CreateNewListItem();
		kListItem.UpdateData(kLeaderboardEntry);
	}
